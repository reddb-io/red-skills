...
- **The lane label implies the run mode, and the CLAIM enforces it.** A `lane:scout` issue is a read-only investigation, but `run_mode=scout` reached the engine only through the `/go --scout` dispatch argv (#3026) — so a plain `--issues N` run picked the same issue up modeless and would have pushed, opened a PR and landed against it. The pair is declared once in `LANE_RUN_MODE_CONTRACT` (`apps/plugin-dev/src/core/lane-run-mode.ts`) and refused in `processIssue` before the claim is acquired, so every entrance — plain run, dispatch, the callers not written yet — inherits it rather than each dispatcher restating it. The guard (`apps/plugin-dev/tests/lane-run-mode-guard.test.ts`) pins the table against the castle drain's live `laneIsolated` set in BOTH directions: a new isolated lane cannot land without stating its mode (`null` is a legal answer — `lane:go` runs the ordinary ship pipeline), and a removed one cannot linger as fiction.
