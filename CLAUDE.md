...
- **Every way a change reaches the trunk is written down, and nothing merges that no other identity judged.** ADR 0154's land precondition is one sentence — no land without a non-voided PASSING verdict matching the head being merged, with the stable patch-id as the clean-rebase fallback — and its failure mode is that a merge happens in seven places across four layers, so a rule each of them spells for itself is a rule some of them spell differently. The vocabulary is declared once in `packages/shared/land-verdict.ts` (the refusal reasons, the subject, the decision, the `LandVerdictGate` port) because the engine and the daemon must ask the same question without inheriting the runtime's verdicts lane; the ANSWER lives in `apps/plugin-dev/src/core/land-precondition.ts`, which voids the stale row on its way to refusing — a superseded authorization left standing is one the next pass lands on. The entrances themselves are a declared table, `LAND_ENTRY_POINTS` (`apps/plugin-dev/src/core/land-entry-points.ts`), and the ratchet (`apps/plugin-dev/tests/land-entry-points-guard.test.ts`, #4138, Spec #4129) pins it in BOTH directions: a declared module must exist, export its named entry and actually reference the symbol it claims enforces, and every file under `apps/plugin-dev/src`, `apps/redskilled/src` and `packages/worker/src` that reaches a land primitive is declared or exempted one file at a time with its reason — so **a new entry point inherits the obligation the moment its first `landPr` or `handoffMergeCustody` lands.** Each entry states its verdict SOURCE, not merely that it has one, because the Spec's own risk note is that a blanket "no verdict, refuse" bricks the legitimate no-agent paths: the human adopting a branch signs `human:<login>` rather than being exempted. An entrance that asks nothing is declared `unenforced` with the gap it carries, and that list only ever shrinks — a hole written down is a hole somebody closes, and a hole nobody wrote down is one the next reader inherits as architecture.
