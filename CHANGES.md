...
- **what changed**:
  ...
  - **Format finding, 0.3 → 0.13.** The bump is not format-neutral. toon 0.13.0 added a cyclic-array wire whose meta keys are `order`, `discriminator`, `rows` and `common`, and its decoder tests for them exactly one level below the document root. A root-level keyed map whose entries carry any of those names — an ordinary shape, not a cyclic section — therefore fails to decode at all: `invalid cyclic array wire`. The defect is in the format, not in one implementation; the bundled decoder and pinned `tq 0.13.0` reject the same bytes the bundled encoder just wrote, and there is no newer toon to bump past it. Two levels deep the names are ordinary keys again, so every keyed map we write now sits under one envelope key. `activity-review`'s token cursor cache was the site this caught; it moved to a `files:` envelope, and `toon-tq-roundtrip.test.ts` pins the shape through the real binary so a writer that forgets the envelope reddens rather than stranding an operator on an unreadable lane.
