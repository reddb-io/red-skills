...
| **[review-adrs](./plugins/dev/skills/engineering/review-adrs/SKILL.md)** | Decision-record doctor — lints `.red/adr/` for contradictions, missing supersession, stale references, and number collisions, then reconciles each finding through a one-question-at-a-time interview (like `/start`); the agreement drives renumbering, supersession notes, the thematic index, and gated propagation into the Wiki + Memory graph. |
