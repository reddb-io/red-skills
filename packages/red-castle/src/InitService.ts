...
const SETUP_ISSUE_TRACKER_PATH = `.red-castle/${SETUP_ISSUE_TRACKER_DOC}`;
...
export interface TemplateMetadata {
   ...
   * `npx tsx .red-castle/main.ts` doesn't crash with ERR_MODULE_NOT_FOUND.
   */
...
export interface SandboxProviderEntry {
  readonly name: string;
  readonly label: string;
  /** Filename written to .red-castle/ (e.g. "Dockerfile" or "Containerfile") */
...
export const listCodexAuthModes = (): Array<{
...
}> => [
  {
    name: "api-key",
    label: "API key",
    description: "Use OPENAI_KEY from .red-castle/.env",
  },
  {
    name: "subscription",
...
export function getNextStepsLines(
  ...
  const usesCodexSubscription =
    agent.name === "codex" && options?.codexAuthMode === "subscription";
  const envStep = usesCodexSubscription
    ? "Set any required issue tracker env vars in .red-castle/.env (see .red-castle/.env.example). Codex subscription auth uses your host `codex login` session"
    : "Set the required env vars in .red-castle/.env (see .red-castle/.env.example)";
  ...
  if (issueTracker.name === "custom") {
    return [
      ...
      `3. Follow .red-castle/${SETUP_ISSUE_TRACKER_DOC} to edit the scaffolded files in place, build the image, and verify.`,
    ];
  }
  if (template === "blank") {
    const lines = ["Next steps:", `1. ${envStep}`];
    ...
    lines.push(
      "2. Read and customize .red-castle/prompt.md to describe what you want the agent to do",
      `3. Customize .red-castle/${mainFilename} — it uses the JS API (\`run()\`) to control how the agent runs`,
      `4. Add "sandcastle": "npx tsx .red-castle/${mainFilename}" to your package.json scripts`,
    ...
    let step = 1;
    const lines: string[] = ["Next steps:", `${step++}. ${envStep}`];
    if (agent.name === "claude-code") {
      lines.push(
        "   To use your Claude subscription instead of an API key, run `claude setup-token` on your host and paste the result into CLAUDE_CODE_OAUTH_TOKEN.",
      );
    ...
    lines.push(
      `${step++}. Add "sandcastle": "npx tsx .red-castle/${mainFilename}" to your package.json scripts`,
    ...
    lines.push(
      `${step++}. Read and customize the prompt files in .red-castle/ — they shape what the agent does`,
    );
    if (hasReviewer) {
      lines.push(
        `${step++}. Customize .red-castle/CODING_STANDARDS.md with your project's standards — the reviewer agent loads it during review`,
      );
    }
    ...
    content = content
      .replace(
        ...
        usesCodexSubscription
          ? `CODEX_HOME: "/home/agent/workspace/.red-castle/codex-home",`
          : "",
      )
      .replace(
...
- **Prompt files (\`.red-castle/*.md\`)** — replace the sentinel
...
- **\`.env.example\`** — replace the \`# TODO\` block with the real env var(s) your tracker needs, then tell the user to set them in \`.red-castle/.env\`.
...
export const scaffold = (
  ...
  Effect.gen(function* () {
    ...
    const fs = yield* FileSystem.FileSystem;
    const configDir = join(repoDir, ".red-castle");
    const usesCodexSubscription =
      agent.name === "codex" && codexAuthMode === "subscription";
    ...
    if (exists) {
      yield* Effect.fail(
        new Error(
          ".red-castle/ directory already exists. Remove it first if you want to re-initialize.",
        ),
      );
    }
...
# and copy it into a sandbox-local CODEX_HOME at startup.`
    ...
    // Rewrite main file with the selected agent factory, model, sandbox provider, and auth mode.
    yield* rewriteMainTs(
      configDir,
      agent,
      model,
