...
export const findCollidingWorktree = (
  ...
  existing.find((wt) => toPosix(wt.path) === toPosix(worktreePath));
...
export const isManagedWorktreePath = (
  worktreePath: string,
  worktreesDir: string,
): boolean => toPosix(worktreePath).startsWith(toPosix(worktreesDir));

/**
 * Whether a directory entry under `.red-castle/worktrees/` is orphaned — not
...
export const isOrphanedWorktreePath = (
  ...
  const normalizedEntry = toPosix(entryPath);
  for (const active of activeWorktreePaths) {
    if (toPosix(active) === normalizedEntry) return false;
  }
  return true;
};
...
> =>
  Effect.gen(function* () {
    ...
    // directory cleanup and lock pruning). Git reports forward slashes, so
    // downstream comparisons go through isOrphanedWorktreePath.
    ...
    if (entries !== null) {
      ...
      // .red-castle is a symlink, joining the un-canonicalized prefix produces
      ...
      // Remove any directory under .red-castle/worktrees/ that is not an active worktree
    ...
    const lockDir = join(repoDir, ".red-castle", "locks");
