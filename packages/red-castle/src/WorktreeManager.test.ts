...
describe("WorktreeManager.create", () => {
  it("creates a worktree at .red-castle/worktrees/<name>/", async () => {
    const repoDir = await setupRepo();
    const { path } = await run(create(repoDir));
    expect(path).toContain(join(repoDir, ".red-castle", "worktrees"));
  ...
  it("does not write upstream tracking config even when autoSetupMerge is enabled", async () => {
    ...
    const { stdout: trackingConfig } = await execAsync(
      `git config --get-regexp "branch\\.red-castle/no-tracking-test\\." || true`,
      { cwd: repoDir },
    );
...
describe("WorktreeManager.pruneStale", () => {
  ...
  it("removes orphaned directories under .red-castle/worktrees/", async () => {
    const repoDir = await setupRepo();
  ...
  it("does not remove active worktrees when .red-castle is a symlink", async () => {
    // Regression test for #470: git canonicalizes worktree paths, so when
    // .red-castle is a symlink the un-canonicalized entryPath never matched
    ...
    await symlink(externalDir, join(repoDir, ".red-castle"));
