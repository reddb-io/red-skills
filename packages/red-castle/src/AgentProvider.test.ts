...
describe("claudeCode factory", () => {
  ...
  it("parseStreamLine emits a usage event from a result line carrying usage + total_cost_usd", () => {
  ...
  it("parseStreamLine result without usage still yields only the result event", () => {
    const provider = claudeCode("claude-opus-4-7");
    const line = JSON.stringify({ type: "result", result: "no usage here" });
    expect(provider.parseStreamLine(line)).toEqual([
      { type: "result", result: "no usage here" },
    ]);
  });
...
// ---------------------------------------------------------------------------
// Reasoning/thinking events — one normalised `reasoning` ParsedStreamEvent per
// runner, populated from each CLI's CONFIRMED real output shape (claude streams
// discrete thinking blocks with text; codex + opencode expose reasoning only as
// per-turn/step token counts). Fixtures mirror live captures + the committed
// opencode reference (ideas/opencode-example-output.jsonl).
// ---------------------------------------------------------------------------
describe("reasoning stream events", () => {
  it("claude: a thinking content block → a text-bearing reasoning event", () => {
    const provider = claudeCode("claude-opus-4-8");
    const line = JSON.stringify({
      type: "assistant",
      message: {
        content: [
          {
            type: "thinking",
            thinking: "Let me work through the base case first.",
          },
        ],
      },
    });
    expect(provider.parseStreamLine(line)).toEqual([
      { type: "reasoning", text: "Let me work through the base case first." },
    ]);
  });

  it("claude: thinking flushes buffered text first, preserving order", () => {
    const provider = claudeCode("claude-opus-4-8");
    const line = JSON.stringify({
      type: "assistant",
      message: {
    ...
    const out = provider.parseStreamLine(line);
    expect(out[0]).toEqual({ type: "text", text: "Plan:" });
    expect(out[1]).toEqual({ type: "reasoning", text: "weigh A vs B" });
  });

  it("codex: turn.completed with reasoning_output_tokens → a token-bearing reasoning event (alongside usage)", () => {
    const provider = codex("gpt-5.4-mini");
    // Shape from a live `codex exec --json` capture.
    const line = JSON.stringify({
      type: "turn.completed",
      usage: {
        input_tokens: 19824,
    ...
    const out = provider.parseStreamLine(line);
    expect(out).toContainEqual({ type: "reasoning", tokens: 13 });
    expect(out.some((e) => e.type === "usage")).toBe(true);
  });

  it("codex: a turn with zero reasoning emits no reasoning event", () => {
    const provider = codex("gpt-5.4-mini");
    const line = JSON.stringify({
      type: "turn.completed",
      usage: {
    ...
    expect(
      provider.parseStreamLine(line).some((e) => e.type === "reasoning"),
    ).toBe(false);
  });

  it("opencode: step_finish with tokens.reasoning → a reasoning event AND a usage event", () => {
    const provider = opencode("anthropic/claude-opus-4-8");
    // Shape from ideas/opencode-example-output.jsonl.
    const line = JSON.stringify({
      type: "step_finish",
      part: {
        type: "step-finish",
        tokens: { total: 15663, input: 15479, output: 184, reasoning: 34 },
      },
    });
    expect(provider.parseStreamLine(line)).toEqual([
      { type: "reasoning", tokens: 34 },
      {
        type: "usage",
        usage: {
          inputTokens: 15479,
  ...
  it("opencode: a step with zero reasoning still emits usage (no reasoning event)", () => {
    const provider = opencode("anthropic/claude-opus-4-8");
    const line = JSON.stringify({
      type: "step_finish",
      part: {
    ...
    // reasoning === 0 → no reasoning event, but the token usage still flows so
    // cost is tracked. reasoningTokens is the accurate 0 for the step.
    expect(provider.parseStreamLine(line)).toEqual([
      {
        type: "usage",
        usage: {
  ...
  it("opencode: a step with no token fields emits nothing", () => {
    const provider = opencode("anthropic/claude-opus-4-8");
    const line = JSON.stringify({
      type: "step_finish",
...
// ---------------------------------------------------------------------------
// systemPrompt delivery — the substrate owns "how" per CLI: claude has a native
// flag (--append-system-prompt); codex/opencode have none, so the contract is
// prepended to the user prompt. Caller passes it once via AgentCommandOptions.
// ---------------------------------------------------------------------------
describe("systemPrompt delivery", () => {
  const CONTRACT = "RULE: emit <promise>DONE</promise> last.";

  it("claude: appends via --append-system-prompt; user prompt (stdin) is untouched", () => {
    const provider = claudeCode("claude-opus-4-8");
    ...
    expect(cmd.command).toContain("--append-system-prompt");
    expect(cmd.command).toContain(CONTRACT);
    expect(cmd.stdin).toBe("do the task"); // contract is NOT in the user turn
  });

  it("claude: no systemPrompt → no flag, byte-for-byte the old command", () => {
    const provider = claudeCode("claude-opus-4-8");
    const cmd = provider.buildPrintCommand({
      prompt: "x",
      dangerouslySkipPermissions: true,
    });
    expect(cmd.command).not.toContain("--append-system-prompt");
  });

  it("codex: no flag exists → contract is prepended to the stdin prompt", () => {
    const provider = codex("gpt-5.4-mini");
    const cmd = provider.buildPrintCommand({
      prompt: "do the task",
      dangerouslySkipPermissions: true,
    ...
    expect(cmd.command).not.toContain("--append-system-prompt");
    expect(cmd.stdin).toBe(`${CONTRACT}\n\n---\n\ndo the task`);
  });

  it("opencode: no flag exists → contract is prepended to the prompt arg", () => {
    const provider = opencode("anthropic/claude-opus-4-8");
    const cmd = provider.buildPrintCommand({
      prompt: "do the task",
      dangerouslySkipPermissions: true,
    ...
    expect(cmd.command).toContain(CONTRACT);
    expect(cmd.command).toContain("do the task");
    // prepended: contract appears before the task in the command string
    expect(cmd.command.indexOf(CONTRACT)).toBeLessThan(
      cmd.command.indexOf("do the task"),
    );
  });

  it("codex/opencode: no systemPrompt → prompt unchanged", () => {
    expect(
      codex("m").buildPrintCommand({
        prompt: "p",
        dangerouslySkipPermissions: true,
      }).stdin,
    ).toBe("p");
    const oc = opencode("m").buildPrintCommand({
      prompt: "p",
      dangerouslySkipPermissions: true,
    });
    expect(oc.command.endsWith("'p'")).toBe(true);
  });
});
