...
export type LoggingOption =
  ...
  | {
      ...
      /**
       * Optional masking hook applied to every textual field of an agent
       * stream event (raw line, text/reasoning message, result, tool-call
       * args) BEFORE the event reaches `onAgentStreamEvent` and before a
       * verbose raw line is written to the log file. Lets callers redact
       * secrets, host identity, or session links at capture time so no
       * downstream consumer ever sees them. Must be pure and fast; a hook
...
export const buildAgentStreamHandler = (
  ...
  const redact =
    logging.type === "file" && logging.redactLine
      ? buildSafeRedactor(logging.redactLine)
      : undefined;
  return (event) => {
    const masked = redact ? redactStreamEvent(event, redact) : event;
    if (userHandler) {
      try {
        userHandler(masked);
      } catch {
    ...
    if (verboseSink && masked.type === "raw") {
      verboseSink(masked.line);
    }
  };
};

/** Wrap a caller-supplied redactor so a throwing hook degrades to identity. */
const buildSafeRedactor = (
...
/**
 * Apply a redactor to every textual field of a stream event, preserving the
 * event shape. Non-textual variants pass through untouched.
 *
 * @internal
 */
...
export interface RunOptions<A extends AgentProvider = AgentProvider> {
   ...
   * `.red-castle/worktrees/`, `.red-castle/.env`, `.red-castle/logs/`,
   * `.red-castle/patches/`, and git operations.
   *
  ...
  /**
   * A system/contract prompt delivered ALONGSIDE the user prompt (prompt or
   * promptFile), once per run. The provider picks the delivery: claude uses
   * `--append-system-prompt` (a real appended system prompt); codex and
   * opencode, which have no such flag, prepend it to the user prompt. Lets a
   * caller keep durable operating rules out of the per-task prompt body.
   */
  ...
  /** Logging mode (default: { type: 'file' } with auto-generated path under .red-castle/logs/) */
