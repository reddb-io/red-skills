...
import { getAbortMetadata } from "./run.js";
...
describe("Orchestrator signal (AbortSignal)", () => {
  ...
  it("attaches observed session metadata when aborting mid-iteration", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "orch-host-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "hello.txt", "hello", "initial commit");

    const ac = new AbortController();
    const reason = new Error("cancelled after session");
    ...
    expect(getAbortMetadata(reason)?.iterations).toEqual([
      {
