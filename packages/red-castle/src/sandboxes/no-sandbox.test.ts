...
import { noSandbox, pickAllowedEnv } from "./no-sandbox.js";
...
describe("noSandbox", () => {
  ...
  describe("pickAllowedEnv", () => {
    it("keeps exact names and *-suffixed prefix globs, drops the rest", () => {
      const env = {
        HOME: "/home/x",
        PATH: "/bin",
        CLAUDE_CODE: "1",
      ...
      const picked = pickAllowedEnv(env, ["HOME", "PATH", "CLAUDE*"]);
    ...
    it("skips undefined values and returns empty for an empty allowlist", () => {
      expect(pickAllowedEnv({ A: undefined, B: "b" }, [])).toEqual({});
      expect(pickAllowedEnv({ A: undefined, B: "b" }, ["A", "B"])).toEqual({
        B: "b",
      });
    });
  });

  describe("hostEnvAllowlist", () => {
    const KEY = "RED_CASTLE_TEST_SENSITIVE";
    afterEach(() => {
      delete process.env[KEY];
    });

    itPosix(
      "spawned exec does NOT inherit host env keys outside the allowlist",
      async () => {
        process.env[KEY] = "super-secret";
        const provider = noSandbox({ hostEnvAllowlist: ["PATH", "HOME"] });
        const handle = await provider.create({
          worktreePath: process.cwd(),
          env: {},
        });
        const result = await handle.exec(`echo "v=[$${KEY}]"`);
    ...
    itPosix(
      "createOptions.env still injects on top of the filtered base",
      async () => {
        process.env[KEY] = "super-secret";
        const provider = noSandbox({ hostEnvAllowlist: ["PATH"] });
        const handle = await provider.create({
          worktreePath: process.cwd(),
          env: { INJECTED: "yes" },
        });
        const result = await handle.exec('echo "i=[$INJECTED]"');
        expect(result.stdout).toContain("i=[yes]");
      },
    );

    itPosix(
      "without the option the full host env is inherited (historical behavior)",
      async () => {
        ...
        const result = await handle.exec(`echo "v=[$${KEY}]"`);
        expect(result.stdout).toContain("v=[visible]");
      },
    );
  });
