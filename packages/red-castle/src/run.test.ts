...
describe("printFileDisplayStartup", () => {
  ...
  it("does not use clack (no @clack/prompts calls)", async () => {
    ...
    const clackSpy = vi
      ...
      .mockImplementation(() => {});
    printFileDisplayStartup({
      logPath: "/project/.red-castle/logs/main.log",
    });
  ...
  it("prints a relative log path when hostRepoDir equals process.cwd()", () => {
    const logPath = join(process.cwd(), ".red-castle", "logs", "main.log");
    printFileDisplayStartup({
      logPath,
      hostRepoDir: process.cwd(),
    });
    const allOutput = consoleSpy.mock.calls.flat().join(" ");
    expect(allOutput).toContain("tail -f .red-castle/logs/main.log");
  ...
  it("prints an absolute log path when hostRepoDir differs from process.cwd()", () => {
    ...
    const logPath = join(hostRepoDir, ".red-castle", "logs", "main.log");
    printFileDisplayStartup({
      logPath,
      hostRepoDir,
    });
    ...
    expect(allOutput).toContain(
      "tail -f /some/other/repo/.red-castle/logs/main.log",
    );
  });
});
...
describe("stream redaction (redactLine)", () => {
  ...
  it("redactStreamEvent masks every textual field, preserving shape", () => {
    const redact = (t: string) => t.replaceAll("secret", "[X]");
    expect(
      redactStreamEvent(
        mkEvent({ type: "raw", line: "a secret b" }) as never,
        redact,
      ),
    ).toMatchObject({ type: "raw", line: "a [X] b" });
    expect(
      redactStreamEvent(
        mkEvent({ type: "text", message: "secret" }) as never,
        redact,
      ),
    ).toMatchObject({ message: "[X]" });
    expect(
      redactStreamEvent(
        mkEvent({ type: "reasoning", message: "secret" }) as never,
        redact,
      ),
    ).toMatchObject({ message: "[X]" });
    expect(
      redactStreamEvent(
        mkEvent({ type: "result", result: "secret" }) as never,
        redact,
      ),
    ).toMatchObject({ result: "[X]" });
    expect(
      redactStreamEvent(
        mkEvent({
          type: "toolCall",
          name: "Bash",
          formattedArgs: "secret",
        }) as never,
        redact,
      ),
    ).toMatchObject({ formattedArgs: "[X]" });
  });

  it("non-textual variants pass through untouched", () => {
    const usage = mkEvent({
      type: "usage",
      inputTokens: 1,
      outputTokens: 2,
    ...
    expect(redactStreamEvent(usage as never, () => "nope")).toBe(usage);
    const sessionId = mkEvent({ type: "sessionId", sessionId: "s-1" });
    expect(redactStreamEvent(sessionId as never, () => "nope")).toBe(sessionId);
  });

  it("buildAgentStreamHandler masks events before the user handler and the verbose sink", () => {
    ...
    const handler = buildAgentStreamHandler({
      ...
      path: logPath,
      verbose: true,
      redactLine: (t) => t.replaceAll("sk-live-123", "[REDACTED_SECRET]"),
      onAgentStreamEvent: (e) => {
        if (e.type === "raw") seen.push(e.line);
      },
    });
    handler!(mkEvent({ type: "raw", line: "token sk-live-123 here" }) as never);
    expect(seen).toEqual(["token [REDACTED_SECRET] here"]);
    expect(readFileSync(logPath, "utf8")).toContain("[REDACTED_SECRET]");
    expect(readFileSync(logPath, "utf8")).not.toContain("sk-live-123");
    rmSync(dir, { recursive: true, force: true });
  });

  it("a throwing redactor degrades to identity instead of dropping events", () => {
    const seen: string[] = [];
    const handler = buildAgentStreamHandler({
      type: "file",
      path: join(tmpdir(), "unused.txt"),
      redactLine: () => {
        throw new Error("boom");
      },
      onAgentStreamEvent: (e) => {
        if (e.type === "text") seen.push(e.message);
      },
    });
    handler!(mkEvent({ type: "text", message: "still delivered" }) as never);
