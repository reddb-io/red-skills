...
export interface CreateWorktreeOptions {
   ...
   * `.red-castle/worktrees/`, `.red-castle/.env`, and git operations.
   *
...
export const createWorktree = async (
  ...
  const { hostRepoDir, worktreeInfo } = await Effect.gen(function* () {
    ...
    const lockDir = join(hostRepoDir, ".red-castle", "locks");
    ...
    const worktreeName = basename(info.path);
    ...
    const rollbackOnError = Effect.tapErrorCause(() =>
      Effect.gen(function* () {
        yield* Effect.promise(() => release(lockDir, worktreeName)).pipe(
          Effect.catchAll(() => Effect.void),
        );
        yield* WorktreeManager.remove(info.path).pipe(
          Effect.catchAll(() => Effect.void),
        );
      }),
    );
