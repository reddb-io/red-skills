...
**Config directory**:
The `.red-castle/` directory in a **host** repo containing sandbox configuration.
_Avoid_: ".red-castle folder", "sandcastle dir"
...
**Run log**:
A log file written to `.red-castle/logs/` during a run session. Like every **host**-repo-derived artifact (`.red-castle/worktrees/`, `.red-castle/.env`, `.red-castle/logs/`, `.red-castle/patches/`), its location is resolved against the **host** anchor — the `cwd` option (default `process.cwd()`), not the git repo root (ADR 0002). In **AFK** consumption `red-skills` passes `cwd` = the worker's attempt directory, so the whole `.red-castle/` artifacts tree nests inside that attempt directory and never materialises at a repo root.
