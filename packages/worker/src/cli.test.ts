...
describe("sandcastle CLI", () => {
  ...
  it("docker build-image errors when .red-castle/ is missing", async () => {
    ...
    try {
      ...
      expect(output).toContain("No .red-castle/ found");
    }
  });
  ...
  it("podman build-image errors when .red-castle/ is missing", async () => {
    const hostDir = await mkdtemp(join(tmpdir(), "cli-host-"));
    await initRepo(hostDir);
    await commitFile(hostDir, "hello.txt", "hello", "initial commit");
  ...
  it("init with full flag set scaffolds non-interactively in a non-TTY env", async () => {
    ...
    const entries = await readdir(join(hostDir, ".red-castle"));
