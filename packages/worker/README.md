...
2. Run `npx @ai-hero/sandcastle init`. This scaffolds a `.red-castle` directory with all the files needed.

```bash
npx @ai-hero/sandcastle init
```

3. Edit `.red-castle/.env` and fill in your default values for `CLAUDE_CODE_OAUTH_TOKEN` (run `claude setup-token` on your host to get one). To use an Anthropic API key instead, uncomment and fill in `ANTHROPIC_API_KEY`.

```bash
cp .red-castle/.env.example .red-castle/.env
```
...
4. Run the `.red-castle/main.ts` (or `main.mts`) file with `npx tsx`

```bash
npx tsx .red-castle/main.ts
```
...
// Opt-in host-env minimization: the agent inherits ONLY the allowlisted
// process.env keys (exact names or `*`-suffixed prefix globs) plus whatever
// is injected explicitly. Omit the option to inherit the full host env.
noSandbox({
  hostEnvAllowlist: ["PATH", "HOME", "LANG", "CLAUDE*", "ANTHROPIC*"],
});
```
...
File-mode logging accepts a `redactLine` hook applied to every textual field of an agent stream event (raw lines, text/reasoning messages, results, tool-call args) before the event reaches `onAgentStreamEvent` or the verbose log sink — use it to mask secrets, host identity, or session links at capture time. A hook that throws degrades to identity for that event.

### Structured logs are TOON, not JSON

This fork's structured log lanes (one record per line, e.g. the liveness lane) are written in [TOONL](https://github.com/reddb-io/toon) via `@reddb-io/toon` instead of JSONL — a deliberate divergence from upstream sandcastle. The format is markedly cheaper in tokens for AI agents that read these files back, which is the primary consumer in AFK orchestration. Readers sniff per line (JSON first, TOON fallback), so files written before a conversion — or by upstream code — still parse. Raw agent output passthrough and prose session logs are exempt: those bytes are not sandcastle's to re-encode.
...
const result = await run({
  ...
  sandbox: docker({
    ...
    // Generated images built with `sandcastle docker build-image` are aligned automatically.
    // Custom images must match these IDs. Pre-flight check catches mismatches.
    // containerUid: 1000,
    // containerGid: 1000,
  ...
  // .red-castle/ artifacts (worktrees, logs, env, patches) and git operations.
  ...
  promptFile: ".red-castle/prompt.md", // path to a prompt file
  ...
  // How to record progress. Default: write to a file under .red-castle/logs/
  logging: {
    type: "file",
    path: ".red-castle/logs/my-run.log",
...
const implResult = await sandbox.run({
  agent: claudeCode("claude-opus-4-8"),
  promptFile: ".red-castle/implement.md",
  maxIterations: 5,
});
...
> **Convention**: `sandcastle init` scaffolds `.red-castle/prompt.md` and all templates explicitly reference it via `promptFile: ".red-castle/prompt.md"`. This is a convention, not an automatic fallback — Sandcastle does not read `.red-castle/prompt.md` unless you pass it as `promptFile`.
...
`sandcastle init` prompts you to choose a sandbox provider (Docker or Podman), an issue tracker (GitHub Issues, Beads, or Custom), and a template, which scaffolds a ready-to-use prompt and `main.mts` suited to a specific workflow. If your project's `package.json` has `"type": "module"`, the file will be named `main.ts` instead. Choosing **Custom** scaffolds the project in a deliberately broken-until-configured state plus a `.red-castle/SETUP_ISSUE_TRACKER.md` prompt you feed to your coding agent, which wires up your own tracker by editing the scaffolded files in place. Five templates are available:
...
Scaffolds the `.red-castle/` config directory and builds the container image. This is the first command you run in a new repo. You choose a sandbox provider (Docker or Podman) during init — selecting Podman writes a `Containerfile` instead of `Dockerfile` and uses `sandcastle podman build-image` for the build step.

Init detects your host package manager (npm, pnpm, yarn, or bun) from a `packageManager` field or lockfile, defaulting to npm. Templates whose `main` file imports a host dependency — the planner templates import [Zod](https://zod.dev) for their `<plan>` output schema — prompt you to install it with that package manager when it isn't already in your `package.json`, so the first `npx tsx .red-castle/main.ts` doesn't fail with `ERR_MODULE_NOT_FOUND`.
...
Builds the Podman image from an existing `.red-castle/` directory. Use this after modifying the Containerfile.
...
Both **agent providers** and **sandbox providers** accept an optional `env: Record<string, string>` in their options. These environment variables are merged with the `.red-castle/.env` resolver output at launch time:
...
- Provider env (agent + sandbox) overrides `.red-castle/.env` resolver output for shared keys
...
Environment variables are also resolved automatically from `.red-castle/.env` and `process.env` — no need to pass them to the API. The required variables depend on the **agent provider** (see `sandcastle init` output for details).
...
### Config directory (`.red-castle/`)

All per-repo sandbox configuration lives in `.red-castle/`. Run `sandcastle init` to create it.

### Custom Dockerfile

The `.red-castle/Dockerfile` controls the sandbox environment. The default template installs:
