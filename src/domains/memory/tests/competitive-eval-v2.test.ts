...
describe("competitive eval v2 scaffold (#155)", () => {
  ...
  test("competitive eval viewer artifact renders self-contained HTML with embedded JSON", async () => {
    ...
    expect(artifact.html).toContain("<title>Memory Competitive Eval</title>");
    expect(artifact.html).toContain('id="memory-competitive-eval-data"');
    expect(artifact.html).toContain("memory.competitive_eval.v2");
