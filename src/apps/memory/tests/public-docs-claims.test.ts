...
// after the impl moved to src/apps/memory. Resolve it relative to this test
...
describe("public Memory documentation claims", () => {
  test("quickstart documents useful source-only plugin behavior", async () => {
    ...
    expect(readme).toContain("pnpm --dir src/apps/memory install");
    expect(readme).toContain("pnpm --dir src/apps/memory build");
