...
- **[review-adrs](./review-adrs/SKILL.md)** — Decision-record doctor. Lints `.red/adr/` for contradictions, missing supersession links, stale references, and number collisions, then **reconciles each finding through a one-question-at-a-time interview** (like `/start`) — the agreement drives the fix: renumbering, supersession notes, the thematic index, and gated propagation into the LLM Wiki and Memory graph. Read-only detect, agreement-gated writes.
