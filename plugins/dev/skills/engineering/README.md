...
- **[retake](./retake/SKILL.md)** — Issue resumption command. Finds the issue, linked PRs, matching branches, local worktrees, HITL state, and prints the next command to continue, fix, or hand the branch to `/ship`.
- **[requeue](./requeue/SKILL.md)** — Safe requeue for an issue parked behind an active `## Current blocker`. Clears the blocker, drops stale `ready-for-human`/`blocked:*` labels, and adds `ready-for-agent` as one transition so AFK preflight does not re-park it. `/hitl` is the interactive sibling for when the human decision still has to be extracted.
...
- **[setup-statusline](./setup-statusline/SKILL.md)** — Install or inspect the RedSkills statusline for the active host: Claude Code command-backed AFK block, Codex native `tui.status_line` footer plus self-heal hook.
- **[implement](./implement/SKILL.md)** — Interactive, human-driven PRD execution: guided `/tdd` at pre-agreed seams, typechecking, full-suite gate, `/review`, and `/ship`. The interactive counterpart to the autonomous `/afk` fleet.
