...
13. **`req:<PRD>` dependency-edge audit** — dependency edges must point at executable slices, never at a PRD (see `triage-labels.md` *Dependency Edges*; #907/#928 incident). List every open issue carrying a `req:N` label whose target #N carries `type:prd`, and emit **one warn line per offending edge** (`⚠️ #<dependent> req:<N> → #<N> is type:prd — re-point at its prd:<N> slices`). Resolve it read-only: for each `req:*` label in use (`gh label list --search req:` or scan `gh issue list --label req:<n>`), check the target's labels with `gh issue view <N> --json labels`; a target with `type:prd` is a finding. Report `✅` when no `req:<PRD>` edge exists, `⚠️` with the count and per-edge lines otherwise. Tag `→ /triage` (re-point each offending edge). During this read-only pass: never edit a label.
...
| `→ /triage` | `req:<PRD>` dependency edges (check 13) — re-point each offending edge at the target PRD's executable slices; `/triage` owns the authoring validation. |
...
| `req:<PRD>` dependency edge (check 13) | **do not silently re-label** — a re-point needs the author to pick the right executable slice(s). Surface each offending edge and delegate to `/triage` (re-point) or `/to-issues` (author the missing slice); never edit the `req:*` label here. | **delegate** |
