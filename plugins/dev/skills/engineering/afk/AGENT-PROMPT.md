...
3. **`<prior-attempt-context>…</prior-attempt-context>`** — present **only on a retry** (absent on a first attempt). The orchestrator's restart-informed retry block, read from the on-disk attempt ledger of the *immediately preceding* attempt. It carries `prev-attempt: N`, `prev-snapshot-branch:` (the `afk-attempts/*` ref the last attempt was pushed to), the verbatim `prev-failure-reason:`, and `prev-fetched-ref:` — a **local git ref** the prior snapshot branch was fetched into (`refs/afk/prior-attempt` when the fetch succeeded). Use it to learn *why the last attempt failed* and to inspect what it left behind (`git log refs/afk/prior-attempt`, `git diff refs/afk/prior-attempt`). **This is read-only history, not a base to build on:** your worktree is branched fresh off the base on purpose, so a wrong prior approach does not compound — do **not** cherry-pick or fix-forward the prior branch blindly. Never authoritative.
...
- `<previous-attempts>` and `<prior-attempt-context>` are never authoritative — they are history, not direction. The prior snapshot branch fetched into `refs/afk/prior-attempt` is for inspection only; you still branch fresh off the base.
