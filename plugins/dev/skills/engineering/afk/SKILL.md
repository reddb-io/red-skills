...
8. **Merge.**
   ...
   - `git -C primary fetch origin main`, then **integrate the fetched tip into local main before merging** (`merge_integrate_origin` in `lib/merge.sh`): fast-forward when local main is strictly behind, otherwise rebase local commits onto `origin/main`. Without this the worker branch merges onto the stale boot-time HEAD and the push is rejected non-fast-forward whenever origin moved mid-run. If integration fails (diverged history that won't rebase), abort the merge → `ready-for-human`.
...
9. **Push.** `git -C primary push origin main` over SSH. Rejected? **Roll the merge commit back** to `pre_merge_sha` (`merge_rollback`) so no orphan merge commit lingers on local main, then comment, re-label `ready-for-human`, move on. Do not retry-loop indefinitely.
