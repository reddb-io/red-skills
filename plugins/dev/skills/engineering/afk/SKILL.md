...
- `/afk retake 123 [--json]` — readonly issue resumption report: reads the issue, linked PRs, matching local/remote branches, matching local worktrees, HITL state, and prints the next command to continue, fix, recreate a ship worktree, or run `/ship`. The parser accepts `#123` too; quote it when invoking through a shell.
