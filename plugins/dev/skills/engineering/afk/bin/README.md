...
the dev plugin's TypeScript implementation, which lives at `src/apps/dev/` in
the repo root (the `@reddb-io/dev` domain, ADR 0034). Do not edit it by hand and do
...
pnpm -C src/apps/dev run bundle:bin   # writes this directory's afk.mjs
```
