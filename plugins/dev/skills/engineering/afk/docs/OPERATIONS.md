...
| `.red/tmp/afk-supervisor.log.jsonl` | Fleet supervisor firehose — append-only TOONL heartbeat records for the supervisor itself. Read with `tail -f .red/tmp/afk-supervisor.log.jsonl \| tq -p toonl -o json -r .msg`. Readers sniff legacy JSONL and mixed files; the lane is never rewritten in place. |
