...
| `.red/tmp/workers/{id}/{N}-a{n}/implementer-runtime.toon` | Per-run skill-projection artifact: enabled plugins, selected `plugin:skill` names, and before/after/delta fields for catalog payload bytes, skill count, and projection boot time. The same metrics are emitted as `worker.implementer-environment` and stamped into worker state for throughput readers. |
