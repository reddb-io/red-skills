...
| `.red/state/castle/afk-supervisor.log.toonl` | Fleet supervisor firehose — append-only TOONL heartbeat records for the supervisor itself (relocated to the castle state lane by ADR 0107 Wave 3; the legacy `.red/tmp/afk-supervisor.log.jsonl` and `.red/state/afk/` copies are boot-migrated). Read with `tail -f .red/state/castle/afk-supervisor.log.toonl \| tq -p toonl -o json -r .msg`. Readers sniff legacy JSONL and mixed files; the lane is never rewritten in place. |
