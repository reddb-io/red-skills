...
| `.red/state/afk/afk-supervisor.log.jsonl` | Fleet supervisor firehose — append-only TOONL heartbeat records for the supervisor itself (relocated to the state tier by issue #1685; the legacy `.red/tmp/afk-supervisor.log.jsonl` is boot-migrated and still read as a fallback). Read with `tail -f .red/state/afk/afk-supervisor.log.jsonl \| tq -p toonl -o json -r .msg`. Readers sniff legacy JSONL and mixed files; the lane is never rewritten in place. |
