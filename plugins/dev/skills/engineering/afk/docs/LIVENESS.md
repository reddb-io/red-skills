...
- **Fleet supervisor firehose** — the supervisor's shared lane at `.red/state/castle/afk-supervisor.log.toonl` (relocated to the castle state lane by ADR 0107 Wave 3; a one-time boot migration moves the legacy `.red/tmp/afk-supervisor.log.jsonl` and `.red/state/afk/` copies) is written as append-only TOONL records with an honest extension. Tail heartbeat messages with `tail -f .red/state/castle/afk-supervisor.log.toonl | tq -p toonl -o json -r .msg`. Readers sniff each row, so older JSONL rows and mixed legacy+TOONL files remain readable; history is never rewritten.
