...
- **Fleet supervisor firehose** — the supervisor's shared lane at `.red/state/afk/afk-supervisor.log.toonl` (relocated to the state tier by ADR 0098 / issue #1685; a one-time boot migration moves the legacy `.red/tmp/afk-supervisor.log.jsonl` and readers still fall back to it) is written as append-only TOONL records with an honest extension. Tail heartbeat messages with `tail -f .red/state/afk/afk-supervisor.log.toonl | tq -p toonl -o json -r .msg`. Readers sniff each row, so older JSONL rows and mixed legacy+TOONL files remain readable; history is never rewritten.
