...
- **Fleet supervisor firehose** — the supervisor's shared lane at `.red/tmp/afk-supervisor.log.jsonl` is written as append-only TOONL records while retaining the historical filename for transition compatibility. Tail heartbeat messages with `tail -f .red/tmp/afk-supervisor.log.jsonl | tq -p toonl -o json -r .msg`. Readers sniff each row, so older JSONL rows and mixed legacy+TOONL files remain readable; history is never rewritten.
