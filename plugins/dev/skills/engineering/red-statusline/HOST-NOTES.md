...
**1. Command-backed host (`statusLine` in `.claude/settings.json`)** — one host producer. The `dev bundle statusline` subcommand renders the local bedrock before appending the daemon-fed tail: stdin supplies model, effort, context, and subscription windows; local git supplies repository, branch, and diff; one bounded local socket read supplies remote counters and Worker rows (ADR 0141). The bedrock renders even when redskilled is absent, with `rsk=bedrock-only` stating the missing tail. The shell invokes the dev bundle once — invoking `redskilled statusline` beneath it repeats the daemon document (#3559). Keep this path on the local socket rather than the MCP server (ADR 0132 decision 9); a `statusLine` entry is a shell command, and an MCP handshake per render tick would make server startup part of the prompt path.
