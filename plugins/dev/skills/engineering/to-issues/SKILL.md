...
- **`req:N` must reference an executable issue — never a `type:prd` parent.** The cascade fires on *close*, and a PRD closes on manual bookkeeping, often long after its substance is delivered — so a `req:<PRD>` edge strands its dependents indefinitely (on 2026-07-02, 14 slices froze behind two fully-delivered-but-open PRDs). Point at the concrete slice(s) delivering what the dependent needs; when none exist yet, publish them first (dependency order above already requires blockers first). Runtime enforcement is tracked in #1048.
