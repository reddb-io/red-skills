...
The installed version must be at least `0.26.2`. Record the same floor in `.red/config.yaml` under `host_binaries.tq.version` so `/red-doctor` can red-flag absence or drift and print the same package-manager fix. Do not document or offer a jq fallback.
...
- **Otherwise** write the entry into `.claude/settings.json` (create the file if missing, merge with existing keys via `jq` if present):
  ...
  ```json
  {
    "statusLine": {
      "type": "command",
      "command": "sh -c 'N=$(command -v node 2>/dev/null || ls -1 /usr/local/bin/node /opt/homebrew/bin/node /usr/bin/node \"$HOME\"/.volta/bin/node \"$HOME\"/.asdf/shims/node \"$HOME\"/.nodenv/shims/node \"$HOME\"/.nvm/versions/node/*/bin/node \"$HOME\"/.local/share/fnm/node-versions/*/installation/bin/node \"$HOME\"/.fnm/node-versions/*/installation/bin/node 2>/dev/null | sort -V | tail -1); [ -z \"$N\" ] && exit 0; s=$(ls -1 \"$HOME\"/.red/redskilled/bundles/statusline-*.bundle.min.mjs 2>/dev/null | sort -V | tail -1); d=$(ls -1 \"$HOME\"/.red/redskilled/bundles/redskilled-*.bundle.min.mjs 2>/dev/null | sort -V | tail -1); F=\"$HOME\"/.red/redskilled/bin/statusline-fast; r=$s; [ -z \"$r\" ] && r=$d; if [ -x \"$F\" ] && [ -n \"$r\" ]; then p=$(cat); printf %s \"$p\" | \"$F\"; ( mkdir -p .red/state/statusline; printf %s \"$p\" | \"$N\" \"$r\" statusline > .red/state/statusline/last-render.txt.tmp 2>/dev/null && mv .red/state/statusline/last-render.txt.tmp .red/state/statusline/last-render.txt ) >/dev/null 2>&1 & elif [ -n \"$r\" ]; then \"$N\" \"$r\" statusline; else echo \"redskilled unreachable — Worker state unknown\"; fi; exit 0'",
      "refreshInterval": 60
    }
  }
  ```

  Do **not** use `$CLAUDE_PLUGIN_ROOT` here: Claude Code does not export it (nor `$CLAUDE_PROJECT_DIR`) to a `statusLine` command — only to plugin hooks and MCP/LSP subprocesses — so that form expands to an empty path and renders blank. Likewise, the command resolves `node` explicitly rather than a bare `exec node`: Claude Code runs the statusLine in a **non-interactive shell** that does not source the user's shell rc, so a Node installed through a version manager (nvm, fnm, volta, asdf, nodenv, …) is off `PATH` and bare `exec node` fails with `node: not found` — another silent blank. `command -v node` covers every host with Node on `PATH` (system package, Homebrew, no version manager); only when that misses does it scan the common install roots, newest wins. **The producer is the `redskilled` bundle, resolved out of the daemon's own home.** The command runs the **highest-version stable copy** the daemon keeps (`ls -1 "$HOME"/.red/redskilled/bundles/redskilled-*.bundle.min.mjs | sort -V | tail -1` — `sort -V` picks the highest semver, NOT `ls -t | head -1` which picks newest-by-mtime and can resolve an OLD version when an older file was touched or re-written more recently). That directory is the copy nothing on the machine prunes: every writer that points a systemd `ExecStart` at a bundle stabilizes it there first, because an absolute path into an npx or mise cache is a dead unit wearing a longer name (#3554). Resolving a file already on disk keeps the network out of the hot path (ADR 0084) — never alter this command to fetch. The project root is read from `workspace.project_dir` in the JSON Claude Code pipes on stdin (no argument needed). **One producer, and the host prints what it hands back.** The redskilled bundle is asked for the whole line: the repo header and the Worker rows under it. The host formats no Worker and orders no row. This was two producers — a dev bundle muted with `--no-workers`, the daemon's own `statusline` echoed under it (#2928, #3166) — and then one dev bundle that composed the tail itself (#3559). Both recipes are dead for the same reason: **ADR 0147 deleted the dev runtime.** `dev-3.21.0.bundle.min.mjs` is the last dev bundle that will ever exist, so a machine still holding one kept resolving it and rendered a 3.21.0-era line against v4 state lanes it cannot read — frozen at that version, silent about Workers, and reporting success the whole time. Do not reintroduce `--no-workers` or any `dev-*` glob: `apps/plugin-dev/tests/statusline-command-doc.test.ts` refuses both. What holds either way is the count: **exactly one renderer draws a Worker row.** A host with no renderer at all prints a stated absence rather than nothing, because a blank line is indistinguishable from a machine with no Workers: the command's `else` branch prints "redskilled unreachable — Worker state unknown" — the same sentence the daemon's own `statusline` prints when its socket does not answer, so the operator reads one absence however deep the failure is (#3074). That `else` branch is not the host rendering a Worker row — ADR 0130 rule 10 holds, and there is no Worker here to render. **An existing `dev-*` bundle is now inert:** the command no longer globs it, so nothing resolves it and it costs only disk — reclaim it with `rm -f ~/.cache/red-skills/bundles/dev-*.bundle.min.mjs`. **The command ends in `; exit 0` on purpose:** the rendered line is the point, and a host that cannot render one is not a failed command. Without it the last statement is a bare test whose status becomes the whole `sh -c`'s status, so a host with no resolvable bundle rendered its header correctly and still reported failure (#3073). Write this command **byte-identical** to the copy in the `red-statusline` skill's `HOST-NOTES.md`; `apps/plugin-dev/tests/statusline-command-doc.test.ts` fails when the two drift or when either can still exit non-zero. This subsection writes only the Claude Code adapter; the shared producer remains host-agnostic, and Codex's adapter lives in the `red-statusline` skill because it edits global `~/.codex/config.toml` rather than repo-local `.claude/settings.json`.
...
Explain the release credential prerequisite before proposing the config: the generated tag-cutting job requires a repository secret named `RELEASE_PAT`, containing a credential that can write repository contents and whose pushes may start downstream workflows. The workflow keeps API calls on `GITHUB_TOKEN` and uses `RELEASE_PAT` as the checkout-persisted Git push identity in **every job that pushes a ref** — the tag-cutting job and, under the Version-PR trigger, the job that pushes the version branch. A branch pushed with `GITHUB_TOKEN` starts no `pull_request` runs, so its PR's required checks never begin and branch protection blocks the merge forever. Ask the operator to provision it before the first release; when it is absent, the job fails at checkout before creating a tag or GitHub Release.
