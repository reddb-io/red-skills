...
- **Required host binaries** — install pinned `tq` (`TQ_VERSION=v0.2.6`) through the toon repo installer and record `host_binaries.tq.version` so `/red-doctor` can enforce the no-jq-fallback TOON/TOONL contract
...
- `tq --version` and `.red/config.yaml` `host_binaries.tq.version` — is the required host binary present and pinned to `0.2.6`?
