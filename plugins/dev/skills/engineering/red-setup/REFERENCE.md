...
- **Execution daemon** — provision the host-scoped `redskilled` daemon (ADR 0130) by running `redskilled provision`, and optionally install the supervising user unit. The daemon's home `~/.red/redskilled/` is owned and created by `redskilled` itself, not by this skill
