...
3. **Provision the shared Repo store.** Create `.red/red.rdb` exactly here, and leave an existing file untouched on re-run. If `.red/memory/graph.rdb` exists and the memory config still points at it, migrate by copying that legacy file to `.red/red.rdb` when the shared store is absent, then repoint `plugins.memory.storePath` to `.red/red.rdb`. This is the one mechanical memory-store migration; memory graph collections and `rsp` elisions stay isolated by collection name inside the same file.
