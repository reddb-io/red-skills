...
1. If `.red/config.yaml` already exists at the repo root, apply the named surgical exceptions: add/update the `plugins.<name>.enabled` flags to match Section A0, add/update the top-level `rsp:` defaults, write the exact user-confirmed ordered command(s) under canonical `plugins.dev.afk.setup`, and — only when each is separately confirmed in Section G — merge the exact Validation moments schedule under `plugins.dev.afk.validation` and the exact `release.*` block. Preserve every unrelated key and comment. Log `.red/config.yaml present - merged plugin activation, rsp defaults, and confirmed AFK setup/Validation and Release authority; left the rest as-is`.
