...
3. **Provision the shared Repo store.** Run `rsp setup` from the repo root. This is the supported provisioning command: it writes the `rsp:` config block, creates `.red/red.rdb` exactly there, and leaves an existing file untouched on re-run. If `.red/memory/graph.rdb` exists and the memory config still points at it, `rsp setup` migrates by copying that legacy file to `.red/red.rdb` when the shared store is absent, then repoints `plugins.memory.storePath` to `.red/red.rdb`. This is the one mechanical memory-store migration; memory graph collections and `rsp` elisions stay isolated by collection name inside the same file.
