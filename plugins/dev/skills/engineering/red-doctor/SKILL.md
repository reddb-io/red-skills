...
18. **Required host binaries** — verify the host has every required pinned binary recorded by `/red-setup`. For ADR 0097, `tq` is mandatory: read `.red/config.yaml` `host_binaries.tq.version` (pin `0.2.6`) and the observed `tq --version` fact, then report `❌` when `tq` is absent or version-drifted from the pin. Absence or version drift is a red finding, not a warning, because there is no jq fallback for RedSkills-owned TOON/TOONL logs. Tag `→ /red-setup` and print the canonical fix: `TQ_VERSION=v0.2.6 curl -fsSL https://raw.githubusercontent.com/reddb-io/toon/v0.2.6/install.sh | sh`. The pure classifier is `apps/dev/src/core/host-binary-doctor.ts` (`auditHostBinaries`); every observed host fact is injected. During this read-only pass: never run installer scripts during Pass 1, never mutate `.red/config.yaml`, and never offer a jq fallback.
