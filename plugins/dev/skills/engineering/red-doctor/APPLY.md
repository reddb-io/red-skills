...
| Red operational probe (check 21) | **run the probe's own canonical fixer** from the registry. For the HTTPS-remote proof probe, rewrite each named local HTTPS remote to its SSH form. A decline leaves the remote config byte-identical and the finding open. | **confirm each** — rewrites git remote configuration |
