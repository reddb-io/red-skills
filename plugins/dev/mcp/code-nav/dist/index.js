...
import { z } from "zod";
import { loadServers, buildExtensionIndex, } from "./config.js";
...
function sessionForFile(filePath) {
    const ext = extname(filePath);
    const name = extIndex.get(ext);
    if (!name) {
        throw new Error(`no language server configured for '${ext}'. ` +
            `Configured extensions: ${[...extIndex.keys()].join(", ") || "(none)"}.`);
    }
    let session = sessions.get(name);
    if (!session) {
        session = new LspSession(servers[name], ROOT);
...
function allSessions() {
    for (const name of Object.keys(servers)) {
        if (!sessions.has(name)) {
            sessions.set(name, new LspSession(servers[name], ROOT));
        }
    }
    return [...sessions.values()];
}
function loc(uri, range) {
    return `${uriToRelative(uri, ROOT)}:${range.start.line + 1}:${range.start.character + 1}`;
}
const server = new McpServer({ name: "code-nav", version: "0.1.0" });
const posShape = {
...
server.registerTool("goto_definition", {
    ...
    description: "Resolve the symbol at a file position to its definition(s). Returns " +
...
server.registerTool("find_references", {
    ...
    description: "Find every reference to the symbol at a file position across the " +
    ...
    const locs = await sessionForFile(file).references(file, line, character, includeDeclaration ?? true);
    if (locs.length === 0)
        return text("No references found.");
    return text(`${locs.length} reference(s):\n` +
        locs.map((l) => loc(l.uri, l.range)).join("\n"));
});
server.registerTool("document_symbols", {
    title: "Document symbols",
    description: "List the symbols (functions, types, methods, …) defined in a file, " +
...
server.registerTool("hover", {
    ...
    description: "Get the type signature and documentation for the symbol at a file " +
...
server.registerTool("workspace_symbols", {
    ...
    description: "Find symbols by name across the whole workspace. Start here when you " +
    ...
    const results = await Promise.all(allSessions().map((s) => s.workspaceSymbols(query).catch(() => [])));
...
function renderSymbols(syms, depth = 0) {
...
async function main() {
    ...
    process.stderr.write(`code-nav MCP ready (root=${ROOT}, languages=${Object.keys(servers).join(",")})\n`);
}
main().catch((err) => {
