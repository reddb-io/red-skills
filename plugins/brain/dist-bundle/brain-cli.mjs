...
async function handleHook(lifecycle, runner) {
  ...
  const stateDir = join5(config.rootDir, ".red", "brain", "sessions");
  await mkdir3(stateDir, { recursive: true });
  await writeFile2(
    join5(stateDir, "last-session.json"),
    JSON.stringify(
      {
        runner,
        lifecycle,
...
async function openBrainRuntime(startDir = process.cwd()) {
  ...
  return {
    config,
    store,
    project: basename2(config.rootDir)
  };
}
...
async function capture(args) {
  ...
  await withBrainRuntime(async ({ config, store }) => {
    ...
    const attachments = await copyAttachments(config.rootDir, identity.id, listFlag(flags, "attach"));
    const artifact = await store.capture({
      ...baseInput,
      metadata: attachments.length > 0 ? { attachments } : void 0
    });
    printJson(artifact);
  });
}
...
function printHelp() {
  ...
  init
  status
  capture [text] --title <title> --kind <${ARTIFACT_KINDS.join("|")}> --tag <tag> --attach <path>
