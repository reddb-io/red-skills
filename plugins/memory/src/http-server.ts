...
async function handleRequest(
  ...
  if (url.pathname === "/api/competitive-radar") {
    sendJson(
      res,
      200,
      await buildMemoryCompetitiveRadar(opts.store, opts.rootDir, { now: opts.now }),
    );
    return;
  }
  ...
  if (url.pathname === "/competitive-eval") {
    sendHtml(
      res,
      buildCompetitiveEvalViewerArtifact(await evaluateCompetitiveEvalV2({ now: opts.now })).html,
    );
    return;
  }
