...
export async function buildMemoryLayersReport(store, opts = {}) {
    const [nodes, rawEdges, docs, vector, events] = await Promise.all([
        store.listNodes(opts.now),
        store.listEdges(),
        store.listDocs(),
        store.vectorStatus(),
        readMemoryEvents(store).catch(() => []),
    ]);
    ...
    return {
        ...
        competitor_alignment: [
            {
                competitor: "neo4j-labs/agent-memory",
                maps_to: ["short-term", "long-term", "reasoning", "vectors"],
                advantage: "Memory exposes the same layered shape over embedded project-local RedDB, without requiring a Neo4j service.",
            },
            {
                competitor: "rohitg00/agentmemory",
                maps_to: ["short-term", "long-term", "docs-code", "vectors"],
                advantage: "Memory combines MCP/HTTP/hook access with RedDB graph, docs, and vector diagnostics for coding-agent workflows.",
            },
            {
                competitor: "safishamsi/graphify",
                maps_to: ["docs-code", "reasoning", "vectors"],
                advantage: "Memory keeps code/doc graph evidence queryable in the same persistent RedDB store used by recall and hooks.",
            },
        ],
        recommended_next_actions: recommendations(layers),
    };
}
function shortTermLayer(rawEvents) {
...
function reasoningLayer(nodes, edges) {
    const reasoning = nodes.filter((node) => node.node_type === "why_note" || node.properties.tier === "reasoning");
...
function docsCodeLayer(nodes, edges, docs) {
    const byType = countTypes(nodes);
    const codeEdges = edges.filter((edge) => ["IMPORTS", "CALLS", "USES_TYPE", "DEFINED_IN", "CONTAINS"].includes(edge.label)).length;
    const codeNodes = (byType.file ?? 0) + (byType.symbol ?? 0) + (byType.import ?? 0);
    return {
        id: "docs-code",
...
function recommendations(layers) {
    const actions = [];
    ...
    if (actions.length === 0)
        actions.push("Memory layers are populated and ready");
    return actions;
}
function layerStatus(layers, id) {
