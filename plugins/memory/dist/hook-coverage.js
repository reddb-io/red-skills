...
const EVENTS = ["SessionStart", "PostToolUse", "Stop", "PreCompact"];
...
export async function buildHookCoverageReport(rootDir, opts = {}) {
    const root = resolve(rootDir);
    const config = await readConfig(root);
    const pluginRoot = opts.pluginRoot ?? PLUGIN_ROOT;
    const runners = await Promise.all(RUNNERS.map((runner) => runnerCoverage(runner, pluginRoot, config)));
    ...
    if (!config)
        gaps.unshift("memory is not initialized");
    if (!config)
        actionableGaps.unshift("memory is not initialized");
...
async function runnerCoverage(runner, pluginRoot, config) {
...
function eventCoverage(runner, event, manifest, config) {
    const groups = manifest?.hooks?.[event] ?? [];
    const commands = groups.flatMap((group) => (group.hooks ?? []).map((hook) => hook.command).filter((command) => Boolean(command)));
    ...
    const fallbackCovered = runner === "codex" &&
        event === "PreCompact" &&
        !wired &&
        config?.mode === "graph" &&
    ...
    const notes = [];
    if (!wired && supported)
        notes.push("not wired by runner manifest");
    if (wired && !config)
        notes.push("memory config is absent");
    if (wired && config?.mode === "markdown-only")
        notes.push("disabled by markdown-only mode");
    ...
    return {
        ...
        matcher: groups.map((group) => group.matcher).find((value) => Boolean(value)) ?? null,
        fail_open: commands.length > 0 && commands.every((command) => command.includes("printf \"{}\"")),
        notes,
    };
}
function runnerGaps(runner, manifest, events, config) {
    const gaps = [];
    if (!manifest)
        gaps.push(`${runner} hook manifest is missing`);
    for (const event of events) {
        if (!event.wired && event.supported)
            gaps.push(`${runner} ${event.event} is not wired`);
...
function actionableRunnerGaps(runner, manifest, events, config) {
    const gaps = [];
    if (!manifest)
        gaps.push(`${runner} hook manifest is missing`);
    for (const event of events) {
        if (event.supported && !event.wired)
            gaps.push(`${runner} ${event.event} is not wired`);
        if (event.wired && config?.mode === "graph" && !event.enabled) {
            gaps.push(`${runner} ${event.event} is wired but disabled`);
        }
...
function enabledHookFlags(hooks) {
    return Object.entries(hooks)
        .filter(([, enabled]) => enabled)
        .map(([flag]) => flag);
}
function recommendations(config, runners, gaps, actionableGaps) {
    const out = [];
    if (!config) {
    ...
    if (gaps.some((gap) => gap.includes("codex has no PreCompact")) &&
    ...
    if (out.length === 0)
        out.push("hook coverage is ready; no action required");
    return out;
}
