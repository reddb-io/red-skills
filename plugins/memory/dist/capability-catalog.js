...
export async function buildMemoryCapabilityCatalog(store, rootDir, opts = {}) {
...
function vectorCapabilityStatus(vector) {
    if (vector.overall === "ready")
        return "ready";
...
function recommendedActions(docs, hooks, extraction) {
    const actions = [];
    if (docs.total_docs === 0)
        actions.push("run `memory ingest . --root .` to populate documentation coverage");
    if (docs.ungrounded_docs > 0)
        actions.push("run `memory ingest . --root .` to refresh graph grounding for docs");
    ...
    if (actions.length === 0)
        actions.push("Memory capability catalog is ready");
