...
import { readDoc } from "./doc-search.js";
export async function buildDocEvidencePack(store, input) {
...
function renderMarkdown(pack) {
    ...
    if (pack.doc.path)
        lines.push(`- Path: ${pack.doc.path}`);
    if (pack.doc.rid != null)
        lines.push(`- Doc rid: ${pack.doc.rid}`);
    if (pack.doc.title)
        lines.push(`- Title: ${pack.doc.title}`);
    ...
    if (pack.related.related_docs.length > 0) {
        ...
        for (const doc of pack.related.related_docs.slice(0, 12)) {
            lines.push(`- ${doc.path} (${doc.shared_references} shared reference(s), rid ${doc.rid})`);
        }
    }
    if (pack.doc.body) {
        lines.push("");
        ...
        if (pack.doc.truncated) {
            lines.push("");
            lines.push(`_Body truncated to ${pack.doc.returned_bytes}/${pack.doc.body_bytes} byte(s)._`);
        }
    }
    return `${lines.join("\n")}\n`;
}
