...
import { competitiveEvalFixture, competitiveInteropFixtures, } from "./competitive-fixtures.js";
...
import { lintMemoryRecords } from "./lint.js";
import { agentmemoryBaselineCommandFromEnv, createAgentmemoryCliBaselineAdapter, createNeo4jAgentMemoryCliBaselineAdapter, defaultCliExecutor, neo4jAgentMemoryBaselineCommandFromEnv, } from "./live-baseline-adapters.js";
 ...
 * the README claims.
 */
export const graphifyOutSummary = {
...
function competitor(name) {
    ...
    if (!found)
        throw new Error(`missing competitor baseline: ${name}`);
    return found;
}
...
function buildAssertions() {
    ...
    return [
        ...
        },
        {
            ...
            pass: memory.recallLatencyP50Ms != null &&
        ...
        },
        {
            ...
            pass: memory.nerExtractionQuality !== "python-ml-stack" &&
...
export function evaluateCompetitiveBaseline(now = new Date(0)) {
...
export function renderComparisonTable(report) {
    const lines = [
        "| Axis | `memory` | `graphify` | `agent-memory` | Framing |",
        "|------|----------|------------|----------------|---------|",
    ];
    for (const row of report.rows) {
        lines.push(`| ${row.axis} | ${row.memory} | ${row.graphify} | ${row.agentMemory} | ${row.framing} |`);
    }
    return `${lines.join("\n")}\n`;
}
class FixtureRecallStore {
    fixture;
    constructor(fixture) {
...
function decisionCount(decisions, decision) {
    return decisions
        .filter((item) => item.decision === decision)
        .reduce((sum, item) => sum + item.count, 0);
}
export function evaluateCompetitiveInteropReport(opts = {}) {
...
function rawCorpusChars(fixture) {
...
async function evaluateFoundationEvidenceGate(fixture, opts) {
    ...
    try {
        ...
        return {
            ...
            agentmemoryLiveBaseline: {
                state: "adapter-ready",
                implemented: true,
                note: "Agentmemory and Neo4j Agent Memory live baseline adapters are available through opt-in eval:competitive:v2 flags; normal runs remain checked-in fixture only.",
            },
        };
    }
    finally {
...
async function seedFoundationEvidence(store, fixture, now) {
    const rids = new Map();
    ...
    for (const edge of fixture.edges) {
        ...
        const label = String(edge.label ?? "REFERENCES");
    ...
    await appendMemoryEvent(store, parseMemoryEvent({
...
async function seedFoundationDocCoverage(store, root, rids, fixture, now) {
    ...
    const docRid = await store.upsertDoc({
        ...
        body: "Competitive Memory Evidence links README claims, hook coverage, doc coverage, vector diagnostics, and the operational dashboard.",
...
async function evaluateOperatorSurface(store, root) {
    ...
    const pass = docCoverage.total_docs > 0 &&
...
async function seedAgentRoutingFiles(root) {
    ...
    await Promise.all([...snippetsByPath.entries()].map(async ([target, snippets]) => {
        const path = join(root, target);
        await mkdir(dirname(path), { recursive: true });
        await writeFile(path, snippets.join("\n"), "utf8");
    }));
}
async function evaluateMultiAgentIntegration(root, now) {
    ...
    const hookReadyAgents = hookCapableAgents.filter((agent) => (agent.hook_coverage?.effective_events ?? 0) > 0 &&
        (agent.hook_coverage?.actionable_gaps ?? 1) === 0);
    const pass = status.summary.agents === SUPPORTED_ROUTING_AGENTS.length &&
...
function mappedRid(rids, rid) {
    return rids.get(rid) ?? rid;
}
async function probeAsOfRecall(uri, commit, fixture, now) {
    if (!commit) {
        return {
            status: "unavailable",
            refKind: "commit",
...
function foundationAxes(input) {
    ...
    const multiAgentIntegrationPass = input.multiAgentIntegration.score === input.multiAgentIntegration.maxScore;
    return [
        {
            id: "retrieval",
...
export async function evaluateCompetitiveEval(opts = {}) {
    ...
    const policyCases = fixture.candidates.map((candidate) => {
        const actual = classifyCandidateMemory(candidate.text);
        const pass = actual.kind === candidate.expectedKind &&
    ...
    const lintRecords = fixture.policyMemories.map((memory) => ({
        id: memory.id,
    ...
    return {
        ...
        policy: {
            ...
            classificationAccuracy: roundMetric(policyCases.filter((item) => item.pass).length / Math.max(1, policyCases.length)),
            cases: policyCases,
...
export function renderCompetitiveEvalJson(report) {
...
export async function evaluateCompetitiveEvalV2(opts = {}) {
    ...
    const unsupportedLiveCompetitorClaims = report.claimGuards.unsupportedLiveCompetitorClaims.filter((key) => !measuredLiveBaselineKeys.has(key));
    const unmeasuredLiveBaselines = report.claimGuards.unmeasuredLiveBaselines.filter((key) => !measuredLiveBaselineKeys.has(key));
    const claimGuardStatus = unsupportedPublicClaims.length === 0 && unsupportedLiveCompetitorClaims.length === 0
        ? "pass"
        : "fail";
    return {
...
function measuredLiveBaselineGuardKeys(liveBaselines) {
    const keys = new Set();
...
function competitiveEvalV2EvidenceIds(dimensions, baseline, fixture, liveBaselines) {
    const evidence = new Set();
    for (const dimension of dimensions) {
        if (dimension.status === "pass" && dimension.score === dimension.maxScore) {
            evidence.add(`dimension:${dimension.id}`);
        }
        for (const id of dimension.evidence)
            evidence.add(id);
    }
    for (const assertion of baseline.assertions) {
        if (assertion.pass)
            evidence.add(`baseline:${assertion.id}`);
    }
...
function competitiveEvalV2Dimensions(report, ctx) {
    ...
    return [
        ...
        },
        {
            ...
            metrics: {
                ...
                capability_catalog_categories: report.foundationGate.operatorSurface.capabilityCatalog.categories,
                capability_catalog_red_db_backed: report.foundationGate.operatorSurface.capabilityCatalog.redDbBacked,
            },
        },
        {
            id: "multi-agent-integration",
            ...
            detail: foundationAxes.get("multi-agent-integration")?.detail ??
...
function intelligenceDimension(ctx) {
    const confidenceScoring = {
        status: "pass",
        detail: "Pure composer + table tests; CLI/MCP/HTTP op `memory.confidence.v1` exposes per-signal breakdown.",
    };
...
async function runReasoningReplaySubCheck() {
    ...
    try {
        ...
        try {
            ...
            const hasOutcome = replay.results.some((result) => ["done", "blocked", "no-sentinel", "unknown"].includes(result.outcome));
 ...
 */
async function runFederationSubCheck() {
 ...
 */
async function runWhatifSubCheck() {
...
async function scaffoldFederationRoot(parent, name, notes) {
    const dir = join(parent, name);
    const notesDir = join(dir, ".red/memory/notes");
    await mkdir(notesDir, { recursive: true });
    await writeFile(join(dir, ".red/memory/config.json"), JSON.stringify({
        version: 1,
        mode: "markdown-only",
 ...
 */
function evaluateAutocureSubCheck() {
...
export function renderCompetitiveEvalV2Json(report) {
    return `${JSON.stringify({
...
export function renderCompetitiveEvalV2Human(report) {
    const lines = [
    ...
    if (report.liveBaselines.length === 0) {
        ...
        for (const baseline of report.liveBaselines) {
            const metrics = Object.entries(baseline.metrics)
                ...
                .join(" ");
            lines.push(`${liveBaselineLabel(baseline.competitor)} live baseline: ${baseline.state} - ${baseline.summary}${metrics ? ` (${metrics})` : ""}`);
        }
    }
    ...
    if (report.claimGuards.unsupportedLiveCompetitorClaims.length > 0) {
        lines.push(`Unsupported live competitor claims: ${report.claimGuards.unsupportedLiveCompetitorClaims.join(", ")}`);
    }
...
function liveBaselineLabel(competitor) {
    if (competitor === "agent-memory")
        return "Neo4j Agent Memory";
    return "Agentmemory";
}
function serializeLiveBaseline(baseline) {
    return {
...
export function renderCompetitiveInteropJson(report) {
    return `${JSON.stringify({
        schema_version: report.schemaVersion,
        generated_at: report.generatedAt,
        live_services: report.liveServices,
...
export function renderCompetitiveInteropHuman(report) {
    const lines = [
    ...
    for (const artifact of report.artifacts) {
        lines.push("", `## ${artifact.competitor}`, `Artifact: ${artifact.artifactName} (${artifact.source})`, `Counts: nodes=${artifact.counts.sourceNodes} edges=${artifact.counts.sourceEdges} preserved=${artifact.counts.preservedConcepts} approximated=${artifact.counts.approximatedConcepts} dropped=${artifact.counts.droppedConcepts}`, "", "Preserved");
...
export function renderCompetitiveEvalHuman(report) {
    const lines = [
...
export function renderBaselineJson(report) {
    return `${JSON.stringify({
        generated_at: report.generatedAt,
...
async function main() {
    ...
    if (flags.has("--interop")) {
        ...
        if (interopReport.claimGuards.fullParityClaimed ||
