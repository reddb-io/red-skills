export function buildMemoryExtractionStatusViewerArtifact(status) {
    return {
        contract: {
...
function renderExtractionStatusViewer(status) {
    const deterministic = Object.entries(status.deterministic)
        .map(([key, ready]) => `<li class="capability">
        <div>
        ...
        .join("");
    const actions = status.recommended_next_actions.length === 0
...
function readyDeterministic(status) {
