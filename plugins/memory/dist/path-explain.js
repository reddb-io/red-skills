import { pathConfidence, } from "./confidence-scoring.js";
import { buildConfidenceContext, confidenceForNode, } from "./engine.js";
export async function buildPathExplainReport(store, input) {
    ...
    const edgeRows = rawEdges.map(normalizeEdge).filter((edge) => edge != null);
    const result = from && to ? findPath(from.rid, to.rid, edgeRows, maxDepth) : null;
    const nodeByRid = new Map(nodes.map((node) => [node.rid, node]));
    const path = result?.path.map((rid) => nodeByRid.get(rid)).filter((node) => node != null) ?? [];
    const confidenceCtx = await buildConfidenceContext(store);
    const pathRefs = path.map((node) => nodeRef(node, confidenceCtx));
    const edges = result?.edges.map((edge) => toExplainedEdge(edge, nodeByRid, confidenceCtx)).filter((edge) => edge != null) ?? [];
    const report = {
...
function findPath(fromRid, toRid, edges, maxDepth) {
    if (fromRid === toRid)
        return { path: [fromRid], edges: [] };
    ...
    const queue = [{ rid: fromRid, path: [fromRid], edges: [] }];
    const visited = new Set([fromRid]);
    while (queue.length > 0) {
        const item = queue.shift();
        if (item.path.length - 1 >= maxDepth)
            continue;
        for (const edge of adjacency.get(item.rid) ?? []) {
            if (visited.has(edge.to_rid))
                continue;
            const nextPath = [...item.path, edge.to_rid];
            const nextEdges = [...item.edges, edge];
            if (edge.to_rid === toRid)
...
function resolveNode(nodes, query) {
    const normalized = query.trim().toLowerCase();
    return (nodes.find((node) => node.label === query) ??
...
function normalizeEdge(row) {
    ...
    if (!Number.isFinite(from) || !Number.isFinite(to) || typeof label !== "string")
        return null;
    return { from_rid: from, to_rid: to, label };
}
function toExplainedEdge(edge, nodeByRid, confidenceCtx) {
...
function nodeRef(node, confidenceCtx) {
...
function renderMarkdown(report) {
    ...
    if (report.reachable) {
        ...
        if (conf != null)
            lines.push(`Path confidence: ${conf.toFixed(3)}`);
        lines.push("", "## Path");
        for (const edge of report.edges) {
            lines.push(`- ${edge.from.title} (${edge.from.label}) --${edge.label}--> ${edge.to.title} (${edge.to.label})`);
        }
    }
    else {
...
function nextActions(from, to, reachable) {
    if (!from)
        return ["check the source label with `memory search <from>`"];
    if (!to)
        return ["check the target label with `memory search <to>`"];
    if (!reachable)
        return ["try a higher --max-depth or inspect neighbors with `memory traverse <label>`"];
