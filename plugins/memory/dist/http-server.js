...
const ENDPOINTS = [
    ...
    "GET /api/workbench",
    "GET /api/dashboard",
    "GET /api/competitive-eval",
    "GET /api/competitive-radar",
    "GET /competitive-eval",
    "GET /api/communities",
...
export function createMemoryHttpServer(opts) {
...
async function handleRequest(req, res, opts) {
    ...
    if (url.pathname === "/api/dashboard") {
        sendJson(res, 200, await buildMemoryOperationalDashboard(opts.store, opts.rootDir, { now: opts.now }));
        return;
    }
    ...
    if (url.pathname === "/api/context-pack") {
        ...
        sendJson(res, 200, await buildContextPack(opts.store, goal, {
            budgetChars: numberParam(url.searchParams.get("budget_chars")),
            limit: numberParam(url.searchParams.get("limit")),
            depth: numberParam(url.searchParams.get("depth")),
            now: opts.now,
    ...
    if (url.pathname === "/communities") {
        sendHtml(res, buildCommunitiesViewerArtifact(await buildCommunityAnalytics(opts.store, { cache: "read-only" })).html);
        return;
    }
    if (url.pathname === "/api/competitive-radar") {
        sendJson(res, 200, await buildMemoryCompetitiveRadar(opts.store, opts.rootDir, { now: opts.now }));
        return;
    }
    if (url.pathname === "/api/competitive-eval") {
        sendJson(res, 200, await evaluateCompetitiveEvalV2({ now: opts.now }));
        return;
    }
    if (url.pathname === "/competitive-eval") {
        sendHtml(res, buildCompetitiveEvalViewerArtifact(await evaluateCompetitiveEvalV2({ now: opts.now })).html);
        return;
    }
    if (url.pathname === "/api/extraction/status") {
        sendJson(res, 200, await buildMemoryExtractionStatus(opts.store, opts.rootDir, { now: opts.now }));
        return;
    }
    if (url.pathname === "/api/governance") {
        sendJson(res, 200, await buildMemoryGovernanceReport(opts.store, {
    ...
    if (url.pathname === "/governance") {
        sendHtml(res, buildMemoryGovernanceViewerArtifact(await buildMemoryGovernanceReport(opts.store, {
            staleProgressDays: numberParam(url.searchParams.get("stale_progress_days")),
            now: opts.now,
        })).html);
        return;
    ...
    if (url.pathname === "/api/handoff") {
        sendJson(res, 200, await buildMemoryHandoff(opts.store, {
    ...
    if (url.pathname === "/handoff") {
        sendHtml(res, buildMemoryHandoffViewerArtifact(await buildMemoryHandoff(opts.store, {
            focus: url.searchParams.get("focus") ?? url.searchParams.get("query") ?? undefined,
            limit: numberParam(url.searchParams.get("limit")),
            now: opts.now,
        })).html);
    ...
    if (url.pathname === "/api/frontier") {
        sendJson(res, 200, await buildWorkFrontier(opts.store, {
            focus: url.searchParams.get("focus") ?? url.searchParams.get("query") ?? undefined,
            limit: numberParam(url.searchParams.get("limit")),
            now: opts.now,
        }));
    ...
    if (url.pathname === "/frontier") {
        sendHtml(res, buildWorkFrontierViewerArtifact(await buildWorkFrontier(opts.store, {
            focus: url.searchParams.get("focus") ?? url.searchParams.get("query") ?? undefined,
            limit: numberParam(url.searchParams.get("limit")),
            now: opts.now,
        })).html);
    ...
    if (url.pathname === "/api/decay") {
        sendJson(res, 200, await buildMemoryDecayReport(opts.store, {
    ...
    if (url.pathname === "/decay") {
        sendHtml(res, buildMemoryDecayViewerArtifact(await buildMemoryDecayReport(opts.store, {
            stale_days: numberParam(url.searchParams.get("stale_days")),
            deprecate_days: numberParam(url.searchParams.get("deprecate_days")),
            limit: numberParam(url.searchParams.get("limit")),
            now: opts.now,
    ...
    if (url.pathname === "/api/routing-guide") {
        sendJson(res, 200, buildMemoryRoutingGuide({ agent: routingAgentParam(url.searchParams.get("agent")) }));
        return;
    }
    if (url.pathname === "/routing-guide") {
        sendHtml(res, buildMemoryRoutingGuideViewerArtifact(buildMemoryRoutingGuide({ agent: routingAgentParam(url.searchParams.get("agent")) })).html);
        return;
    }
    if (url.pathname === "/api/integration-status") {
        sendJson(res, 200, await buildMemoryAgentIntegrationStatus(opts.rootDir, {
    ...
    if (url.pathname === "/integration-status") {
        sendHtml(res, buildMemoryAgentIntegrationStatusViewerArtifact(await buildMemoryAgentIntegrationStatus(opts.rootDir, {
            agent: routingAgentParam(url.searchParams.get("agent")),
            now: opts.now,
        })).html);
        return;
    ...
    if (url.pathname === "/api/memory/health") {
        sendJson(res, 200, await buildMemoryHealthReport(opts.store, {
            stale_days: numberParam(url.searchParams.get("stale_days")),
        }));
        return;
    }
    if (url.pathname === "/memory/health") {
        sendHtml(res, buildMemoryHealthViewerArtifact(await buildMemoryHealthReport(opts.store, {
            stale_days: numberParam(url.searchParams.get("stale_days")),
        })).html);
        return;
    }
    if (url.pathname === "/layers") {
        sendHtml(res, buildMemoryLayersViewerArtifact(await buildMemoryLayersReport(opts.store, { now: opts.now })).html);
        return;
    }
    if (url.pathname === "/api/learning-debt") {
        sendJson(res, 200, await buildLearningDebtReport(opts.store, {
            staleDays: numberParam(url.searchParams.get("stale_days")),
    ...
    if (url.pathname === "/learning-debt") {
        sendHtml(res, buildLearningDebtViewerArtifact(await buildLearningDebtReport(opts.store, {
            staleDays: numberParam(url.searchParams.get("stale_days")),
            minRepeatedFailures: numberParam(url.searchParams.get("min_repeated_failures")),
            rollups: await safeSkillRollups(opts.store),
            skillTelemetryEnabled: true,
    ...
    if (url.pathname === "/api/onboarding-map") {
        sendJson(res, 200, await buildOnboardingMap(opts.store, {
            staleDays: numberParam(url.searchParams.get("stale_days")),
            rollups: await safeSkillRollups(opts.store),
        }));
        return;
    ...
    if (url.pathname === "/onboarding-map") {
        sendHtml(res, buildOnboardingMapViewerArtifact(await buildOnboardingMap(opts.store, {
            staleDays: numberParam(url.searchParams.get("stale_days")),
            rollups: await safeSkillRollups(opts.store),
        })).html);
        return;
    ...
    if (url.pathname === "/api/session/timeline") {
        sendJson(res, 200, await buildSessionTimeline(opts.store, {
    ...
    if (url.pathname === "/api/assets") {
        sendJson(res, 200, await buildMemoryAssetInventory(opts.store, {
            kind: url.searchParams.get("kind") ?? undefined,
            query: url.searchParams.get("query") ?? url.searchParams.get("q") ?? undefined,
        }));
        return;
    ...
    if (url.pathname === "/api/docs/brief") {
        ...
        sendJson(res, 200, await buildDocBrief(opts.store, {
            query,
            limit: numberParam(url.searchParams.get("limit")),
            max_bytes: numberParam(url.searchParams.get("max_bytes")),
        }));
    ...
    if (url.pathname === "/api/docs/bundle") {
        ...
        sendJson(res, 200, await buildDocBundle(opts.store, {
            query,
            limit: numberParam(url.searchParams.get("limit")),
            max_bytes: numberParam(url.searchParams.get("max_bytes")),
        }));
    ...
    if (url.pathname === "/api/docs/evidence-pack") {
        ...
        sendJson(res, 200, await buildDocEvidencePack(opts.store, {
            path,
            rid,
            max_bytes: numberParam(url.searchParams.get("max_bytes")),
        }));
    ...
    if (url.pathname === "/api/docs/read") {
        ...
        sendJson(res, 200, await readDoc(opts.store, {
            path,
            rid,
    ...
    if (url.pathname === "/api/path-explain") {
        ...
        sendJson(res, 200, await buildPathExplainReport(opts.store, {
            from,
            to,
    ...
    if (url.pathname === "/api/vector/search") {
        ...
        sendJson(res, 200, await buildVectorSearchReport(opts.store, query, {
            limit: numberParam(url.searchParams.get("limit")),
        }));
        return;
    }
    ...
    if (url.pathname === "/api/federate") {
        ...
        sendJson(res, 200, await buildFederationReport(opts.rootDir, query, {
            limit: numberParam(url.searchParams.get("limit")),
    ...
    if (url.pathname === "/api/reasoning-replay") {
        ...
        sendJson(res, 200, await buildReasoningReplay(opts.store, task, {
            limit: numberParam(url.searchParams.get("limit")),
            now: opts.now,
        }));
        return;
    ...
    if (url.pathname === "/api/whatif") {
        ...
        sendJson(res, 200, await buildWhatifReport(opts.store, changes, {
            limit: numberParam(url.searchParams.get("limit")),
            now: opts.now,
        }));
        return;
...
function openApiDocument(opts) {
    ...
    return {
        ...
        info: {
            ...
            description: "Optional loopback-only read-only API over the project-local RedDB Memory store.",
        },
        servers: [{ url: "/" }],
        security,
        paths: {
            ...
            "/api/competitive-radar": {
                get: { summary: "Memory competitive radar JSON", responses: { "200": jsonResponse("Competitive radar JSON") } },
            },
            "/api/competitive-eval": {
                get: { summary: "Memory competitive eval v2 JSON", responses: { "200": jsonResponse("Competitive eval JSON") } },
            },
            "/competitive-eval": {
                get: { summary: "Memory competitive eval HTML", responses: { "200": htmlResponse("Competitive eval viewer HTML") } },
            },
            "/api/communities": {
                get: {
...
async function collectWhatifChanges(req, url) {
    const out = [];
    for (const raw of url.searchParams.getAll("change")) {
        if (typeof raw === "string" && raw.trim())
            out.push(parseWhatifChange(raw));
    }
    if (req.method === "POST") {
        ...
        if (body.trim()) {
            ...
            const candidate = parsed.changes;
...
function docBacklinksInput(url) {
    const rid = positiveIntParam(url.searchParams.get("rid"));
    const label = url.searchParams.get("label") ?? undefined;
    const title = url.searchParams.get("title") ?? undefined;
    const query = url.searchParams.get("query") ?? url.searchParams.get("q") ?? undefined;
    if (rid == null && !label && !title && !query)
        return null;
    return { rid, label, title, query };
}
function routingAgentParam(value) {
    if (!value)
        return undefined;
    return SUPPORTED_ROUTING_AGENTS.includes(value)
        ? value
        : undefined;
}
...
function boundedIntParam(value, max) {
    if (!value)
        return undefined;
