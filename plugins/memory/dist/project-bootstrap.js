...
export async function bootstrapProjectMemory(store, opts) {
...
export async function discoverBootstrapSources(rootDir, opts = {}) {
    const patterns = [
        "README.md",
        "AGENTS.md",
        "CLAUDE.md",
...
async function bootstrapFile(store, root, candidate, opts) {
...
async function bootstrapGitLog(store, root, path, opts) {
...
async function readGitLog(root) {
    try {
        const { stdout } = await execFileAsync("git", ["log", "--oneline", "--decorate", "--max-count=30"], { cwd: root, encoding: "utf8" });
...
function sourceKind(root, path) {
    const rel = relative(root, path).replaceAll("\\", "/");
    if (/^readme\.md$/i.test(rel))
        return "readme";
    if (rel === "AGENTS.md" || rel === "CLAUDE.md")
        return "agent-rules";
    if (rel.startsWith(".red/adr/"))
        return "adr";
    if (rel === ".red/CONTEXT.md" || rel.startsWith(".red/contexts/"))
        return "context";
    return "docs";
}
function sourceRank(root, path) {
...
function recommendedActions(summary, opts) {
