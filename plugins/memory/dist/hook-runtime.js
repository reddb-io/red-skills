...
const DECISION_CUES = /\b(decided|decision|chose|choosing|we'?ll use|going with|switch(?:ed|ing)? to|approach is|opted to|settled on)\b/i;
const WHYNOTE_CUES = /\b(because|the reason|since|so that|to avoid|in order to|trade-?off|that'?s why|rationale)\b/i;
/**
 ...
 */
export const heuristicExtractor = (transcript) => {
    const out = [];
    const seen = new Set();
...
function extractedToNode(m, input) {
    return {
 ...
 */
export async function dispatch(input, rootDir, deps = defaultDeps()) {
    const config = await deps.readConfig(rootDir);
    if (!config)
        return { noop: true, reason: "memory not initialized" };
...
async function recordLifecycle(input, rootDir, config, deps, work) {
...
async function handleSessionStart(input, rootDir, config, deps) {
    const query = focusQuery(input);
    if (!query)
        return { noop: true, reason: "no focus signal to recall on" };
    const store = await deps.openStore(resolveStoreUri(rootDir, config));
    try {
        const result = await deps.recall(store, query, { k: 8, depth: 1 });
...
async function handlePostToolUse(input, rootDir, config, deps) {
    ...
    try {
        ...
        if (report.files === 0)
            return { noop: true, reason: "no indexable files changed" };
...
async function handleFlush(input, rootDir, config, deps) {
    const text = input.transcriptText?.trim();
    if (!text)
        return { noop: true, reason: "empty transcript" };
