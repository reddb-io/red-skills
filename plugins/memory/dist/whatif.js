...
export async function buildWhatifReport(store, changes, opts = {}) {
    ...
    for (const change of normalized) {
        if (!change.file && !change.symbol)
            continue;
        let impact;
        try {
            impact = await readStructuralImpact(store, {
                file: change.file,
 ...
 */
export function parseWhatifChange(raw) {
    const trimmed = raw.trim();
    if (!trimmed) {
        throw new Error("whatif change cannot be empty");
    }
    const lower = trimmed.toLowerCase();
    const kind = lower.startsWith("rename")
        ? "rename"
...
function synthesiseDescription(change) {
...
function collectImpact(impact, change) {
    ...
    if (change.kind !== "edit") {
        for (const edge of impact.imports)
            addFileFromEdge(fileSet, edge, "to");
    }
    const fanOut = impact.importedBy.length +
        impact.calledBy.length +
        impact.referencedBy.length +
        impact.usedByTypes.length;
    if (change.file)
        fileSet.delete(normalizePath(change.file));
    if (change.symbol)
        symbolSet.delete(change.symbol);
    return { fileSet, symbolSet, fanOut };
}
function addFile(set, node) {
    if (!node)
        return;
...
function addFileFromEdge(set, edge, side) {
    ...
    if (node.node_type === "import") {
        const resolved = typeof node.properties.resolved_path === "string" ? node.properties.resolved_path : "";
        const title = typeof node.properties.title === "string" ? node.properties.title : "";
        if (resolved)
            set.add(normalizePath(resolved));
        else if (title)
            set.add(normalizePath(title));
        return;
    }
    addFile(set, node);
}
function addFileFromCallSite(set, edge, side) {
    ...
    const path = typeof node.properties.file === "string"
...
function addSymbolFromEdge(set, edge, side) {
    const node = side === "from" ? edge.from : edge.to;
    if (node.node_type !== "symbol")
        return;
    addSymbol(set, node);
}
function pickFilePath(node) {
    if (node.node_type === "file") {
...
function pickSymbolName(node) {
    const title = node.properties.title;
    if (typeof title === "string" && title)
        return title;
    const hash = node.label.indexOf("#");
    if (hash >= 0 && hash < node.label.length - 1)
        return node.label.slice(hash + 1);
    return node.label;
}
...
async function collectHistorical(store, changes, opts) {
    if (changes.length === 0)
        return [];
...
function scoreHistorical(results) {
    if (results.length === 0)
        return 0;
    let weight = 0;
    let blockedWeight = 0;
