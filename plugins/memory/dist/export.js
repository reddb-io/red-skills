...
import { redactGraphData } from "./privacy.js";
...
export async function exportGraph(store, outDir, opts = {}) {
    ...
    const [communities, superseded, doctor] = await Promise.all([
        opts.communities ? store.communities() : Promise.resolve(new Map()),
    ...
    const withCommunity = (rid) => opts.communities ? { community: communities.get(rid) ?? null } : {};
    const json = {
    ...
    if (interop) {
        writes.push(writeFile(interop.nodesJsonlPath, renderNodesJsonl(nodes, dashboard), "utf8"), writeFile(interop.edgesJsonlPath, renderEdgesJsonl(edges), "utf8"), writeFile(interop.graphmlPath, renderGraphml(nodes, edges, dashboard), "utf8"), writeFile(interop.cypherPath, renderNeo4jCypher(nodes, edges, dashboard), "utf8"));
    }
...
function exportVectorStatus(vector) {
    return {
...
// Interop artifacts
// ---------------------------------------------------------------------------
function renderNodesJsonl(nodes, dashboard) {
    return `${nodes
...
function renderEdgesJsonl(edges) {
    return `${edges
...
function renderNeo4jCypher(nodes, edges, dashboard) {
    const lines = [
    ...
    for (const edge of edges) {
        ...
        lines.push(`MATCH (a:MemoryNode {rid: ${edge.from}}), (b:MemoryNode {rid: ${edge.to}}) MERGE (a)-[r:${relType} {rid: ${edge.rid}}]->(b) SET r += ${cypherMap(props)};`);
    }
    return `${lines.join("\n")}\n`;
}
function renderGraphml(nodes, edges, dashboard) {
    const lines = [
    ...
    for (const edge of edges) {
        lines.push(`    <edge id="memory_edges:${edge.rid}" source="memory_nodes:${edge.from}" target="memory_nodes:${edge.to}">`);
...
function cypherRelationship(value) {
...
function buildDashboard(nodes, edges, stats, docs, superseded, stale) {
    const byRid = new Map(nodes.map((n) => [n.rid, n]));
    const staleByRid = new Map(stale.map((n) => [n.rid, n]));
    const degree = new Map(nodes.map((n) => [n.rid, 0]));
    for (const edge of edges) {
    ...
    for (const node of nodes) {
        const statuses = [];
        if (superseded.has(node.rid))
            statuses.push("superseded");
        else
            statuses.push("active");
        if (staleByRid.has(node.rid))
            statuses.push("stale");
        if ((node.properties.confidence ?? "AMBIGUOUS") === "AMBIGUOUS")
...
function renderContextPack(nodes) {
    const lines = [
    ...
    if (nodes.length === 0) {
        ...
        for (const node of nodes) {
            ...
            lines.push(`- [memory_nodes:${node.rid}] ${node.properties.title ?? node.label} (${node.node_type}, ${confidence})${content ? ` — ${content}` : ""}`);
        }
    }
    return `${lines.join("\n")}\n`;
}
function activeHead(rid, superseded, nodes) {
    const seen = new Set([rid]);
    let current = rid;
    while (true) {
        const next = superseded.get(current);
        if (next == null || seen.has(next) || !nodes.has(next))
            return current;
        seen.add(next);
        current = next;
    }
}
function titleOf(node, fallbackRid) {
...
function renderAudit(nodes, edges, stats, docs, vector, dashboard) {
    ...
    if (vector.stale > 0)
        lines.push(`- **Stale:** ${vector.stale}`);
    if (vector.unavailable > 0)
        lines.push(`- **Unavailable:** ${vector.unavailable}`);
    if (vector.failed > 0)
        lines.push(`- **Failed:** ${vector.failed}`);
    lines.push("");
    ...
    if (docs.length === 0) {
        ...
        if (docs.length > 50)
            lines.push(`- … and ${docs.length - 50} more`);
    }
    lines.push("");
    ...
    if (dashboard.contradictions.length === 0) {
        ...
        for (const item of dashboard.contradictions) {
            ...
            lines.push(`- memory_nodes:${item.from_rid} **${item.from_title}** contradicts memory_nodes:${item.to_rid} **${item.to_title}** (${state})${reason}`);
        }
    }
    lines.push("");
    ...
    if (dashboard.supersession.length === 0) {
        ...
        for (const item of dashboard.supersession) {
            const reason = item.reason ? ` — ${item.reason}` : "";
            lines.push(`- memory_nodes:${item.from_rid} **${item.from_title}** → memory_nodes:${item.to_rid} **${item.to_title}**${reason}`);
        }
    }
    lines.push("");
...
function renderHtml(nodes, edges, stats, docs, vector, communities = new Map(), dashboard) {
    ...
    const statusRows = [
        ...
        .join("");
    const contradictionRows = dashboard.contradictions.length === 0
        ...
        : dashboard.contradictions
            ...
            .join("");
    const supersessionRows = dashboard.supersession.length === 0
        ...
        : dashboard.supersession
            ...
            .join("");
    const staleRows = dashboard.evidence.stale.length === 0
