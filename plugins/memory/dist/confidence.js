 ...
 */
import { scoreConfidence, } from "./confidence-scoring.js";
import { buildConfidenceContext, confidenceSignalsFor, } from "./engine.js";
export async function buildConfidenceReport(store, nodeId, now = Date.now()) {
    ...
    if (!node)
        return null;
    const ctx = await buildConfidenceContext(store, now);
    ...
    return {
        ...
        node: {
            ...
            title: typeof node.properties.title === "string" && node.properties.title.length > 0
...
export function renderConfidenceMarkdown(report) {
    const lines = [
