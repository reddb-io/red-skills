export async function buildMemoryAssetInventory(store, opts = {}) {
...
function assetScore(item, query) {
    const terms = query
        .toLowerCase()
        .split(/[^a-z0-9_.:/-]+/)
        .filter(Boolean);
...
function assetItem(node) {
    const props = node.properties;
    if (typeof props.asset_kind !== "string")
        return null;
