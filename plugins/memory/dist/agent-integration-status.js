...
export async function buildMemoryAgentIntegrationStatus(rootDir, opts = {}) {
    ...
    const items = await Promise.all(agents.map((agent) => agentStatus(root, agent, hookCoverage)));
    const summary = {
        agents: items.length,
...
async function agentStatus(root, agent, hookCoverage) {
    const guide = buildMemoryRoutingGuide({ agent });
    const targetFiles = await Promise.all(guide.targetFiles.map((target) => ruleFileStatus(root, target)));
    const runnerCoverage = agent === "codex" || agent === "claude"
    ...
    const hooksReady = hookStatus == null ||
        (hookStatus.effective_events > 0 && hookStatus.actionable_gaps === 0);
    const state = hasRoutedFile && hooksReady
        ? "ready"
...
async function ruleFileStatus(root, target) {
    ...
    return {
        path: target,
        exists,
        contains_memory_routing: body.includes("## Memory Routing") ||
...
function itemActions(agent, files, hookStatus) {
    const actions = [];
    ...
    if (actions.length === 0)
        actions.push("agent integration is ready");
    return actions;
}
function nextActions(items) {
    ...
    if (actions.length === 0)
        actions.push("all selected agent integrations are ready");
