...
function viewedFrom(payload, filePath) {
    const tool = str(payload, "tool_name") ?? str(payload, "toolName");
    if (tool !== "Read")
        return undefined;
    if (!filePath || !isSkillManifest(filePath))
        return undefined;
...
function editFrom(payload, filePath) {
    const tool = str(payload, "tool_name") ?? str(payload, "toolName");
    if (!tool || !filePath || !isSkillManifest(filePath))
        return undefined;
    if (tool === "Write")
        return { eventType: "changed", name: skillNameFromPath(filePath), path: filePath };
 ...
 * `[]`. Never throws.
 */
export function claudeSkillEvents(payload, context = {}) {
    try {
