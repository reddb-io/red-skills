...
import { buildSkillRecommendationsFromEvidence, renderSkillRecommendationsSection, } from "./skill-recommendations.js";
...
export async function buildContextPack(store, goal, opts = {}) {
    ...
    if (activeNodes.length === 0) {
        const skillRecommendations = buildSkillRecommendationsFromEvidence(goal, [], opts.skillRollups ?? []);
        const baseMarkdown = [
        ...
        const skillMarkdown = skillRecommendations.recommendations.length > 0
            ? `\n${renderSkillRecommendationsSection(skillRecommendations)}`
            : "";
        const markdown = fitToBudget(`${baseMarkdown}${baseMarkdown.length + skillMarkdown.length <= budgetChars ? skillMarkdown : ""}`, budgetChars);
        return {
            goal,
            status: "insufficient-context",
            budgetChars,
    ...
    const skillRecommendations = buildSkillRecommendationsFromEvidence(goal, activeNodes, opts.skillRollups ?? []);
...
async function buildWarnings(store, nodes, superseded) {
...
function toEntry(node, marker, goal) {
...
function inclusionReason(node, section, goal) {
    const reasonBySection = {
...
function renderPack(goal, entries, warnings, budgetChars, skillRecommendations) {
    const included = [];
...
function header(goal, status, warnings) {
...
function fitToBudget(markdown, budgetChars) {
