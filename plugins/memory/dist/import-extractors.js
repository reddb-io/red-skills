 ...
 */
export const typescriptJavascriptImportExtractor = (_parseTree, sourceText) => {
    assertImportClausesParse(sourceText);
    const imports = [];
    const re = /\b(?:import\s+(?:type\s+)?(?:[\s\S]*?\s+from\s+)?|export\s+(?:type\s+)?[\s\S]*?\s+from\s+)["']([^"']+)["']/g;
 ...
 */
export const rustImportExtractor = (_parseTree, sourceText) => {
    const imports = [];
    ...
    const externCrateRe = /\b(?:pub(?:\([^)]*\))?\s+)?extern\s+crate\s+([A-Za-z_]\w*)(?:\s+as\s+[A-Za-z_]\w*)?\s*;/g;
 ...
 */
export const goImportExtractor = (_parseTree, sourceText) => {
    assertGoImportClausesParse(sourceText);
    const imports = [];
    const declarationRe = /\bimport\s*(?:\(([\s\S]*?)\)|(?:(?:[A-Za-z_]\w*|[._])\s+)?["`]([^"`]+)["`])/g;
 ...
 */
export const pythonImportExtractor = (_parseTree, sourceText) => {
    const imports = [];
...
const EXTRACTORS_BY_EXT = new Map();
...
export function extractImportsForFile(sourcePath, parseTree, sourceText) {
    const ext = sourcePath.slice(sourcePath.lastIndexOf(".")).toLowerCase();
    const extractor = EXTRACTORS_BY_EXT.get(ext);
    if (!extractor)
        return [];
    try {
        return extractor(parseTree, sourceText).map((imp) => imp.kind === "relative"
            ? {
                ...imp,
                resolvedPath: ext === ".rs"
...
function isRustRelative(specifier) {
    return /^(?:crate|self|super)(?:::|$)/.test(specifier);
}
function assertImportClausesParse(sourceText) {
    const declarations = [
    ...
    for (const declaration of declarations) {
        ...
        for (const char of text) {
            ...
            if (char === "}")
                braceDepth -= 1;
            if (braceDepth < 0)
                throw new Error("malformed import/export clause");
        }
        if (braceDepth !== 0)
            throw new Error("malformed import/export clause");
    }
}
function assertGoImportClausesParse(sourceText) {
...
function assertGoImportGroupParse(group) {
...
function goImportSpecifiers(group) {
    return [...group.matchAll(/(?:^|[\s;])(?:(?:[A-Za-z_]\w*|[._])\s+)?["`]([^"`]+)["`]/g)]
        .map((match) => match[1])
        .filter((specifier) => Boolean(specifier));
}
function rustUseDeclarations(sourceText) {
    return [...sourceText.matchAll(/\b(?:pub(?:\([^)]*\))?\s+)?use\s+[\s\S]*?(?:;|$)/g)].map((m) => m[0] ?? "");
}
function assertBalancedBraces(text) {
    ...
    for (const char of text) {
        ...
        if (char === "}")
            braceDepth -= 1;
        if (braceDepth < 0)
            throw new Error("malformed Rust use clause");
    }
    if (braceDepth !== 0)
        throw new Error("malformed Rust use clause");
}
function assertBalancedParens(text, message) {
    let parenDepth = 0;
    for (const char of text) {
        if (char === "(")
            parenDepth += 1;
...
function pythonImportDeclarations(sourceText) {
    const declarations = [];
...
function expandRustUseTree(useTree, prefix = []) {
    ...
    for (const item of splitTopLevel(useTree, ",")) {
        ...
        if (segments.length > 0)
            imports.push([...prefix, ...segments].join("::"));
    }
    return imports;
}
...
function stripTopLevelAlias(path) {
    return path.replace(/\s+as\s+[A-Za-z_]\w*\s*$/, "").trim();
}
function rustPathSegments(path) {
    return path
        .split("::")
        .map((segment) => segment.trim())
        .filter(Boolean);
}
function resolveRustRelativeImport(sourcePath, specifier) {
    const segments = rustPathSegments(specifier);
    if (segments.length === 0)
        return dirname(sourcePath);
    const [head, ...tail] = segments;
    if (head === "crate")
        return join(rustCrateModuleRoot(sourcePath), ...tail);
    if (head === "self")
        return join(dirname(sourcePath), ...tail);
    if (head === "super") {
...
function resolvePythonRelativeImport(sourcePath, specifier) {
    const match = /^(\.+)(.*)$/.exec(specifier);
    if (!match)
        return join(dirname(sourcePath), ...specifier.split(".").filter(Boolean));
...
function rustCrateModuleRoot(sourcePath) {
    const marker = `${sep}src${sep}`;
    const index = sourcePath.lastIndexOf(marker);
    if (index >= 0)
        return sourcePath.slice(0, index + marker.length - 1);
    return dirname(sourcePath);
}
