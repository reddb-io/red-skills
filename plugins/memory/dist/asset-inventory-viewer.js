export function buildMemoryAssetInventoryViewerArtifact(report) {
    return {
        contract: {
...
function renderAssetInventory(report) {
    const kinds = report.kinds
        .map((kind) => `<li><strong>${escapeHtml(kind.kind)}</strong><p class="meta">${kind.count} asset(s), ${formatBytes(kind.bytes)}</p></li>`)
        .join("");
    const assets = report.assets
        .map((asset) => `<li class="asset">
