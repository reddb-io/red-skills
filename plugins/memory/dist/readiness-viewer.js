export function buildReadinessViewerArtifact(envelope) {
    return {
        contract: {
...
function renderReadinessViewer(envelope) {
...
function missingSection(envelope) {
...
function warningSection(title, warnings, severityClass) {
    return `<section>
    <h2>${escapeHtml(title)}</h2>
    ${warnings.length === 0
        ? `<p class="empty">No ${escapeHtml(title.toLowerCase())} available.</p>`
...
function contractSection(envelope) {
    return `<section>
