...
export async function buildSessionTimeline(store, opts = {}) {
...
function engineOutcome(outcome) {
    if (outcome === "failed")
        return "failed";
    if (outcome === "miss")
        return "noop";
...
function timelineActions(entries, sessionId) {
    if (entries.length === 0) {
        return [
            sessionId
    ...
    if (actions.length === 0)
        actions.push("Memory session timeline is ready");
    return actions;
}
