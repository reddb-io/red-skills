...
function lastAccess(node, overlayAt) {
    if (overlayAt != null && overlayAt > 0)
        return overlayAt;
 ...
 */
export async function diagnose(store, opts = {}) {
    ...
    for (const node of nodes) {
        ...
        if (importance >= PINNED_IMPORTANCE_THRESHOLD)
            continue;
 ...
 */
export async function prune(store, stale) {
    let pruned = 0;
