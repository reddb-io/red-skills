...
function assertSha(value, context) {
    if (!SHA_RE.test(value)) {
        throw new Error(`audit marker: ${context} — expected a 7–40 char lowercase hex SHA, got ${JSON.stringify(value)}`);
    }
    return value;
}
function assertIso8601(value) {
    if (!ISO8601_RE.test(value) || Number.isNaN(Date.parse(value))) {
        throw new Error(`audit marker: malformed timestamp — expected an ISO-8601 instant, got ${JSON.stringify(value)}`);
    }
    return value;
}
/**
 ...
 */
export function parseAuditMarker(input) {
    ...
    if (fields.length === 4 && /^\d{4}-/.test(fields[0])) {
        throw new Error(`audit marker: audit-log entry must use the "${AUDIT_LOG_ACTION}" action, got ${JSON.stringify(fields[1])}`);
    }
    throw new Error(`audit marker: unrecognised — expected "${INGEST_TRAILER_KEY}: <sha>", ` +
...
export function formatIngestTrailer(sha) {
...
export function formatNoIngestTrailer(reason) {
...
export function formatAuditLogEntry(at, path, sha) {
    return [
        assertIso8601(at),
        AUDIT_LOG_ACTION,
        path,
 ...
 */
export function ingestGuidance(headSha) {
    const sha = headSha && SHA_RE.test(headSha) ? headSha : "<ingest-sha>";
    const trailer = sha === "<ingest-sha>" ? `${INGEST_TRAILER_KEY}: <ingest-sha>` : formatIngestTrailer(sha);
    return [
