 ...
 */
export function redDbProviderClient(store, config) {
 ...
 */
export function applyProviderEnv(resolved, apiKeyEnv) {
    if (resolved.endpoint && !process.env.RED_AI_BASE_URL) {
        process.env.RED_AI_BASE_URL = resolved.endpoint;
    }
    if (!process.env.RED_AI_MODEL)
        process.env.RED_AI_MODEL = resolved.model;
    if (!process.env.RED_AI_PROVIDER)
        process.env.RED_AI_PROVIDER = resolved.mode;
