...
export const SKILL_EVENT_TYPES = ["viewed", "used", "result", "changed", "patched"];
...
export function parseSkillEventInput(input) {
    const trimmed = input.trim();
    if (!trimmed)
        throw new Error("skill event input is empty");
    const raw = parseJsonOrJsonl(trimmed);
    const events = Array.isArray(raw) ? raw : [raw];
    if (events.length === 0)
        throw new Error("skill event batch is empty");
    return events.map((event, index) => parseSkillEvent(event, index));
}
export function parseSkillEvent(input, index = 0) {
    ...
    const detail = parsed.error.issues
        ...
        const extras = "keys" in issue && Array.isArray(issue.keys) ? ` (${issue.keys.join(", ")})` : "";
        return `${path}: ${issue.message}${extras}`;
    })
        .join("; ");
    throw new Error(`invalid skill event: ${detail}`);
}
export async function ingestSkillEvents(store, events) {
    ...
    for (const event of events) {
        ...
        for (const rid of graph.nodes)
            touchedNodes.add(rid);
        for (const rid of graph.edges)
            touchedEdges.add(rid);
...
export async function readSkillRollups(store) {
 ...
 */
export async function readRecentSkillEvents(store, limit = 10) {
    const nodes = await store.listNodes();
    const events = [];
    for (const node of nodes) {
        if (!node.label.startsWith("skill-event:"))
            continue;
        const raw = node.properties?.skill_event;
...
export function skillEventToNode(event) {
    ...
    return {
        ...
        properties: {
            ...
            content: `${event.timestamp} ${event.runner} ${event.event_type} ` +
...
async function upsertSkillEventGraph(store, event) {
...
async function readSeenEventIds(store) {
    return parseKvObject(await store.kvGet(SKILL_EVENT_SEEN_KEY));
}
async function hasSeenEventId(store, eventId) {
    const partitioned = await store.kvGet(skillEventSeenKey(eventId));
    if (partitioned === true)
        return true;
    const legacy = await readSeenEventIds(store);
    return legacy[eventId] === true;
}
async function writeSeenEventId(store, eventId) {
    await store.kvPut(skillEventSeenKey(eventId), true);
}
async function readSkillRollup(store, event) {
    const key = skillRollupKey(event);
    const partitioned = await store.kvGet(skillRollupStorageKey(event));
    if (partitioned != null)
        return { [key]: parseKvValue(partitioned) };
    const legacy = await readSkillRollupMap(store);
    return legacy[key] != null ? { [key]: legacy[key] } : {};
}
async function writeSkillRollup(store, event, rollup) {
    await store.kvPut(skillRollupStorageKey(event), rollup);
}
async function readSkillRollupMap(store) {
    const rollups = parseKvObject(await store.kvGet(SKILL_ROLLUPS_KEY));
    const nodes = await store.listNodes();
    for (const node of nodes) {
        const skill = node.properties?.skill;
        if (!isStoredSkill(skill))
            continue;
        const stored = await store.kvGet(skillRollupStorageKey(skill));
        if (stored == null)
            continue;
        rollups[skillRollupKey(skill)] = parseKvValue(stored);
    }
    return rollups;
}
function parseKvObject(raw) {
    if (raw == null)
        return {};
    return parseKvValue(raw);
}
function parseKvValue(raw) {
    return (typeof raw === "string" ? JSON.parse(raw) : raw);
}
function updateSkillRollup(rollups, event) {
    const key = skillRollupKey(event);
    const current = rollups[key] ??
        {
            name: event.name,
            source_kind: event.source_kind,
            path: event.path,
