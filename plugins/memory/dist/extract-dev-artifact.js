...
export async function extractDevArtifact(path) {
    ...
    const importNodes = imports.map((item) => ({
...
export function isDevArtifact(path) {
...
function extractWorkflows(path, source) {
    const name = basename(path).toLowerCase();
    if (name === "package.json")
        return packageScripts(source);
    if (name === "dockerfile" || name.endsWith(".dockerfile"))
        return dockerSteps(source);
    if (path.includes("/.github/workflows/"))
        return githubWorkflowJobs(source);
    if ([".sh", ".bash"].includes(extname(path).toLowerCase()))
        return shellFunctions(source);
    return [];
}
function extractImports(path, source) {
    const name = basename(path).toLowerCase();
    if (name === "dockerfile" || name.endsWith(".dockerfile")) {
...
function githubWorkflowJobs(source) {
...
function lineForText(source, needle) {
