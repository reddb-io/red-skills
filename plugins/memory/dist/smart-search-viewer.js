export function buildMemorySmartSearchViewerArtifact(report) {
    return {
        contract: {
...
function renderSmartSearchViewer(report) {
    ...
    const docs = report.docs.hits.length === 0
        ...
        : `<ul>${report.docs.hits
            ...
            .join("")}</ul>`;
    const assets = report.assets.assets.length === 0
        ...
        : `<ul>${report.assets.assets
            ...
            .join("")}</ul>`;
    const vectors = report.vector.hits.length === 0
