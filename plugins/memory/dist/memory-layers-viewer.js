export function buildMemoryLayersViewerArtifact(report) {
    return {
        contract: {
...
function renderMemoryLayersViewer(report) {
    return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
    ...
    <div class="layout">
      ...
      <div class="stack">
        <section>
          ...
          ${report.competitor_alignment.length === 0 ? `<p class="empty">No competitor alignment.</p>` : `<ul>${report.competitor_alignment.map(alignmentItem).join("")}</ul>`}
        </section>
        <section>
...
function alignmentItem(item) {
    return `<li>
    <div>
      <h3>${escapeHtml(item.competitor)}</h3>
