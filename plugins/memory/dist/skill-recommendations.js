import { recall as graphRecall, } from "./engine.js";
import { tokenize } from "./recall.js";
export async function buildSkillRecommendations(store, task, opts = {}) {
    ...
    return buildSkillRecommendationsFromEvidence(task, recalled.nodes, opts.skillRollups ?? [], opts.limit ?? 5);
}
export function renderSkillRecommendationsSection(report) {
    ...
    for (const recommendation of report.recommendations) {
        ...
        lines.push(`- ${recommendation.name} (${recommendation.confidence} confidence, ${recommendation.evidenceStrength} evidence${source}; score ${recommendation.score.toFixed(4)})`);
        if (recommendation.path)
            lines.push(`  Path: ${recommendation.path}`);
        lines.push(`  Why: ${recommendation.reasons.join("; ")}`);
        lines.push(`  Citations: ${recommendation.citations.map((citation) => citation.marker).join(" ") || "none"}`);
    }
    const citationLines = report.recommendations.flatMap((recommendation) => recommendation.citations);
    if (citationLines.length > 0) {
        lines.push("");
        lines.push("Skill recommendation citations");
        for (const citation of citationLines) {
            lines.push(`- ${citation.marker} ${citation.urn} (${citation.kind}) ${citation.title}: ${citation.detail}`);
        }
    }
    return `${lines.join("\n")}\n`;
}
export function buildSkillRecommendationsFromEvidence(task, memoryEvidence, telemetry = [], limit = 5) {
    ...
    for (const node of memoryEvidence) {
        ...
        for (const name of skillNames) {
            ...
            candidate.reasons.add(`Memory evidence [M${memoryMarker}] names this skill for a similar task`);
    ...
    for (const rollup of telemetry) {
        ...
        if (matchScore > 0)
            candidate.reasons.add("task text matched the skill name or path");
        candidate.reasons.add(`Skill telemetry shows ${rollup.event_count} event(s), ${rollup.use_count} use(s), and ${rollup.result_count} result(s)`);
        candidate.citations.push({
    ...
    if (memoryEvidence.length === 0)
        missingEvidence.push("no Memory evidence matched this task");
    if (telemetry.length === 0)
        missingEvidence.push("no Skill telemetry rollups are available");
...
function candidateFor(candidates, name, sourceKind, path) {
...
function toRecommendation(candidate) {
    ...
    return {
        ...
        confidence: evidenceStrength === "strong" ? "high" : evidenceStrength === "moderate" ? "medium" : "low",
        evidenceStrength,
...
function strength(candidate) {
    if (candidate.hasMemory && candidate.hasTelemetry)
        return "strong";
    if (candidate.hasMemory || candidate.hasTelemetry)
        return "moderate";
    return "missing";
}
function taskMatchScore(task, fields) {
...
function telemetryQualityScore(rollup) {
...
function telemetryDetail(rollup) {
    const succeeded = rollup.outcome_counts.succeeded ?? 0;
    const failed = rollup.outcome_counts.failed ?? 0;
...
function skillNamesFromNode(node) {
    const names = new Set();
