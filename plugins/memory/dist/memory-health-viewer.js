export function buildMemoryHealthViewerArtifact(report) {
    return {
        contract: {
...
function renderMemoryHealthViewer(report) {
    return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
      ...
      <div class="stack">
        <section>
          ...
          <ul>
            ...
            ${signalItem("Engine events", `${report.engine_events.status}, ${report.engine_events.total} event(s) — recall hit rate ${(report.engine_events.recall_hit_rate * 100).toFixed(0)}%, ${report.engine_events.conflict_count} conflict(s), ${report.engine_events.promotion_count} promotion(s), ${report.engine_events.eviction_count} eviction(s)`, report.engine_events.status === "available" ? "ready" : "degraded")}
          </ul>
        </section>
      </div>
      <div class="stack">
...
function signalItem(title, detail, status) {
    return `<li>
    <div>
...
function stateClass(status) {
    if (status === "ready" || status === "available")
        return "";
    if (status === "attention" || status === "stale" || status === "unavailable")
        return "attention";
    return "degraded";
}
