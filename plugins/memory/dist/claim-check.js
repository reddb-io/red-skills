...
export async function claimCheck(store, assertion) {
    ...
    const evidence = recalled.nodes.map((node, index) => toClaimEvidence(node, index + 1, superseded));
    ...
    const conflicts = collectConflicts(await store.listEdges(), evidenceByRid, nodesByRid, superseded);
    ...
    const activeWithConflicts = mergeEvidence(active, conflicts.flatMap((item) => [item.from, item.to]));
...
function toClaimEvidence(node, marker, superseded) {
...
function supportsAssertion(item, assertion) {
    const terms = meaningfulTokens(assertion);
    if (terms.length === 0)
        return false;
    const evidenceTokens = new Set(meaningfulTokens([item.label, item.title, item.excerpt].join(" ")));
...
function collectConflicts(edges, evidenceByRid, nodesByRid, superseded) {
    const conflicts = [];
    ...
    for (const edge of edges) {
        ...
        if (from.status !== "active" || to.status !== "active")
            continue;
        if (from.activeRid === to.activeRid)
            continue;
...
function sessionFromEdge(edge, side) {
    const props = edge.properties ?? edge.PROPERTIES;
...
function evidenceForStored(node, marker, superseded) {
    if (!node)
        return null;
    const activeRid = activeHead(node.rid, superseded);
    return {
        ...
        node_type: node.node_type,
        title: node.properties.title ?? node.label,
        excerpt: (node.properties.summary ??
