export async function findNodeForProvenance(store, target) {
    const asRid = Number(target);
    if (Number.isInteger(asRid) && asRid > 0)
        return store.getNode(asRid);
    const rid = await store.findNodeByLabel(target);
    return rid == null ? null : store.getNode(rid);
}
export function buildProvenanceReport(node) {
...
export function formatProvenanceHuman(report) {
