...
export async function listContradictions(store, opts = {}) {
    ...
    for (const edge of await store.listEdges()) {
        ...
        if (resolved && !opts.includeResolved)
            continue;
        out.push({
...
export async function resolveConflict(store, input) {
    const nodes = new Map((await store.listNodes()).map((node) => [node.rid, node]));
    if (!nodes.has(input.activeRid))
        throw new Error(`active node not found: ${input.activeRid}`);
...
export async function supersessionTimeline(store, topic) {
    ...
    const timelineRids = new Set();
    for (const seed of seeds)
        addChain(seed, superseded, reverse, nodes, timelineRids);
    const entries = [...timelineRids]
        ...
        return {
            ...summarizeNode(node),
            status: head === node.rid ? "active" : "superseded",
            activeRid: head,
...
function matchingNodeRids(nodes, topic) {
...
function addChain(seed, superseded, reverse, nodes, out) {
    const visit = (rid) => {
        if (out.has(rid) || !nodes.has(rid))
            return;
        out.add(rid);
        const next = superseded.get(rid);
        if (next != null)
            visit(next);
        for (const prev of reverse.get(rid) ?? [])
            visit(prev);
    };
    visit(seed);
}
function reverseSupersededMap(map) {
