...
import { CURATE_CATEGORIES } from "./types.js";
...
export function totalCandidates(byCategory) {
    let n = 0;
    for (const c of CURATE_CATEGORIES)
        n += byCategory[c]?.length ?? 0;
    return n;
}
/** Pure: title for the filed issue. */
export function renderIssueTitle(byCategory) {
...
export function renderIssueBody(report) {
    ...
    const lines = [];
    lines.push("Filed by `/curate --background`. Each entry is a **Curatable skill** surfaced by Skill telemetry; no Skill file has been mutated.");
    lines.push("");
    lines.push(`Totals: ${totals.curatableSkills} curatable / ${totals.totalSkills} total skills (${totals.readOnlySkills} read-only).`);
    ...
    lines.push("");
    lines.push("Approve by running interactive `/curate` (or `/afk` against this issue) and naming the skills to archive. The archive path is the same atomic-rename + hash-manifest flow used by the tracer slice; recover any archive with `/curate --restore <name>`.");
    return lines.join("\n");
}
/**
 ...
 */
export async function fileBackgroundIssue(report, opts) {
    ...
    try {
        ...
        const proc = spawn("gh", ["issue", "create", "--title", title, "--body-file", bodyPath, "--label", label], { cwd: opts.cwd, encoding: "utf8" });
        if (proc.status !== 0) {
            const stderr = (proc.stderr ?? "").toString().trim();
            throw new Error(`curate background: gh issue create exited ${proc.status ?? "?"}${stderr ? `: ${stderr}` : ""}`);
        }
