...
function archiveDirOf(flags) {
    return typeof flags["archive-dir"] === "string" ? flags["archive-dir"] : undefined;
}
...
async function runArchive(args) {
    ...
    if (!result.ok) {
        console.error(`curate archive: refused — ${result.rejection.reason}: ${result.rejection.detail}`);
        return 3;
    }
    console.log(`curate: archived "${result.receipt.name}" → ${result.receipt.archiveRoot} ` +
...
async function runRestore(args) {
    ...
    console.log(`curate: restored "${receipt.name}" → ${receipt.restoredRoot} ` +
