...
export async function buildMemoryDecayReport(store, input = {}) {
    ...
    const items = nodes.map((node) => classifyNode(node, {
        now,
        staleDays,
        deprecateDays,
    ...
    const summary = {
        ...
        protected: keep.filter((item) => item.reasons.some((reason) => reason.includes("protected"))).length,
    ...
    const report = {
        ...
        status: summary.review + summary.deprecate + summary.expire > 0 ? "attention" : "clean",
        policy: {
...
function classifyNode(node, context) {
    ...
    const accessedAt = context.access?.accessed_at ??
    ...
    if (expired) {
        ...
        reasons.push(importance >= PINNED_IMPORTANCE_THRESHOLD
...
function score(action, ageDays, importance, accessCount) {
...
function appendSection(lines, title, items) {
    ...
    for (const item of items) {
        ...
        lines.push(`  Action: ${item.action}; tier: ${item.tier}; age: ${item.age_days}d; score: ${item.score.toFixed(4)}`);
...
function nextActions(summary) {
    ...
    if (actions.length === 0)
        actions.push("memory retention posture is clean");
    return actions;
}
