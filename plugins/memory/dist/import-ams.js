...
import { runPromotionEngine, } from "./promotion-engine.js";
import { start as sessionStart, current as sessionCurrent } from "./session-manager.js";
import { slugify } from "./store.js";
import { appendEvent, setRawTranscript, } from "./working-memory.js";
 ...
 */
export function inferNodeType(entry) {
    ...
    if (DECISION_PREFIX.test(text))
        return "decision";
    if (FIX_PREFIX.test(text))
        return "fix";
    if (GOTCHA_PREFIX.test(text))
        return "why_note";
    if (PROBLEM_PREFIX.test(text))
        return "problem";
    if (SOLUTION_PREFIX.test(text))
        return "solution";
    if (VALIDATION_PREFIX.test(text))
        return "validation";
    if (GOAL_PREFIX.test(text))
        return "goal";
    if (WHY_PREFIX.test(text))
        return "why_note";
    return "concept";
}
 ...
 */
export async function importAms(store, rootDir, dump) {
    const workingReport = {
        sessions: [],
        events: 0,
        transcripts: 0,
...
async function importLongTerm(store, _rootDir, entries) {
    const promoted_rids = [];
    const reinforced_rids = [];
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        const contradicts = CONFLICT_NODE_TYPES.has(nodeType) && detectConflicts(candidateNode, conflictPeers).length > 0;
        if (!contradicts) {
