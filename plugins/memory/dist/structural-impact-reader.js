...
export function structuralImpactReader(store) {
 ...
 */
export async function readStructuralImpact(store, target) {
    ...
    if (anchorFile == null && symbolNode == null)
        return { ...EMPTY };
    ...
    const imports = anchorFile
        ? materializeEdges(edges.filter((e) => e.label === "IMPORTS" && e.from_rid === anchorFile.rid), nodeByRid)
        : [];
    const importedBy = anchorFile
        ? materializeEdges(edges.filter((e) => {
            if (e.label !== "IMPORTS")
                return false;
        ...
        : [];
    const callAnchors = new Set(symbolNode ? [symbolNode.rid] : defines.map((node) => node.rid));
    const calls = callAnchors.size > 0
        ? materializeEdges(edges.filter((e) => e.label === "CALLS" && callAnchors.has(e.from_rid)), nodeByRid)
        : [];
    const calledBy = callAnchors.size > 0
        ? materializeEdges(edges.filter((e) => e.label === "CALLS" && callAnchors.has(e.to_rid)), nodeByRid)
        : [];
    const usesTypes = callAnchors.size > 0
        ? materializeEdges(edges.filter((e) => e.label === "USES_TYPE" && callAnchors.has(e.from_rid)), nodeByRid)
        : [];
    const usedByTypes = callAnchors.size > 0
        ? materializeEdges(edges.filter((e) => e.label === "USES_TYPE" && callAnchors.has(e.to_rid)), nodeByRid)
        : [];
    const referenceAnchors = callAnchors;
    const references = referenceAnchors.size > 0
        ? materializeEdges(edges.filter((e) => e.label === "REFERENCES" && referenceAnchors.has(e.from_rid)), nodeByRid)
        : [];
    const referencedBy = referenceAnchors.size > 0
        ? materializeEdges(edges.filter((e) => e.label === "REFERENCES" && referenceAnchors.has(e.to_rid)), nodeByRid)
        : [];
    return {
        imports,
        importedBy,
...
function findFile(nodes, targetFile) {
    return (nodes.find((n) => n.node_type === "file" && pathCandidates(n).some((path) => pathMatches(path, targetFile))) ?? null);
}
function findSymbol(nodes, edges, nodeByRid, symbol, fileNode) {
    const symbols = nodes.filter((n) => n.node_type === "symbol" && symbolMatches(n, symbol));
    if (fileNode == null)
        return symbols[0] ?? null;
    return (symbols.find((n) => edges.some((e) => e.label === "DEFINED_IN" && e.from_rid === n.rid && e.to_rid === fileNode.rid)) ??
        symbols.find((n) => definedInFile(n, edges, nodeByRid)?.rid === fileNode.rid) ??
        null);
}
function definedInFile(symbol, edges, nodeByRid) {
...
function materializeEdges(edges, nodeByRid) {
...
function importTargetsFile(importNode, fileNode) {
