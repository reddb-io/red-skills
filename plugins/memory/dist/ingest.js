 ...
 */
export async function ingestProject(store, opts) {
 ...
 */
export async function indexFile(store, path) {
...
function graphElements(path, nodes, hasDoc) {
    return [
        ...new Set([
            ...(hasDoc ? [`doc:${path}`] : []),
...
async function writeExtraction(store, extraction) {
    const report = { nodes: 0, edges: 0, docs: 0, elements: extraction.elements };
    if (extraction.doc) {
 ...
 */
export async function refreshFiles(store, paths, opts = {}) {
    const start = Date.now();
    ...
    for (const rawPath of [...new Set(paths)]) {
        ...
        if (!INDEXABLE_EXT.has(extname(path).toLowerCase()))
            continue;
 ...
 */
export async function reindexFiles(store, paths) {
...
async function readIndexHash(path) {
    if (isAssetFile(path)) {
        return contentHash(path, await readFile(path, "base64"));
    }
    return contentHash(path, await readFile(path, "utf8"));
}
async function readLegacyFileManifest(store) {
    const raw = await store.kvGet(LEGACY_FILE_MANIFEST_KEY);
    if (raw == null)
        return {};
    return typeof raw === "string" ? JSON.parse(raw) : raw;
}
async function readFileManifestEntry(store, path, legacyEntry) {
    const raw = await store.kvGet(fileManifestEntryKey(path));
    if (raw == null)
        return legacyEntry ? normalizeManifestEntry(legacyEntry) : undefined;
    const entry = typeof raw === "string" ? JSON.parse(raw) : raw;
    if (entry.format === "element-hash-chunks") {
        const elements = [];
        const chunks = Math.max(0, Math.trunc(entry.chunks ?? 0));
        for (let index = 0; index < chunks; index += 1) {
            const chunk = await store.kvGet(fileManifestChunkKey(path, index));
            if (typeof chunk === "string") {
                elements.push(...JSON.parse(chunk));
            }
...
async function writeFileManifestEntry(store, path, entry) {
...
function normalizeManifestEntry(entry) {
    if (entry.format === "element-hashes")
        return entry;
    return {
        hash: entry.hash,
...
function fileManifestEntryKey(path) {
    return `${FILE_MANIFEST_KEY_PREFIX}${contentHash("path", path)}`;
}
function fileManifestChunkKey(path, index) {
