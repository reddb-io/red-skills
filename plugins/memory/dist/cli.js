...
import { DEFAULT_MEMORY_EVENT_RETENTION_DAYS, readConfig, resolveL2Policy, resolveNotesDir, resolveStoreUri, skillTelemetryEnabled, } from "./config.js";
...
import { formatOutput, parseInput } from "./hook-adapters.js";
import { dispatch } from "./hook-runtime.js";
import { importAmsDump } from "./import-ams.js";
import { formatMarkdownReport, runBenchRecall } from "./bench-recall.js";
import { DEFAULT_WORKLOAD, formatLatencyReport, loadWorkloadOverrides, runBenchLatency, } from "./bench-latency.js";
import { runPromote } from "./promote.js";
import { runAfkLifecycle } from "./afk-lifecycle.js";
import { approveInboxItem, inboxItemToProvenance, listInboxItems, markInboxItemPromoted, quarantineInboxItem, readInboxItem, rejectInboxItem, } from "./inbox.js";
...
import { redactSensitiveValue, scanPrivacy, } from "./privacy.js";
...
import { buildPrePrMemoryReview, } from "./pre-pr-review.js";
...
import { buildMemoryOperationalDashboard, buildMemoryOperationalDashboardArtifact, } from "./operational-dashboard.js";
...
import { buildMemoryRoutingGuide, } from "./routing-guide.js";
...
import { buildMemoryHealthReport, engineEventHealth, } from "./memory-health.js";
...
import { buildWhatifReport, parseWhatifChange } from "./whatif.js";
...
import { structuralImpactReader, } from "./structural-impact-reader.js";
import { buildStructuralImpactViewerArtifact } from "./structural-impact-viewer.js";
import { listContradictions, resolveConflict, supersessionTimeline, } from "./supersession.js";
import { classifyCandidateMemory } from "./store-classifier.js";
import { recordReasoningAttempt, } from "./reasoning/attempt-writer.js";
...
import { ingestSkillEvents, parseSkillEvent, parseSkillEventInput, readRecentSkillEvents, readSkillRollups, } from "./skill-events.js";
...
const MEMORY_LAYERS = ["L1", "L2", "L3"];
function parseLayerFlag(value) {
    if (value == null || value === false)
        return undefined;
    if (value === true)
        throw new Error("--layer requires a value (L1|L2|L3)");
    const upper = value.toUpperCase();
    if (MEMORY_LAYERS.includes(upper))
        return upper;
    throw new Error(`invalid memory layer "${value}" — expected L1, L2, or L3`);
}
const CONFIDENCE_VALUES = ["EXTRACTED", "INFERRED", "AMBIGUOUS"];
function parseConfidence(value) {
    if (value == null || value === false)
        return undefined;
    if (value === true)
        throw new Error("--confidence requires a value");
...
async function requireConfig(rootDir) {
...
async function runInit(args) {
    ...
    if (!mode && args.flags.yes !== true && process.stdin.isTTY) {
        ...
        const answer = (await rl.question("What do you want to use? [markdown-only] / graph: ")).trim();
        rl.close();
    ...
    if (mode === "markdown-only") {
        ...
        if (skillTelemetry) {
            console.log(`  note:   skill telemetry is unsupported in markdown-only mode — re-run \`memory init --mode graph --skill-telemetry\` to enable it`);
        }
        return;
    }
    if (mode === "graph") {
        ...
        console.log(`  event retention: ${result.config.eventLog?.retentionDays ?? DEFAULT_MEMORY_EVENT_RETENTION_DAYS} day(s)`);
        console.log(`  vcs versioned: ${result.versioning.versioned.join(", ")}`);
        console.log(`  vcs skipped:   ${result.versioning.skipped.join(", ")}`);
        return;
    }
    throw new Error(`mode "${mode}" is not available yet — this build supports markdown-only and graph`);
}
async function runStore(args) {
    const rootDir = rootOf(args.flags);
    const fact = args.positional.join(" ").trim();
    if (!fact)
        throw new Error("nothing to store — pass a fact: memory store <fact>");
...
function printCommitResult(result) {
    if (result.committed) {
        ...
        if (result.previousCommit)
            console.log(`  previous: ${result.previousCommit}`);
    }
...
async function runInbox(args) {
    ...
    switch (action) {
        ...
        case "inspect": {
            ...
            if (!id)
                throw new Error("memory inbox inspect needs an item id");
        ...
        case "approve": {
            const id = args.positional[1];
            if (!id)
                throw new Error("memory inbox approve needs an item id");
        ...
        case "reject": {
            const id = args.positional[1];
            if (!id)
                throw new Error("memory inbox reject needs an item id");
            if (args.flags.yes !== true) {
        ...
        case "promote": {
            const id = args.positional[1];
            if (!id)
                throw new Error("memory inbox promote needs an item id");
            if (args.flags.yes !== true) {
                throw new Error("memory inbox promote requires explicit --yes approval");
            }
            const config = await requireConfig(rootDir);
            if (config.mode !== "graph") {
                throw new Error(`memory inbox promote needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
            }
            ...
            let rid = 0;
            try {
                rid = await store.upsertNode(factToNode(pending.fact, slugify, {
        ...
        default:
            throw new Error("usage: memory inbox quarantine|list|inspect|approve|reject|promote [args]");
    }
}
function parseInboxStatusFilter(value) {
...
function printInboxResult(action, item, json) {
    if (json) {
        console.log(JSON.stringify({ state: action, item }, null, 2));
        return;
    }
    console.log(`memory inbox: ${action} ${item.id}`);
    if (item.promotedRid != null)
        console.log(`  promoted node: ${item.promotedRid}`);
}
function printInboxList(items) {
    ...
    for (const item of items) {
        ...
        console.log(`  ${item.id} [${item.status}] ${item.fact.slice(0, 100)}${privacy} confidence=${item.provenance.confidence}`);
    }
}
function printInboxItem(item) {
    console.log(`memory inbox: ${item.id}`);
    console.log(`status: ${item.status}`);
    console.log(`fact: ${item.fact}`);
    console.log(`reason: ${item.reason}`);
    console.log(`evidence: ${item.evidenceSummary}`);
    console.log(`classification: ${item.classification.kind} tier=${item.classification.recommendedTier} scope=${item.classification.recommendedScope}`);
    ...
    if (item.rejectionReason)
        console.log(`rejection: ${item.rejectionReason}`);
    if (item.promotedRid != null)
        console.log(`promoted node: ${item.promotedRid}`);
}
function formatInboxProvenance(item) {
    const p = item.provenance;
    const parts = [p.sourceKind];
    if (p.writer)
        parts.push(`writer=${p.writer}`);
    if (p.command)
        parts.push(`command=${p.command}`);
    if (p.hook)
...
async function runProvenance(args) {
    const target = args.positional.join(" ").trim();
    if (!target)
        throw new Error("pass a node rid or label: memory provenance <rid|label>");
    const { store } = await openGraphStore(args);
    try {
        const node = await findNodeForProvenance(store, target);
        if (!node)
            throw new Error(`memory node not found: ${target}`);
...
async function runRecall(args) {
    ...
    if (!query)
        throw new Error("nothing to recall — pass a query: memory recall <query>");
    const config = await requireConfig(rootDir);
...
async function runFederate(args) {
    const rootDir = rootOf(args.flags);
    const query = (stringFlag(args.flags, "query") ?? args.positional.join(" ")).trim();
    if (!query) {
        throw new Error('nothing to federate — pass --query "<topic>" or memory federate <topic>');
    }
    ...
    console.log(`memory federate: "${report.query}" — ${report.results.length} hit(s) across ${report.roots_queried} root(s)`);
...
async function runAutocure(args) {
    ...
    try {
        ...
        console.log(`memory autocure (${mode}): ${report.actions_proposed.length} proposed, ${report.actions_applied.length} applied, ${report.skipped_claim_guarded.length} skipped (claim-guarded)`);
        console.log(`  entropy: ${report.entropy_before} -> ${report.entropy_after} (nodes=${report.totals.nodes}, edges=${report.totals.edges}, claim_guarded=${report.totals.claim_guarded})`);
        for (const [kind, counts] of Object.entries(report.by_kind)) {
            if (counts.proposed === 0 && counts.applied === 0)
                continue;
...
async function runReasoningReplay(args) {
    const task = (stringFlag(args.flags, "task") ?? args.positional.join(" ")).trim();
    if (!task) {
        throw new Error("nothing to replay — pass --task \"<descriptor>\" or memory reasoning-replay <descriptor>");
    }
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`memory reasoning-replay: "${report.task}" — ${report.results.length}/${report.total_attempts} attempt(s)`);
        if (report.results.length === 0) {
            console.log("  no past attempts in the reasoning tier yet");
            return;
        }
        for (const result of report.results) {
            console.log(`  [${result.similarity.toFixed(4)}] ${result.attempt_id}  ${result.when}`);
...
async function runWhatif(args) {
    ...
    if (sources.length === 0) {
        throw new Error('nothing to evaluate — pass one or more --change "<descriptor>" or memory whatif "<descriptor>" ["<descriptor>" ...]');
    }
    const changes = sources.map(parseWhatifChange);
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`memory whatif: ${report.changes.length} change(s) — breakage_likelihood ${report.breakage_likelihood.toFixed(3)} (self_confidence ${report.self_confidence.toFixed(2)})`);
        console.log(`  affected: ${report.affected.files.length} file(s), ${report.affected.symbols.length} symbol(s), ${report.affected.tests.length} test(s)`);
        ...
        if (report.historical_attempts.length === 0) {
            ...
            for (const attempt of report.historical_attempts) {
                console.log(`    [${attempt.similarity.toFixed(3)}] ${attempt.attempt_id} (${attempt.outcome})  ${attempt.when}`);
            }
        }
    }
    finally {
...
function collectRepeatedFlag(argv, key) {
    const flag = `--${key}`;
    const out = [];
...
async function runSmartSearch(args) {
    ...
    const query = args.positional.join(" ").trim();
    if (!query)
        throw new Error("nothing to search — pass a query: memory smart-search <query>");
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`  recall=${report.summary.recall_hits} docs=${report.summary.doc_hits} vector=${report.summary.vector_hits} (${report.summary.vector_status})`);
        for (const result of report.top_results.slice(0, 8)) {
            const ref = result.ref.path ?? result.ref.label ?? result.ref.rid ?? result.id;
            console.log(`  #${result.rank} ${result.kind} [${result.score.toFixed(3)}] ${ref} (${result.sources.join("+")})`);
            console.log(`      ${result.excerpt}`);
        }
...
async function runSmartSearchViewer(args) {
    const rootDir = rootOf(args.flags);
    const query = args.positional.join(" ").trim();
    if (!query) {
    ...
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, `.red/memory/smart-search-${safeName}.html`));
    const { store } = await openGraphStore(args);
    try {
        const report = await buildMemorySmartSearch(store, query, {
            limit: intFlag(args.flags, "limit"),
...
async function runContextPack(args) {
    const rootDir = rootOf(args.flags);
    const goal = args.positional.join(" ").trim();
    if (!goal)
        throw new Error("nothing to pack — pass a goal: memory context-pack <goal>");
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`context-pack needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    const budgetChars = typeof args.flags.budget === "string" ? Number(args.flags.budget) : undefined;
...
async function runContextPackViewer(args) {
    const rootDir = rootOf(args.flags);
    const goal = args.positional.join(" ").trim();
    if (!goal) {
    ...
    try {
        ...
        const outPath = resolve(stringFlag(args.flags, "out") ??
...
async function runRecommend(args) {
    ...
    if (!task)
        throw new Error("nothing to recommend — pass a task: memory recommend skills <task>");
    const { store } = await openGraphStore(args);
    try {
        const skillRollups = await readSkillRollups(store);
...
async function runPreflight(args) {
    ...
    if (!task)
        throw new Error("nothing to brief — pass a task: memory preflight <task>");
    const { store } = await openGraphStore(args);
    try {
...
async function runReadiness(args) {
    const goal = args.positional.join(" ").trim();
    if (!goal)
        throw new Error("nothing to assess — pass a goal: memory readiness <goal>");
    const { store } = await openGraphStore(args);
    try {
...
async function runReadinessViewer(args) {
    const goal = args.positional.join(" ").trim();
    if (!goal) {
        throw new Error("nothing to inspect — pass a goal: memory readiness-viewer <goal>");
    }
    const rootDir = rootOf(args.flags);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/readiness-viewer.html"));
    const { store } = await openGraphStore(args);
    try {
        const envelope = await buildReadinessEnvelope(store, goal, {
            limit: intFlag(args.flags, "limit"),
...
async function runDashboard(args) {
    ...
    try {
        ...
        const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/dashboard.html"));
...
async function runWorkbench(args) {
    ...
    try {
        ...
        const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/workbench.html"));
...
async function runCapabilities(args) {
    ...
    try {
        ...
        console.log(`memory capabilities: ${catalog.summary.ready}/${catalog.summary.total} ready, ${catalog.summary.red_db_backed} RedDB-backed`);
        for (const item of catalog.capabilities) {
            console.log(`  ${item.category}/${item.id}: ${item.status}`);
            if (item.cli.length > 0)
                console.log(`      cli: ${item.cli.join(", ")}`);
            if (item.mcp.length > 0)
                console.log(`      mcp: ${item.mcp.join(", ")}`);
        }
    }
    finally {
        await store.close();
...
async function runCompetitiveRadar(args) {
    const rootDir = resolve(rootOf(args.flags));
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`memory competitive radar: ${radar.summary.competitors} competitor(s), ${radar.summary.degraded_or_not_configured} gap signal(s)`);
        console.log(`  note: ${radar.note}`);
        for (const competitor of radar.competitors) {
            console.log(`  ${competitor.repository}: ${competitor.posture} score=${competitor.score.toFixed(3)} capabilities=${competitor.relevant_capabilities}`);
...
async function runCompetitiveEval(args) {
    const report = await evaluateCompetitiveEvalV2({ now: Date.now() });
    if (args.flags.json === true) {
        console.log(JSON.stringify(report, null, 2));
        return;
    }
    console.log(`memory competitive eval: ${report.composite.score}/${report.composite.maxScore} ${report.composite.status}`);
    ...
    if (report.claimGuards.unsupportedPublicClaims.length > 0) {
        console.log(`  unsupported public claims: ${report.claimGuards.unsupportedPublicClaims.join(", ")}`);
    }
}
async function runCompetitiveEvalViewer(args) {
    const rootDir = rootOf(args.flags);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/competitive-eval.html"));
    const report = await evaluateCompetitiveEvalV2({ now: Date.now() });
    const artifact = buildCompetitiveEvalViewerArtifact(report);
    await mkdir(dirname(outPath), { recursive: true });
    await writeFile(outPath, artifact.html, "utf8");
    console.log(`memory: competitive eval viewer written ${outPath}`);
    console.log(`  composite: ${report.composite.score}/${report.composite.maxScore} ${report.composite.status}`);
    console.log(`  contract: ${artifact.contract.consumes}`);
}
async function runMemoryLayers(args) {
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`memory layers: ${report.summary.ready_layers}/${report.summary.total_layers} ready, ${report.summary.red_db_backed_layers} RedDB-backed`);
...
async function runMemoryLayersViewer(args) {
    const rootDir = rootOf(args.flags);
    const { store } = await openGraphStore(args);
    try {
        const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/layers-viewer.html"));
        const report = await buildMemoryLayersReport(store);
        const artifact = buildMemoryLayersViewerArtifact(report);
        await mkdir(dirname(outPath), { recursive: true });
        await writeFile(outPath, artifact.html, "utf8");
        console.log(`memory: layers viewer written ${outPath}`);
        console.log(`  ready: ${report.summary.ready_layers}/${report.summary.total_layers}`);
        console.log(`  contract: ${artifact.contract.consumes}`);
    }
    finally {
        await store.close();
...
async function runHandoffViewer(args) {
    const rootDir = rootOf(args.flags);
    const focus = args.positional.join(" ").trim();
    const { store } = await openGraphStore(args);
    try {
        ...
        const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/handoff-viewer.html"));
        await mkdir(dirname(outPath), { recursive: true });
        await writeFile(outPath, artifact.html, "utf8");
...
async function runWorkFrontier(args) {
    const focus = args.positional.join(" ").trim();
    const { store } = await openGraphStore(args);
    try {
...
async function runWorkFrontierViewer(args) {
    const rootDir = rootOf(args.flags);
    const focus = args.positional.join(" ").trim();
    const { store } = await openGraphStore(args);
    try {
        ...
        const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/work-frontier.html"));
        await mkdir(dirname(outPath), { recursive: true });
        await writeFile(outPath, artifact.html, "utf8");
...
async function runMemoryDecay(args) {
    const { store } = await openGraphStore(args);
    try {
...
async function runMemoryDecayViewer(args) {
    const rootDir = rootOf(args.flags);
    const { store } = await openGraphStore(args);
    try {
        const report = await buildMemoryDecayReport(store, {
        ...
        const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/decay.html"));
        await mkdir(dirname(outPath), { recursive: true });
        await writeFile(outPath, artifact.html, "utf8");
...
async function runSessionShow(args) {
    const rootDir = rootOf(args.flags);
...
async function runSession(args) {
    ...
    if (action === "show")
        return runSessionShow(args);
    if (action === "start")
        return runSessionStart(args);
    if (action === "end")
        return runSessionEnd(args);
    if (action !== "timeline" && action !== "timeline-viewer") {
        throw new Error("session needs an action — supported: memory session show|start|end|timeline|timeline-viewer");
    }
    const { store } = await openGraphStore(args);
    try {
        ...
        if (action === "timeline-viewer") {
            const rootDir = rootOf(args.flags);
            const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/session-timeline.html"));
        ...
        for (const entry of timeline.entries) {
            console.log(`  ${entry.occurred_at} ${entry.session_id} ${entry.actor} ${entry.title} [${entry.outcome}]`);
            if (entry.detail)
                console.log(`      ${entry.detail}`);
        }
        for (const action of timeline.recommended_next_actions)
            console.log(`  next: ${action}`);
    }
    finally {
        await store.close();
...
async function runWorking(args) {
    const action = args.positional[0];
    if (action !== "append" &&
        action !== "get" &&
        action !== "raw" &&
        action !== "evict") {
        throw new Error("working needs an action — supported: memory working append|get|raw|evict");
    }
    const rootDir = rootOf(args.flags);
    const { store } = await openGraphStore(args);
    try {
        if (action === "append") {
            ...
            if (!type)
                throw new Error("working append requires --type <event-type>");
            if (value == null)
                throw new Error("working append requires --value <text>");
            const event = await workingAppendEvent(store, rootDir, { type, value });
            if (args.flags.json === true) {
                console.log(JSON.stringify(event, null, 2));
                return;
            }
            console.log(`memory: working append ok — session=${event.session_id} type=${event.type} seq=${event.sequence}`);
            return;
        }
        if (action === "get") {
            const type = stringFlag(args.flags, "type");
        ...
        if (action === "evict") {
            ...
            console.log(`memory: working evict — scanned=${report.scanned_nodes} evicted=${report.evicted.length} (ttl_ms=${ttlMs} byte_budget=${byteBudget})`);
            ...
            for (const s of report.by_session) {
                if (s.byte_budget_triggered) {
                    console.log(`  session=${s.session_id}: ${s.bytes_before}B → ${s.bytes_after}B (${s.evicted} event(s) evicted by budget)`);
                }
            }
            return;
        }
        ...
        if (setVal != null) {
            ...
            console.log(`memory: working raw set — session=${result.session_id} bytes=${Buffer.byteLength(result.value, "utf8")}`);
            return;
        }
...
async function runLearningDebt(args) {
    const rootDir = rootOf(args.flags);
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`learning-debt needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
...
async function runLearningDebtViewer(args) {
    const rootDir = rootOf(args.flags);
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`learning-debt-viewer needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    const telemetryEnabled = skillTelemetryEnabled(config);
    const store = await MemoryStore.open({ uri: resolveStoreUri(rootDir, config) });
    try {
        ...
        const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/learning-debt-viewer.html"));
        await mkdir(dirname(outPath), { recursive: true });
        await writeFile(outPath, artifact.html, "utf8");
...
async function runOnboardingMap(args) {
    const rootDir = rootOf(args.flags);
    ...
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`onboarding-map needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    const telemetryEnabled = skillTelemetryEnabled(config);
    const store = await MemoryStore.open({ uri: resolveStoreUri(rootDir, config) });
    try {
...
async function runOnboardingMapViewer(args) {
    const rootDir = rootOf(args.flags);
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`onboarding-map-viewer needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    const outPath = stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/onboarding-map-viewer.html");
    const telemetryEnabled = skillTelemetryEnabled(config);
    const store = await MemoryStore.open({ uri: resolveStoreUri(rootDir, config) });
    try {
        const map = await buildOnboardingMap(store, {
...
async function runRoutingGuide(args) {
...
async function runRoutingGuideViewer(args) {
    const rootDir = resolve(rootOf(args.flags));
    const guide = buildMemoryRoutingGuide({ agent: routingAgentFlag(args.flags) });
...
async function runAgentIntegrationStatus(args) {
    const rootDir = resolve(rootOf(args.flags));
    ...
    console.log(`  ready=${report.summary.ready} partial=${report.summary.partial} missing=${report.summary.missing}`);
    for (const agent of report.agents) {
        console.log(`  ${agent.agent}: ${agent.state} (${agent.target_files.map((file) => file.path).join(", ")})`);
    }
}
async function runAgentIntegrationStatusViewer(args) {
    const rootDir = resolve(rootOf(args.flags));
    const report = await buildMemoryAgentIntegrationStatus(rootDir, {
        agent: routingAgentFlag(args.flags),
    });
    const artifact = buildMemoryAgentIntegrationStatusViewerArtifact(report);
    const outPath = stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/agent-integration-status.html");
    await mkdir(dirname(outPath), { recursive: true });
    await writeFile(outPath, artifact.html, "utf8");
...
function routingAgentFlag(flags) {
...
function printRoutingGuide(guide) {
...
async function runOnboardingMapExport(args, rootDir) {
    if (args.flags["public-safe"] !== true) {
        throw new Error("onboarding-map export writes demo/comparison artifacts and requires --public-safe");
    }
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`onboarding-map export needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    ...
    if (privacy.status !== "ok") {
        throw new Error(`public-safe export refused: privacy scan could not guarantee safe output (${privacy.warnings.join("; ") || privacy.status})`);
    }
    ...
    try {
        ...
        const safeMap = redactSensitiveValue(map);
        ...
        const evidenceItems = safeMap.summary.concepts +
...
function publicSafeRefusalMessage(findings) {
    const lines = [
...
function publicFindingDiagnostic(finding) {
    return {
        kind: finding.kind,
...
function graphStateMetadata(nodes, edges) {
    const structural = {
        nodes: nodes
        ...
        edges: edges
            ...
            .sort((a, b) => a.rid - b.rid || a.label.localeCompare(b.label) || a.from - b.from || a.to - b.to),
    };
    return {
...
async function runAsk(args) {
    ...
    if (!question)
        throw new Error("nothing to ask — pass a question: memory ask <question>");
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`ask needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    const store = await MemoryStore.open({ uri: resolveStoreUri(rootDir, config) });
    try {
        ...
        for (const item of [...result.evidence.active, ...result.evidence.superseded]) {
            ...
            console.log(`  ${item.citation} memory_nodes:${item.rid} ${item.title} (${item.confidence}, ${item.status}${source})`);
        }
        ...
        if (result.federation_hits.length > 0) {
            ...
            for (const hit of result.federation_hits) {
                console.log(`  ${hit.origin_repo}${hit.id ? `:${hit.id}` : ""} score=${hit.score} local=${hit.confidence_local} remote=${hit.confidence_remote}`);
            }
        }
        if (result.cost) {
            console.log(`cost: ${result.cost.provider}/${result.cost.model} prompt=${result.cost.prompt_tokens} completion=${result.cost.completion_tokens} usd=${result.cost.cost_usd}`);
        }
    }
    finally {
        await store.close();
...
async function runDocs(args) {
    const action = args.positional[0];
    if (action === "brief")
        return runDocsBrief(args);
    if (action === "brief-viewer")
        return runDocsBriefViewer(args);
    if (action === "bundle")
        return runDocsBundle(args);
    if (action === "bundle-viewer")
        return runDocsBundleViewer(args);
    if (action === "read")
        return runDocsRead(args);
    if (action === "evidence-pack")
        return runDocsEvidencePack(args);
    if (action === "evidence-pack-viewer")
        return runDocsEvidencePackViewer(args);
    if (action === "backlinks")
        return runDocsBacklinks(args);
    if (action === "backlinks-viewer")
        return runDocsBacklinksViewer(args);
    if (action === "related")
        return runDocsRelated(args);
    if (action === "related-viewer")
        return runDocsRelatedViewer(args);
    if (action === "restore")
        return runDocsRestore(args);
    if (action === "coverage")
        return runDocsCoverage(args);
    if (action === "coverage-viewer")
        return runDocsCoverageViewer(args);
    if (action === "reference-graph")
        return runDocsReferenceGraph(args);
    if (action === "reference-graph-viewer")
        return runDocsReferenceGraphViewer(args);
    if (action === "search-viewer")
        return runDocsSearchViewer(args);
    if (action !== "search") {
        throw new Error("docs needs an action — supported: memory docs search <query>, memory docs search-viewer <query>, memory docs brief <query>, memory docs brief-viewer <query>, memory docs bundle <query>, memory docs bundle-viewer <query>, memory docs read <path|rid>, memory docs evidence-pack <path|rid>, memory docs evidence-pack-viewer <path|rid>, memory docs backlinks <label|rid>, memory docs backlinks-viewer <label|rid>, memory docs related <path|rid>, memory docs related-viewer <path|rid>, memory docs restore [path|rid], memory docs coverage, memory docs coverage-viewer, memory docs reference-graph, memory docs reference-graph-viewer");
    }
    const query = args.positional.slice(1).join(" ").trim();
    if (!query)
        throw new Error("nothing to search — pass a query: memory docs search <query>");
    const { store } = await openGraphStore(args);
    try {
...
async function runAssets(args) {
    ...
    try {
        ...
        console.log(`memory assets: ${report.total_assets} asset(s), ${formatAssetBytes(report.total_bytes)}`);
        ...
        for (const asset of report.assets) {
            console.log(`  ${asset.path}: ${asset.asset_kind}, ${asset.media_type}, ${formatAssetBytes(asset.bytes)}`);
        }
    }
    finally {
        await store.close();
...
async function runAssetsViewer(args) {
    const rootDir = rootOf(args.flags);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/asset-inventory-viewer.html"));
    const { store } = await openGraphStore(args);
    try {
        const report = await buildMemoryAssetInventory(store, {
            kind: stringFlag(args.flags, "kind"),
...
async function runDocsBundle(args) {
    const query = args.positional.slice(1).join(" ").trim();
    if (!query)
        throw new Error("nothing to bundle — pass a query: memory docs bundle <query>");
    const { store } = await openGraphStore(args);
    try {
...
async function runDocsSearchViewer(args) {
    const query = args.positional.slice(1).join(" ").trim();
    if (!query) {
    ...
    const rootDir = rootOf(args.flags);
    const safeName = createHash("sha256").update(query).digest("hex").slice(0, 12);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, `.red/memory/doc-search-${safeName}.html`));
    const { store } = await openGraphStore(args);
    try {
        const report = await searchDocs(store, query, { limit: intFlag(args.flags, "limit") });
...
async function runDocsBrief(args) {
    const query = args.positional.slice(1).join(" ").trim();
    if (!query)
        throw new Error("nothing to brief — pass a query: memory docs brief <query>");
    const { store } = await openGraphStore(args);
    try {
...
async function runDocsBriefViewer(args) {
    const query = args.positional.slice(1).join(" ").trim();
    if (!query) {
    ...
    const rootDir = rootOf(args.flags);
    const safeName = createHash("sha256").update(query).digest("hex").slice(0, 12);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, `.red/memory/doc-brief-${safeName}.html`));
    const { store } = await openGraphStore(args);
    try {
        const brief = await buildDocBrief(store, {
            query,
...
async function runDocsBundleViewer(args) {
    const query = args.positional.slice(1).join(" ").trim();
    if (!query) {
    ...
    const rootDir = rootOf(args.flags);
    const safeName = createHash("sha256").update(query).digest("hex").slice(0, 12);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, `.red/memory/doc-bundle-${safeName}.html`));
    const { store } = await openGraphStore(args);
    try {
        const bundle = await buildDocBundle(store, {
            query,
...
async function runDocsCoverage(args) {
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`memory docs coverage: ${report.grounded_docs}/${report.total_docs} grounded, ${report.docs_with_references} with references`);
        console.log(`  vectors: ${report.vector.overall} (${report.vector.ready}/${report.vector.total} ready)`);
        for (const warning of report.warnings)
            console.log(`  warning: ${warning}`);
        for (const doc of report.docs) {
            const title = doc.title ? ` — ${doc.title}` : "";
            console.log(`  ${doc.path}${title}: ${doc.graph_status}, refs=${doc.references.count}, vector=${doc.vector_status}`);
        }
    }
    finally {
        await store.close();
...
async function runDocsCoverageViewer(args) {
    const rootDir = rootOf(args.flags);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/doc-coverage-viewer.html"));
    const { store } = await openGraphStore(args);
    try {
        const report = await buildDocCoverageReport(store);
...
async function runDocsReferenceGraph(args) {
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`memory docs reference-graph: ${report.reference_edges} edge(s), ${report.reference_nodes} referenced node(s), ${report.grounded_docs}/${report.total_docs} grounded docs`);
        for (const warning of report.warnings)
            console.log(`  warning: ${warning}`);
        for (const ref of report.top_references.slice(0, 10)) {
            console.log(`  ${ref.node.title} (${ref.node.label}) referenced by ${ref.incoming_docs} doc(s)`);
        }
    }
    finally {
        await store.close();
...
async function runDocsReferenceGraphViewer(args) {
    const rootDir = rootOf(args.flags);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/doc-reference-graph-viewer.html"));
    const { store } = await openGraphStore(args);
    try {
        const report = await buildDocReferenceGraphReport(store);
...
async function runDocsRelated(args) {
    const target = args.positional.slice(1).join(" ").trim();
    if (!target)
        throw new Error("nothing to relate — pass a path or rid: memory docs related <path|rid>");
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`memory docs related: ${report.target.path ?? report.target.title} (${report.references.length} reference(s), ${report.related_docs.length} related doc(s))`);
        for (const warning of report.warnings)
            console.log(`  warning: ${warning}`);
...
async function runDocsBacklinks(args) {
    const target = args.positional.slice(1).join(" ").trim();
    if (!target) {
    ...
    try {
        ...
        console.log(`memory docs backlinks: ${report.references.length} reference node(s), ${report.docs.length} doc(s) for ${target}`);
        for (const warning of report.warnings)
            console.log(`  warning: ${warning}`);
...
async function runDocsBacklinksViewer(args) {
    const target = args.positional.slice(1).join(" ").trim();
    if (!target) {
        throw new Error("nothing to render — pass a label, title, or rid: memory docs backlinks-viewer <label|rid>");
    }
    const rootDir = rootOf(args.flags);
    const safeName = isIntegerText(target)
        ? `rid-${target}`
        : createHash("sha256").update(target).digest("hex").slice(0, 12);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, `.red/memory/doc-backlinks-${safeName}.html`));
    const { store } = await openGraphStore(args);
    try {
        const report = await buildDocBacklinksReport(store, {
            ...(isIntegerText(target) ? { rid: Number(target) } : { query: target }),
...
async function runDocsRelatedViewer(args) {
    const target = args.positional.slice(1).join(" ").trim();
    if (!target) {
    ...
    const safeName = isIntegerText(target)
        ? `rid-${target}`
        : createHash("sha256").update(target).digest("hex").slice(0, 12);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, `.red/memory/doc-related-${safeName}.html`));
    const { store } = await openGraphStore(args);
    try {
        const report = await buildDocRelatedReport(store, {
            ...(isIntegerText(target) ? { rid: Number(target) } : { path: target }),
...
async function runDocsRead(args) {
    const target = args.positional.slice(1).join(" ").trim();
    if (!target)
        throw new Error("nothing to read — pass a path or rid: memory docs read <path|rid>");
    const { store } = await openGraphStore(args);
    try {
...
async function runDocsEvidencePack(args) {
    const target = args.positional.slice(1).join(" ").trim();
    if (!target) {
        throw new Error("nothing to pack — pass a path or rid: memory docs evidence-pack <path|rid>");
    }
    const { store } = await openGraphStore(args);
    try {
...
async function runDocsEvidencePackViewer(args) {
    const target = args.positional.slice(1).join(" ").trim();
    if (!target) {
        throw new Error("nothing to render — pass a path or rid: memory docs evidence-pack-viewer <path|rid>");
    }
    const rootDir = rootOf(args.flags);
    const safeName = isIntegerText(target)
        ? `rid-${target}`
...
async function runDocsRestore(args) {
    const rootDir = rootOf(args.flags);
    const target = args.positional.slice(1).join(" ").trim();
    ...
    try {
        ...
        console.log(`memory docs ${mode}: ${report.summary.restored} restored, ${report.summary.planned} planned, ${report.summary.skipped} skipped, ${report.summary.missing} missing`);
        if (report.dry_run)
            console.log("  pass --yes to write restored document files");
...
async function runBackup(args) {
    ...
    if (action === "list") {
        ...
        for (const backup of backups) {
            console.log(`  ${backup.name} ${backup.mode} files=${backup.files} bytes=${backup.bytes} created=${backup.created_at}`);
        }
        return;
    }
    if (action === "inspect") {
        ...
        if (!name)
            throw new Error("memory backup inspect needs a backup name");
    ...
    if (action === "restore") {
        const name = args.positional[1];
        if (!name)
            throw new Error("memory backup restore needs a backup name");
        if (args.flags.yes !== true) {
...
async function runServe(args) {
    ...
    if (tokenEnv && !token)
        throw new Error(`--token-env ${tokenEnv} is not set`);
    const { store } = await openGraphStore(args);
...
async function runHooks(args) {
    const action = args.positional[0];
    if (action === "coverage-viewer")
        return runHooksCoverageViewer(args);
    if (action !== "coverage") {
    ...
    console.log(`memory hooks coverage: ${report.summary.enabled_events}/${report.summary.total_events} enabled (${report.mode})`);
    for (const runner of report.runners) {
        console.log(`  ${runner.runner}: ${runner.coverage.enabled}/${runner.coverage.total} enabled, ${runner.coverage.wired} wired`);
...
async function runHooksCoverageViewer(args) {
    const rootDir = rootOf(args.flags);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/hook-coverage-viewer.html"));
...
async function runClaimCheck(args) {
    const rootDir = rootOf(args.flags);
    ...
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`claim-check needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    const store = await MemoryStore.open({ uri: resolveStoreUri(rootDir, config) });
    try {
...
function printClaimCheck(result) {
...
async function runIngest(args) {
    ...
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`ingest needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    const cwd = isAbsolute(target) ? target : resolve(rootDir, target);
    const maxFiles = typeof args.flags["max-files"] === "string"
    ...
    try {
        ...
        console.log(`  ${report.files} file(s) → ${report.nodes} node(s), ${report.edges} edge(s), ${report.docs} doc(s) in ${report.durationMs}ms`);
 ...
 */
async function runDriftGuard(args) {
    const rootDir = rootOf(args.flags);
    ...
    if (verdict.status === "pass") {
        if (args.flags.json !== true) {
            if (verdict.reason === "no-watched-paths") {
                ...
                console.log(`memory drift-guard: audit marker present (${verdict.marker.form}) — pass`);
            }
        }
        return;
    }
...
async function driftGuardChangedFiles(rootDir, args) {
    ...
    throw new Error("drift-guard needs the PR's changed files — pass --changed-files <path> or --base <ref>");
}
/** Resolve the head commit message: an explicit `--head-message <path>`, or `git log -1` at HEAD. */
async function driftGuardHeadMessage(rootDir, args) {
...
async function driftGuardAuditLog(rootDir) {
    try {
...
async function driftGuardRecordEvent(rootDir, event) {
    try {
        const config = await readConfig(rootDir);
...
async function runBootstrap(args) {
    ...
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`bootstrap needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    const store = await MemoryStore.open({ uri: resolveStoreUri(rootDir, config) });
    try {
        ...
        console.log(`memory bootstrap: ${mode} ${report.summary.indexed_sources}/${report.summary.discovered_sources} source(s)`);
        console.log(`  ${report.summary.nodes} node(s), ${report.summary.edges} edge(s), ${report.summary.docs} doc(s)`);
...
async function runRefresh(args) {
    ...
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`refresh needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    ...
    console.log(`  ${report.added} added, ${report.updated} updated, ${report.skipped} skipped, ${report.stale} stale graph element(s) in ${report.durationMs}ms`);
}
async function refreshPaths(rootDir, args) {
    const paths = [...args.positional];
    const hasRefreshSource = paths.length > 0 ||
    ...
    if (!hasRefreshSource) {
        throw new Error("refresh needs changed files — pass paths, --stdin, --staged, or --changed");
    }
    return [...new Set(paths)];
}
function splitPathList(input) {
    return input
        .split(/\0|\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean);
}
async function gitDiffPaths(rootDir, mode) {
    const diffArgs = [
        "-C",
        rootDir,
        "diff",
...
async function runSkillEvent(args) {
    const kind = args.positional[0];
    if (kind !== "skill") {
    ...
    if (config.mode !== "graph") {
        console.log(`memory: skill event ignored — needs graph mode, this project is "${config.mode}"`);
        return;
    }
    if (!skillTelemetryEnabled(config)) {
        console.log("memory: skill event ignored — skill telemetry is not enabled, re-run `memory init --mode graph --skill-telemetry`");
        return;
    }
...
async function runCurate(args) {
    ...
    if (!skillTelemetryEnabled(config)) {
        console.log("memory: curate ignored — skill telemetry is not enabled, re-run `memory init --mode graph --skill-telemetry`");
        return;
    }
    const store = await MemoryStore.open({ uri: resolveStoreUri(rootDir, config) });
    let report;
    ...
    console.log(`memory: skill curator (report-only) — ${report.totalSkills} skill(s), ` +
    ...
    console.log(`  ${report.recommendations.length} recommendation(s) (stale threshold ${report.staleDays}d):`);
 ...
 */
async function runImprove(args) {
    const kind = args.positional[0];
    if (kind === "apply")
        return runImproveApply(args);
    if (kind === "proposals")
        return runImproveProposals(args);
    if (kind !== "skills") {
    ...
    if (!skillTelemetryEnabled(config)) {
        return reportImproveState(json, "unavailable", "skill telemetry is not enabled, re-run `memory init --mode graph --skill-telemetry`", []);
    }
    const store = await MemoryStore.open({ uri: resolveStoreUri(rootDir, config) });
    let report;
    let recent;
...
async function runImproveProposals(args) {
...
async function runImproveProposalsList(args) {
    const rootDir = rootOf(args.flags);
    const json = args.flags.json === true;
...
async function runImproveProposalsShow(args) {
    const proposalArg = args.positional[2];
    if (!proposalArg)
        throw new Error("memory improve proposals show needs a proposal file");
    const rootDir = rootOf(args.flags);
    const json = args.flags.json === true;
...
async function runImproveProposalsArchive(args) {
    const proposalArg = args.positional[2];
    if (!proposalArg)
        throw new Error("memory improve proposals archive needs a proposal file");
    if (args.flags.yes !== true) {
...
async function listPendingProposalFiles(rootDir) {
...
async function summarizeProposalFile(rootDir, proposalPath, body) {
...
function firstProposalField(body, pattern) {
    const match = body.match(pattern);
    return match ? match[1].trim() : null;
}
function proposalRoot(rootDir) {
    return join(rootDir, ".red", "memory", "proposals");
}
function assertInsideProposalTree(rootDir, filePath) {
...
function isArchiveReason(reason) {
    return reason === "applied" || reason === "rejected" || reason === "stale";
}
async function runImproveApply(args) {
    const proposalArg = args.positional[1];
    if (!proposalArg)
        throw new Error("memory improve apply needs a proposal file");
    if (args.flags.yes !== true) {
...
function parseSkillPatchBlock(proposal) {
    const match = proposal.match(/```json memory-skill-patch\s*([\s\S]*?)```/);
    if (!match)
        throw new Error("proposal needs a structured memory-skill-patch block");
    let raw;
    try {
    ...
    if (typeof path !== "string" || path.trim() === "")
        throw new Error("memory-skill-patch.path is required");
    if (typeof oldString !== "string" || oldString === "")
        throw new Error("memory-skill-patch.oldString is required");
    if (typeof newString !== "string")
        throw new Error("memory-skill-patch.newString is required");
    return { path, oldString, newString };
}
function assertInsideRoot(rootDir, filePath, label) {
...
async function buildSkillImprovementProposals(rootDir, recommendations, recentEvents, writeProposal) {
...
async function renderSkillImprovementProposal(rootDir, rec, evidence, fingerprint) {
    const relSkillPath = isAbsolute(rec.path) ? toPosix(relative(rootDir, rec.path)) : rec.path;
...
function recentFailureEvidence(skillName, events) {
    return events
        .filter((event) => event.name === skillName && event.event_type === "result" && event.status === "failed")
        .slice(0, 5);
}
function renderRecentFailureEvidence(evidence) {
...
function semanticTroubleshootingNote(reason, evidence) {
...
async function renderDraftSkillPatchBlock(rootDir, rec, relSkillPath, evidence) {
...
function semanticSectionAnchor(text, evidence) {
    ...
    for (const heading of headings) {
        ...
        if (section && countOccurrences(text, section) === 1)
            return section;
    }
    return null;
}
function semanticHeadingCandidates(stage, klass) {
    const candidates = [];
...
function markdownSectionByHeading(text, headingPattern) {
...
function uniqueTailAnchor(text) {
    ...
    for (let lineCount = 1; lineCount <= Math.min(lines.length, 40); lineCount++) {
        ...
        if (candidate.length > maxAnchorChars)
            break;
...
function reportImproveState(json, state, reason, proposals) {
    if (json) {
...
async function runStatus(args) {
    ...
    if (config.mode !== "graph") {
        return reportStatusState(json, "no-op", `skill telemetry needs graph mode, this project is "${config.mode}"`, { hint: "re-run `memory init --mode graph --skill-telemetry`" });
    }
    if (!skillTelemetryEnabled(config)) {
    ...
    console.log(`  ${rollups.length} ${plural(rollups.length, "skill")} observed ` +
    ...
    if (shownRollups.length === 0) {
        ...
        for (const r of shownRollups) {
            ...
            console.log(`  ${r.name} (${r.source_kind}) — ${r.event_count} event(s) ` +
...
async function runHealth(args) {
    ...
    console.log(`  initialized=${yesNo(report.initialized)} graph=${yesNo(report.graphMode)} ` +
        `telemetry=${report.skillTelemetry} freshness=${report.graphFreshness.state}`);
    console.log(`  rollups=${report.rollups} proposal-candidates=${report.proposalCandidates} ` +
    ...
    if (report.topProposals.length > 0) {
        ...
        for (const proposal of report.topProposals) {
            console.log(`  - ${proposal.priority} ${proposal.score.toFixed(2)} ${proposal.skill}: ${proposal.reason}`);
        }
    }
...
async function runHealthViewer(args) {
    const rootDir = rootOf(args.flags);
    const { store } = await openGraphStore(args);
    try {
        ...
        const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/health-viewer.html"));
        await mkdir(dirname(outPath), { recursive: true });
        await writeFile(outPath, artifact.html, "utf8");
...
async function runGovernance(args) {
    const { store } = await openGraphStore(args);
    try {
...
async function runGovernanceViewer(args) {
    const rootDir = rootOf(args.flags);
    const { store } = await openGraphStore(args);
    try {
        const report = await buildMemoryGovernanceReport(store, {
        ...
        const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/governance-viewer.html"));
        await mkdir(dirname(outPath), { recursive: true });
        await writeFile(outPath, artifact.html, "utf8");
...
function printGovernance(report) {
    ...
    console.log(`  provenance=${report.summary.nodes_with_provenance}/${report.summary.total_nodes} ` +
...
async function runPrivacy(args) {
    ...
    if (action === "export") {
        ...
        try {
            ...
            console.log(`memory: redacted export — ${result.nodes} node(s), ${result.edges} edge(s), ${result.privacyFindings} finding(s) redacted`);
            if (communities)
                console.log(`  communities: coloured via native Louvain`);
...
function printPrivacyReport(report) {
    console.log(`memory: privacy scan — ${report.status} (${report.mode}, ${report.totalMemories} ${plural(report.totalMemories, "memory")})`);
    for (const warning of report.warnings) {
        console.log(`  warning: ${warning}`);
    }
...
function printLintReport(report) {
    console.log(`memory: lint — ${report.status} (${report.mode}, ${report.totalMemories} ${plural(report.totalMemories, "memory")})`);
    for (const warning of report.warnings) {
        console.log(`  warning: ${warning}`);
    }
    if (report.findings.length === 0) {
...
async function healthReport(rootDir) {
    ...
    if (graphMode && config) {
        ...
        try {
            if (telemetryEnabled) {
                ...
                topProposals = await buildSkillImprovementProposals(rootDir, curated.recommendations, recent, false);
            }
...
function healthState(input) {
    if (!input.initialized)
        return "missing";
    if (!input.graphMode || !input.telemetryEnabled || input.graphFreshnessState === "stale")
        return "degraded";
    if (input.highPriorityProposals > 0 || input.pendingProposalFiles > 0)
        return "attention";
    return "ready";
}
function healthRecommendations(input) {
    ...
    if (items.length === 0)
        items.push("memory is healthy; continue collecting telemetry");
    return items;
}
async function runContextStatus(args) {
    const rootDir = resolve(rootOf(args.flags));
    const json = args.flags.json === true;
    ...
    console.log(`  domain: glossary=${yesNo(report.committedContext.domainGlossary)} ` +
        `ADRs=${report.committedContext.adrCount} map=${yesNo(report.committedContext.contextMap)}`);
    console.log(`  memory: ${report.memory.mode}` +
...
async function contextStatusReport(rootDir) {
    const config = await readConfig(rootDir);
    ...
    const graphFreshness = config?.mode === "graph" && graphStoreExists
        ? await graphFreshnessStatus(rootDir, graphStorePath)
        : { state: "unavailable", newerFiles: [] };
    ...
    const checks = [
        ...
        },
        {
            ...
            reason: graphFreshness.state === "fresh"
...
function contextRecommendations(input) {
    const items = [];
    if (!input.agentRules)
        items.push("add CLAUDE.md or AGENTS.md with agent rules");
    if (!input.domainGlossary)
        items.push("add .red/CONTEXT.md for the project glossary");
    if (!input.config) {
    ...
    if (input.wikiState === "absent")
        items.push("run `/wiki-init` if the project needs a durable research/wiki layer");
    if (input.wikiState === "partial")
        items.push("repair the LLM Wiki setup so both .red/agents/wiki.md and .red/wiki/ exist");
    return items;
}
...
async function storeExists(rootDir, storePath) {
...
async function graphFreshnessStatus(rootDir, storePath) {
...
async function scanProjectFreshness(rootDir, storeMtimeMs) {
    const newerFiles = [];
    let newestProjectMtimeMs = null;
    let scannedFiles = 0;
    async function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            if (shouldSkipFreshnessPath(rel, entry.isDirectory()))
                continue;
...
function shouldSkipFreshnessPath(rel, isDir) {
    const first = rel.split("/")[0];
    if ([".git", "node_modules", "dist", "build", "coverage", ".turbo", ".next"].includes(first)) {
        return true;
    }
    if (rel === ".red/memory" || rel.startsWith(".red/memory/"))
        return true;
    if (rel === ".red/wiki" || rel.startsWith(".red/wiki/"))
        return true;
    if (isDir && entryLooksLikeCache(first))
        return true;
    return false;
}
function entryLooksLikeCache(name) {
...
function reportStatusState(json, state, reason, opts = {}) {
    if (json) {
    ...
    console.log(`  ${reason}`);
    if (opts.hint)
        console.log(`  ${opts.hint}`);
}
/** Compact `succeeded=3 failed=1` summary of a rollup's outcome counts. */
function formatOutcomes(counts) {
    return Object.entries(counts)
        .filter(([, n]) => typeof n === "number" && n > 0)
        .map(([status, n]) => `${status}=${n}`)
        .join(" ");
}
function skillEventFromFlags(flags) {
    const event = {
    ...
    if (resultKeys.some((key) => flags[key] !== undefined)) {
        event.result = {
            status: flags.status,
            duration_ms: typeof flags["duration-ms"] === "string" ? Number(flags["duration-ms"]) : undefined,
...
async function runExtract(args) {
    ...
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`extract needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
...
async function runExtraction(args) {
    ...
    if (action === "status-viewer")
        return runExtractionStatusViewer(args);
    if (action !== "status") {
    ...
    try {
        ...
        console.log(`memory extraction: inferred=${status.inferred.available ? "available" : "unavailable"} facts=${status.inferred.facts}`);
        if (status.inferred.mode) {
            console.log(`  provider: ${status.inferred.mode}/${status.inferred.model} (${status.inferred.egress})`);
        }
        if (status.inferred.error)
            console.log(`  warning: ${status.inferred.error}`);
        for (const action of status.recommended_next_actions)
            console.log(`  next: ${action}`);
    }
    finally {
        await store.close();
...
async function runExtractionStatusViewer(args) {
    const rootDir = resolve(rootOf(args.flags));
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/extraction-status-viewer.html"));
    const { store } = await openGraphStore(args);
    try {
        const status = await buildMemoryExtractionStatus(store, rootDir);
...
async function openGraphStore(args) {
    const rootDir = rootOf(args.flags);
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`this verb needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
...
function strFlag(flags, key, fallback) {
    return typeof flags[key] === "string" ? flags[key] : fallback;
}
async function runSearch(args) {
    const query = args.positional.join(" ").trim();
    if (!query)
        throw new Error("nothing to search — pass a query: memory search <query>");
    const { store } = await openGraphStore(args);
    try {
        const hits = await search(store, query, intFlag(args.flags, "limit") ?? 20);
        if (hits.length === 0)
            return void console.log(`memory: no matches for "${query}"`);
        console.log(`memory: ${hits.length} match(es) for "${query}"`);
        for (const h of hits) {
...
async function runNeighbors(args) {
    const label = args.positional[0];
    if (!label)
        throw new Error("pass a node label: memory neighbors <label>");
    const { store } = await openGraphStore(args);
    try {
        const rows = await neighbors(store, label, intFlag(args.flags, "depth") ?? 1, strFlag(args.flags, "direction", "both"));
        console.log(`memory: ${rows.length} neighbor(s) of "${label}"`);
        for (const n of rows)
            console.log(`  d${n.depth} ${n.rid} (${n.node_type}) ${n.label}`);
    }
    finally {
        await store.close();
    }
}
async function runTraverse(args) {
    const label = args.positional[0];
    if (!label)
        throw new Error("pass a start label: memory traverse <label>");
    const { store } = await openGraphStore(args);
    try {
...
async function runPath(args) {
    ...
    if (!from || !to)
        throw new Error("pass two labels: memory path <from> <to>");
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`memory: path "${from}" → "${to}": ${result.hopCount} hop(s), weight ${result.totalWeight}`);
    }
    finally {
        await store.close();
    }
}
async function runPathExplain(args) {
    const [from, to] = args.positional;
    if (!from || !to)
        throw new Error("pass two labels: memory path-explain <from> <to>");
    const { store } = await openGraphStore(args);
    try {
...
async function runPathExplainViewer(args) {
    const [from, to] = args.positional;
    if (!from || !to)
        throw new Error("pass two labels: memory path-explain-viewer <from> <to>");
    const rootDir = rootOf(args.flags);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/path-explain-viewer.html"));
    const { store } = await openGraphStore(args);
    try {
        const report = await buildPathExplainReport(store, {
            from,
...
async function runConfidence(args) {
...
async function runConflicts(args) {
    const { store } = await openGraphStore(args);
    try {
...
async function runSupersede(args) {
...
async function runResolveConflict(args) {
    const activeArg = typeof args.flags.active === "string" ? args.flags.active : args.positional[0];
    const supersededArg = typeof args.flags.superseded === "string" ? args.flags.superseded : args.positional[1];
    if (!activeArg || !supersededArg) {
        throw new Error("pass active and superseded rids: memory resolve-conflict <active-rid> <superseded-rid>");
    }
...
async function runTimeline(args) {
    ...
    if (!topic)
        throw new Error("pass a topic or rid: memory timeline <topic|rid>");
    const { store } = await openGraphStore(args);
    try {
...
async function runCommunities(args) {
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`memory: ${report.communities.length} community(ies), ${report.assignments.length} assigned node(s)`);
...
async function runCommunitiesViewer(args) {
    const rootDir = rootOf(args.flags);
...
function printConflicts(conflicts) {
    ...
    for (const conflict of conflicts) {
        ...
        console.log(`  [${status}] ${conflict.from.rid} ${conflict.from.label} <-> ${conflict.to.rid} ${conflict.to.label}`);
        if (conflict.reason)
            console.log(`        ${conflict.reason}`);
    }
}
function printTimeline(timeline, opts) {
...
async function runStructuralImpact(args) {
    const target = {
...
async function runPrePrReview(args) {
    const rootDir = rootOf(args.flags);
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`pre-pr-review needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
...
async function runPrePrReviewViewer(args) {
    const rootDir = rootOf(args.flags);
    const config = await requireConfig(rootDir);
    if (config.mode !== "graph") {
        throw new Error(`pre-pr-review-viewer needs graph mode — this project is "${config.mode}". Re-run \`memory init --mode graph\` first`);
    }
    const comparison = stringFlag(args.flags, "range") ?? stringFlag(args.flags, "comparison");
    const changedFiles = await readChangedFiles(rootDir, comparison);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/pre-pr-review-viewer.html"));
    const store = await MemoryStore.open({ uri: resolveStoreUri(rootDir, config) });
    try {
        const review = await buildPrePrMemoryReview(store, { changedFiles, comparison });
...
async function runStructuralImpactViewer(args) {
    const target = {
        file: typeof args.flags.file === "string" ? args.flags.file : undefined,
        symbol: typeof args.flags.symbol === "string" ? args.flags.symbol : undefined,
    };
    ...
    const rootDir = rootOf(args.flags);
    const outPath = resolve(stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/structural-impact-viewer.html"));
    const { store } = await openGraphStore(args);
    try {
        const impact = await structuralImpactReader(store)(target);
...
function printStructuralImpact(target, impact) {
...
async function readChangedFiles(rootDir, comparison) {
    ...
    try {
        ...
        if (comparison)
            throw err;
        const { stdout } = await execFileAsync("git", ["diff", "--name-only", "--diff-filter=ACMRTUXB", "--"], { cwd: rootDir });
        return parseChangedFiles(stdout);
    }
}
...
function printPrePrReview(review) {
    ...
    if (review.evidence.length > 0) {
        ...
        for (const item of review.evidence) {
            const source = item.source ? ` source=${item.source}` : "";
            console.log(`  ${item.marker} ${item.urn} ${item.title} (${item.nodeType}, ${item.confidence}${source})`);
        }
    }
}
function printPrePrSection(title, section) {
    console.log(`${title}:`);
...
function printReadinessEnvelope(envelope) {
    console.log(`memory: readiness — ${envelope.status}`);
    console.log(`  goal: ${envelope.request.goal}`);
    console.log(`  evidence: ${envelope.retrieval.recall.active_evidence_count}/${envelope.retrieval.recall.evidence_count} active`);
    console.log(`  vector: ${envelope.retrieval.vector.overall} (${envelope.retrieval.vector.ready}/${envelope.retrieval.vector.total} ready)`);
    console.log(`  trust: provenance=${envelope.trust.provenance.nodes_with_provenance}/${envelope.trust.provenance.total_nodes} ` +
    ...
    console.log(`  vcs: ${envelope.vcs.time_travel} (${envelope.vcs.collections
        .map((collection) => `${collection.name}:${collection.status}`)
        .join(", ")})`);
    console.log(`  telemetry: ${envelope.operations.event_log.total_events} event(s) ` +
        `community-signals=${envelope.communities.communities}/${envelope.communities.assignments}`);
    if (envelope.evidence.missing.missing) {
        console.log(`  missing evidence: ${envelope.evidence.missing.active_count}/${envelope.evidence.missing.expected_minimum} active`);
        for (const message of envelope.evidence.missing.messages)
            console.log(`    ${message}`);
    }
    if (envelope.evidence.contradictions.length > 0) {
        console.log(`  contradictions: ${envelope.evidence.contradictions.length}`);
        for (const warning of envelope.evidence.contradictions)
    ...
    if (envelope.skills.signal_status === "available" && envelope.skills.recommendations.length > 0) {
        console.log(`  skills: ${envelope.skills.recommendations.map((item) => item.name).join(", ")}`);
    }
    else {
        console.log(`  skills: ${envelope.skills.status}`);
    }
    console.log(`  learning debt: ${envelope.learning_debt.status}` +
...
async function runVector(args) {
    const action = args.positional[0];
    if (action !== "status" && action !== "status-viewer" && action !== "maintain" && action !== "search") {
        throw new Error("vector needs an action — supported: memory vector status|status-viewer|maintain|search");
    }
    ...
    try {
        if (action === "search") {
            const query = args.positional.slice(1).join(" ").trim();
            if (!query)
                throw new Error("nothing to search — pass a query: memory vector search <query>");
            ...
            console.log(`memory: vector search ${report.hits.length}/${report.limit} hit(s) for "${query}"`);
            for (const hit of report.hits) {
                const source = hit.source ? ` source=${hit.source}` : "";
                console.log(`  ${hit.score.toFixed(3)} memory_nodes:${hit.rid} (${hit.node_type}) ${hit.title}${source}`);
                if (hit.excerpt)
                    console.log(`      ${hit.excerpt}`);
            }
            return;
        ...
        if (action === "status-viewer") {
            const rootDir = rootOf(args.flags);
            const outPath = stringFlag(args.flags, "out") ?? join(rootDir, ".red/memory/vector-status-viewer.html");
        ...
        console.log(`memory: vector projection ${report.overall} — ${report.ready}/${report.total} ready`);
        if (report.stale > 0)
        ...
        if (args.flags.local === true) {
            if (previousProvider == null)
                delete process.env.RED_MEMORY_VECTOR_PROVIDER;
            else
                process.env.RED_MEMORY_VECTOR_PROVIDER = previousProvider;
        }
    }
}
async function runDoctor(args) {
    ...
    try {
        ...
        if (report.stale.length === 0) {
            console.log(`memory: healthy — 0 of ${report.totalNodes} node(s) stale (unaccessed ${staleDays}+ days, never recalled)`);
            return;
        }
        console.log(`memory: ${report.stale.length} of ${report.totalNodes} node(s) stale (unaccessed ${staleDays}+ days, never recalled):`);
        ...
        if (!confirmed) {
            ...
            const rl = createInterface({ input: process.stdin, output: process.stdout });
            const answer = (await rl.question(`\nDelete ${report.stale.length} stale node(s)? Type "yes" to confirm: `)).trim();
            rl.close();
...
const HOOK_EVENTS = ["SessionStart", "PostToolUse", "Stop", "PreCompact"];
/**
...
async function runHook(args) {
    ...
    const runner = args.flags.runner === "codex" ? "codex" : "claude";
    ...
    try {
        ...
        const rootDir = rootOf(args.flags) !== process.cwd()
            ? rootOf(args.flags)
...
async function runAttempt(args) {
    ...
    const raw = await readStdin();
    if (!raw.trim())
        throw new Error("attempt record needs a JSON payload on stdin");
    const payload = JSON.parse(raw);
    const { store } = await openGraphStore(args);
    try {
        const receipt = await recordReasoningAttempt(store, payload);
        console.log(`memory: recorded attempt ${payload.repository}#${payload.issueNumber}/${payload.attemptNumber} (rid ${receipt.attemptRid})`);
    }
    finally {
        await store.close();
    }
}
async function runAttemptLearn(args) {
    const action = args.positional[1];
    if (action === "apply")
        return runAttemptLearnApply(args);
    if (action != null)
        throw new Error("memory attempt learn supports: apply");
    const rootDir = rootOf(args.flags);
    const json = args.flags.json === true;
    const writeProposal = args.flags["write-proposal"] === true;
    const { store } = await openGraphStore(args);
    try {
        ...
        const proposalFile = writeProposal
            ...
            : null;
        const state = report.proposals.length === 0
            ? "no-candidates"
            : writeProposal
                ? "proposal-written"
                : "proposal-ready";
...
async function runAttemptLearnApply(args) {
    const proposalArg = args.positional[2];
    if (!proposalArg)
        throw new Error("memory attempt learn apply needs a proposal file");
    if (args.flags.yes !== true) {
...
async function runImport(args) {
    ...
    try {
        ...
        console.log(`memory import ams: working_memory sessions=${report.working.sessions.length} events=${report.working.events} transcripts=${report.working.transcripts}`);
        console.log(`memory import ams: long_term_memory promoted=${report.long_term.promoted} reinforced=${report.long_term.reinforced} skipped=${report.long_term.skipped}`);
    }
    finally {
        await store.close();
    }
...
async function runBench(args) {
    ...
    if (sub !== "recall") {
        throw new Error(`usage: memory bench (recall|latency) ... (see \`memory --help\`)`);
    }
    const rootDir = rootOf(args.flags);
    const corpusDir = stringFlag(args.flags, "corpus") ?? join(rootDir, "plugins/memory/bench/recall");
    ...
    console.log(`memory bench recall: corpus=${report.corpus_size} queries=${report.query_count} k=${ks.join(",")}`);
    for (const k of ks) {
        ...
        console.log(`  k=${k}  ours P=${op.toFixed(3)} R=${or.toFixed(3)}  ams P=${ap.toFixed(3)} R=${ar.toFixed(3)}`);
    }
    if (outPath)
...
async function runBenchLatencyCmd(args) {
    const rootDir = rootOf(args.flags);
    const workloadDir = stringFlag(args.flags, "workload") ?? join(rootDir, "plugins/memory/bench/latency");
    ...
    if (opsRaw !== undefined) {
        const allowed = ["working-get", "session-recall", "long-term-recall"];
        const parts = opsRaw.split(",").map((s) => s.trim());
        const picked = parts.filter((p) => allowed.includes(p));
        if (picked.length === 0) {
            throw new Error(`memory bench latency: --ops must include one of ${allowed.join(", ")}`);
        }
    ...
    console.log(`memory bench latency: sessions=${workload.sessions} events/session=${workload.events_per_session} long-term=${workload.long_term_nodes} iters=${workload.iterations} (+${workload.warmup} warmup) seed=0x${workload.seed.toString(16)}`);
    for (const r of report.results) {
        console.log(`  ${r.op.padEnd(20)} ours p50=${r.ours.p50_us.toFixed(2)}µs p99=${r.ours.p99_us.toFixed(2)}µs  ams p50=${r.ams_reference.p50_us.toFixed(2)}µs p99=${r.ams_reference.p99_us.toFixed(2)}µs  speedup p99×${r.speedup.p99.toFixed(2)}`);
    }
    if (outPath)
        console.log(`  json written ${outPath}`);
    if (reportPath)
...
async function runPromoteCmd(args) {
    const rootDir = rootOf(args.flags);
    const json = Boolean(args.flags.json);
    const triggeredByFlag = stringFlag(args.flags, "triggered-by");
    const triggeredBy = triggeredByFlag === "hook" || triggeredByFlag === "overflow"
        ? triggeredByFlag
        : "explicit";
    const { store } = await openGraphStore(args);
    try {
        ...
        console.log(`memory: promoted=${report.promoted} reinforced=${report.reinforced} skipped=${report.skipped} (session=${report.session_id})`);
        for (const rid of report.promoted_rids)
            console.log(`  +promote rid=${rid}`);
        for (const rid of report.reinforced_rids)
            console.log(`  ~reinforce rid=${rid}`);
    }
    finally {
        await store.close();
    }
}
async function runAfkFinalize(args) {
    const rootDir = rootOf(args.flags);
    const json = Boolean(args.flags.json);
    const worktreeId = stringFlag(args.flags, "worktree") ??
        process.env.RED_AFK_WORKER_ID ??
        process.env.RED_AFK_ITER_DIR;
    if (!worktreeId) {
        throw new Error("memory afk-finalize: --worktree <id> required (or set RED_AFK_WORKER_ID)");
    }
    ...
    try {
        ...
        console.log(`memory afk-finalize: session=${report.session_id} worktree=${report.worktree_id} promoted=${report.promote.promoted} reinforced=${report.promote.reinforced} transcript=${report.transcript_rid ?? "none"}${report.transcript_created ? " (new)" : report.transcript_rid ? " (deduped)" : ""} dropped=${report.dropped_rids.length}`);
    }
    finally {
        await store.close();
    }
