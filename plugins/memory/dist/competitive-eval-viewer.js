export function buildCompetitiveEvalViewerArtifact(report) {
    return {
        contract: {
            name: "memory.competitive_eval.viewer",
            version: "memory.competitive_eval.viewer.v1",
...
function renderCompetitiveEvalViewer(report) {
    return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Memory Competitive Eval</title>
  <style>
    :root {
      color-scheme: light;
      --bg: #f7f7f2;
      ...
      <div class="stack">
        ...
        <section>
          ...
          <ul>
            ...
            ${guardItem("Unsupported live competitor claims", report.claimGuards.unsupportedLiveCompetitorClaims)}
            ${guardItem("Unmeasured live baselines", report.claimGuards.unmeasuredLiveBaselines)}
          </ul>
        </section>
      </div>
    </div>
    <script id="memory-competitive-eval-data" type="application/json">${jsonForScript(report)}</script>
  </main>
</body>
</html>`;
}
function dimensionItem(dimension) {
    const subChecks = dimension.subChecks ?? [];
    const subChecksHtml = subChecks.length === 0
        ? ""
        : `<div class="counts">${subChecks
            .map((check) => `<span class="count ${statusClass(check.status)}" title="${escapeHtml(check.detail)}">${escapeHtml(check.id)}: ${escapeHtml(check.status)}</span>`)
            .join("")}</div>`;
    return `<li>
    <div>
...
function baselineItem(baseline) {
    return `<li>
    <div>
...
function guardItem(label, values) {
...
function statusClass(status) {
    if (status === "fail" || status === "failed")
        return "fail";
    if (status === "warn" || status === "skipped" || status === "unavailable")
        return "warn";
    return "";
}
