...
import { buildMemoryOperationalDashboard, } from "./operational-dashboard.js";
...
import { readAutoCureRunLog, runAutoCure, } from "./auto-curation.js";
...
export async function buildMemoryWorkbench(store, rootDir, opts = {}) {
    const [dashboard, capabilities, competitiveRadar, competitiveEval, contextPack, extractionStatus, governance, handoff, workFrontier, learningDebt, memoryDecay, memoryHealth, memoryLayers, routingGuide, agentIntegrationStatus, sessionTimeline, reasoningReplay, whatif, federation, autocure, autocureRuns,] = await Promise.all([
        ...
        buildWhatifReport(store, [{ kind: "edit", description: "memory workbench preview", file: rootDir }], { limit: 3, now: opts.now }),
...
export function buildMemoryWorkbenchArtifact(workbench) {
    return {
        contract: {
...
function renderWorkbench(workbench) {
    const stateClass = workbench.dashboard.state === "ready"
        ? "ok"
...
function competitiveRadarSection(workbench) {
...
function competitiveEvalSection(workbench) {
...
function workflowCard(title, command, description) {
...
function summarySection(workbench) {
...
function capabilitiesSection(workbench) {
...
function layersSection(workbench) {
...
function contextPackSection(workbench) {
...
function workbenchCoveragePlaceholder() {
...
function handoffSection(workbench) {
...
function workFrontierSection(workbench) {
...
function routingGuideSection(workbench) {
...
function agentIntegrationStatusSection(workbench) {
...
function vectorDiagnosticsSection(workbench) {
    return `<section>
...
function governanceSection(workbench) {
...
function decaySection(workbench) {
...
function memoryHealthSection(workbench) {
...
function extractionStatusSection(workbench) {
...
function learningDebtSection(workbench) {
...
function hookDiagnosticsSection(workbench) {
...
function timelineSection(workbench) {
...
function reasoningReplaySection(workbench) {
    ...
    ${items.length === 0
        ...
        : `<ul>${items
            .map((item) => `<li class="capability"><div><h3>${escapeHtml(item.attempt_id)}</h3><p class="meta">${escapeHtml(item.when)}</p><p>${escapeHtml(item.summary)}</p></div><span class="pill ${outcomeClass(item.outcome)}" data-outcome="${escapeHtml(item.outcome)}">${escapeHtml(item.outcome)}</span><span class="pill">${item.similarity.toFixed(4)}</span></li>`)
            .join("")}</ul>`}
    ${gapsHtml}
  </section>`;
}
function whatifSection(workbench) {
    const whatif = workbench.whatif;
    const riskClass = whatif.breakage_likelihood >= 0.66
        ? "bad"
...
function federationStatusSection(workbench) {
    ...
    ${roots.length === 0
        ...
        : `<ul>${roots
            .map((root) => `<li class="capability"><div><h3>${escapeHtml(root.origin_repo)}</h3><p class="meta">${escapeHtml(root.path)}</p></div><span class="pill ${root.status === "ok" ? "ok" : "warn"}">${escapeHtml(root.status)} - ${root.hits} hit(s)</span></li>`)
            .join("")}</ul>`}
...
function autocureHealthSection(workbench) {
    const autocure = workbench.autocure;
    const runs = workbench.autocure_runs.entries;
    const recent = runs.slice(-5).reverse();
    const trend = recent
        .map((entry) => `<li><span class="pill ${entry.dry_run ? "warn" : "ok"}">${entry.dry_run ? "dry-run" : "apply"}</span> ${escapeHtml(entry.generated_at)} — entropy ${entry.entropy_before} → ${entry.entropy_after}, proposed=${entry.proposed} applied=${entry.applied} skipped_claim_guarded=${entry.skipped_claim_guarded}</li>`)
        .join("");
    return `<section>
...
function actionsSection(workbench) {
    const actions = [
...
function debtTotal(report) {
    return (report.summary.repeatedFailurePatterns +
...
function statusClass(status) {
    if (status === "ready" || status === "succeeded")
        return "ok";
    if (status === "failed" || status === "degraded" || status === "gap")
        return "bad";
    return "warn";
}
...
function agentIntegrationStatusScript() {
    return `(() => {
...
function docsReferenceGraphScript() {
    return `(() => {
...
function vectorDiagnosticsScript() {
    return `(() => {
...
function extractionStatusScript() {
    return `(() => {
