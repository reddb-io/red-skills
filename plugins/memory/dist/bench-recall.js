...
export async function loadQueries(dir) {
    ...
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed))
        throw new Error("queries.json must be a JSON array");
    return parsed.map(asQueryEntry);
}
function asCorpusEntry(value) {
    if (!value || typeof value !== "object")
        throw new Error("corpus entry must be an object");
    const r = value;
...
function asQueryEntry(value) {
    if (!value || typeof value !== "object")
        throw new Error("query entry must be an object");
    const r = value;
    const id = stringField(r, "id");
    const query = stringField(r, "query");
    const intent = stringField(r, "intent");
    const relevant_ids = Array.isArray(r.relevant_ids)
        ? r.relevant_ids.filter((t) => typeof t === "string")
        : [];
 ...
 */
export function rankAmsReference(corpus, query, k) {
 ...
 * uses.
 */
export function rankOurs(corpus, query, k) {
 ...
 */
function inferQueryTags(query, corpus) {
    const queryTokens = new Set(tokenize(query.query));
    const scored = corpus
        .map((c) => {
        let hits = 0;
    ...
    for (const s of scored)
        for (const t of s.c.tags)
            tags.add(t);
    return tags;
}
function scoreAndRank(corpus, score) {
    const scored = corpus
    ...
    for (let i = 0; i < scored.length; i++)
        ranks.set(scored[i].id, i + 1);
    return ranks;
}
const RRF_K = 60;
function rrfFuse(rankings, k) {
    const fused = new Map();
 ...
 * Metrics
 * --------------------------------------------------------------------------*/
export function precisionAtK(hits, relevant, k) {
    if (k === 0)
        return 0;
...
export async function runBenchRecall(opts) {
...
export function formatMarkdownReport(report) {
    ...
    const aggRow = (label, p, r) => `| ${label} | ${ks.map((k) => fmt(p[String(k)])).join(" | ")} | ${ks
        .map((k) => fmt(r[String(k)]))
        .join(" | ")} |`;
    const lines = [];
    lines.push(`# memory bench recall — ${report.generated_at.slice(0, 10)}`);
    lines.push("");
    lines.push(`Corpus: ${report.corpus_size} transcript chunks, ${report.query_count} labeled queries. Reference baseline is pure-vector cosine ranking (the AMS recall path per ADR 0005).`);
    lines.push("");
    ...
    lines.push("");
    lines.push("The bench is deterministic: corpus, queries, embeddings (char-bigram TF), and ranking algorithms are pure functions of the checked-in inputs. Same git ref ⇒ identical numbers. Tolerance is zero — the test suite asserts byte-equal per-query hits across runs.");
    lines.push("");
    ...
    for (const r of report.per_query) {
        lines.push(`| ${r.query_id} | ${r.intent} | ${ks.map((k) => fmt(r.ours.precision_at_k[String(k)])).join(" | ")} | ${ks
