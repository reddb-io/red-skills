...
export async function buildDocBundle(store, input) {
    const limit = Math.min(MAX_DOC_BUNDLE_LIMIT, Math.max(1, Math.floor(input.limit ?? DEFAULT_DOC_BUNDLE_LIMIT)));
    const search = await searchDocs(store, input.query, { limit });
    const packs = await Promise.all(search.hits.map((hit) => buildDocEvidencePack(store, {
        rid: hit.rid,
