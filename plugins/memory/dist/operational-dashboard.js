...
export async function buildMemoryOperationalDashboard(store, rootDir, opts = {}) {
...
export function buildMemoryOperationalDashboardArtifact(dashboard) {
    return {
        contract: {
...
function dashboardState(docs, hooks, stale, decay, warnings) {
    if (docs.vector.failed > 0 || !hooks.config_found)
        return "degraded";
    if (docs.ungrounded_docs > 0 ||
...
function dashboardActions(docs, hooks, extraction, stale, decay, warnings) {
    const actions = [];
    if (docs.ungrounded_docs > 0)
        actions.push("run `memory ingest . --root .` to refresh document graph grounding");
    ...
    if (stale.stale.length > 0)
        actions.push("review stale Memory nodes with `memory doctor`");
    if (actions.length === 0 && warnings.length === 0)
        actions.push("Memory operational dashboard is ready");
    return [...new Set(actions)];
}
function isActionableRecommendation(action) {
    return !(/\bis ready; no action required\b/i.test(action) || /\bare ready\b/i.test(action));
}
function renderOperationalDashboard(dashboard) {
...
function sourceSection(dashboard) {
    return `<section>
