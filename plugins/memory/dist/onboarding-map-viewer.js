...
export function buildOnboardingMapViewerArtifact(map) {
    return {
        contract: {
...
function renderOnboardingMapViewer(map) {
    return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
function sectionBlock(section, map) {
    const entries = map.sections[section];
    return `<section>
    <h2>${escapeHtml(SECTION_TITLES[section])}</h2>
    ${entries.length === 0 ? `<p class="empty">No ${escapeHtml(SECTION_TITLES[section].toLowerCase())} evidence.</p>` : `<ul>${entries.map((entry) => section === "suggestedSkills" ? skillItem(entry) : evidenceItem(entry)).join("")}</ul>`}
  </section>`;
}
