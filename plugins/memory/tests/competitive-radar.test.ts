...
import { buildMemoryCompetitiveRadar } from "../src/competitive-radar.js";
...
async function tempRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "memory-competitive-radar-"));
  roots.push(root);
  return root;
}
...
describe("Memory competitive radar", () => {
  test("maps capability catalog evidence to named competitor axes", async () => {
    ...
    try {
      ...
      const radar = await buildMemoryCompetitiveRadar(store, root, {
      ...
      expect(radar.competitors.map((competitor) => competitor.id)).toEqual([
        "agentmemory",
        "neo4j-agent-memory",
        "gbrain",
        "graphify",
      ...
      const agentmemory = radar.competitors.find((item) => item.id === "agentmemory");
      ...
      const graphify = radar.competitors.find((item) => item.id === "graphify");
  ...
  test(
    "CLI emits competitive radar JSON",
    async () => {
      const root = await tempRoot();
      ...
      const result = runMemory(["competitive-radar", "--root", root, "--json"]);
      expect(result.status, result.stderr).toBe(0);
      const body = JSON.parse(result.stdout) as {
        schema_version: string;
        note: string;
        summary: { competitors: number };
        competitors: Array<{ id: string; repository: string; capabilities: Array<{ id: string }> }>;
      };
      expect(body.schema_version).toBe("memory.competitive_radar.v1");
      expect(body.note).toContain("not a public benchmark claim");
      expect(body.summary.competitors).toBe(5);
      expect(body.competitors.find((item) => item.id === "gbrain")?.repository).toBe(
        "garrytan/gbrain",
      );
      expect(
        body.competitors.find((item) => item.id === "neo4j-agent-memory")?.capabilities.map(
