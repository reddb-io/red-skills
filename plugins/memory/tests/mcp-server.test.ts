  ...
  test(
    ...
    async () => {
      ...
      const radarRes = (await client.callTool({
        name: "memory_competitive_radar",
        arguments: {},
      })) as ToolResult;
      const radar = JSON.parse(radarRes.content[0]?.text ?? "{}") as {
        schema_version: string;
        summary: { competitors: number };
        competitors: Array<{ id: string; repository: string }>;
      };
      expect(radar.schema_version).toBe("memory.competitive_radar.v1");
      expect(radar.summary.competitors).toBe(5);
      expect(radar.competitors.find((item) => item.id === "agentmemory")?.repository).toBe(
        "rohitg00/agentmemory",
      );
      expect(radarRes.structuredContent).toMatchObject({
        operation_id: "memory.competitive-radar",
