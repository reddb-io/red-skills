...
describe("the duplicate-implementation ratchets can go red", () => {
  it("fails on a second custody re-queuer", () => {
    const findings = collectCustodyVerdictParkFindingsFromFiles(
      duplicate("apps/dev/src/core/landing-retry.ts", `
    ...
    expect(findings).toMatchObject([
      { rule: "custody-requeuer", kind: "duplicate", relativePath: "apps/dev/src/core/landing-retry.ts" },
    ]);
  });
  ...
  it("fails on a second requeue applier", () => {
    const findings = collectCustodyVerdictParkFindingsFromFiles(
      duplicate("apps/dev/src/commands/quick-requeue.ts", `
    ...
    expect(findings).toMatchObject([
      { rule: "requeue-applier", kind: "duplicate", relativePath: "apps/dev/src/commands/quick-requeue.ts" },
    ]);
  });
  ...
  it("fails when the classification hook is reintroduced", () => {
    const findings = collectCustodyVerdictParkFindingsFromFiles([
      ...OWNER_FILES,
      {
        relativePath: "apps/dev/src/core/feedback-override.ts",
        sourceText: `hooks.on_feedback_classify = "classify-failure";`,
      },
    ]);

    expect(findings).toMatchObject([
      { rule: "classification-hook", kind: "forbidden", relativePath: "apps/dev/src/core/feedback-override.ts" },
    ]);
  });
  ...
  it("does not treat prose or canonical API callers as rival implementations", () => {
    const findings = collectCustodyVerdictParkFindingsFromFiles([
      ...OWNER_FILES,
      {
        relativePath: "apps/dev/src/core/caller.ts",
        sourceText: `
