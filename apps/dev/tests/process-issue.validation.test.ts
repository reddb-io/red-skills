...
// ---------- trust before the expensive suite (ADR 0119, issue #2245) ----------

describe("processIssue — sensitive-path guard runs BEFORE validation", () => {
  /** Make the branch diff report a CI workflow file — an always-blocking path. */
  function withSensitiveDiff(deps: ProcessIssueDeps): void {
    const inner = deps.mergeExec;
    deps.mergeExec = async (argv) => {
      if (argv.join(" ").includes("diff --name-only")) {
        return { code: 0, stdout: ".github/workflows/ci.yml\n", stderr: "" };
      }
      return await inner(argv);
    };
  }

  it("parks sensitive-path without burning the package suite", async () => {
    const { deps, input, trace } = harness({ outcome: "done", feedbackOk: true });
    withSensitiveDiff(deps);

    const result = await processIssue(deps, input);

    expect(result.outcome).toBe("sensitive-path");
    // The whole point of the ordering: the expensive suite never ran.
    expect(trace.pnpmArgs).toEqual([]);
    expect(trace.pushedAttempt).toEqual([]);
    expect(trace.closed).toEqual([]);
    expect(trace.comments.some((c) => c.body.includes("sensitive path"))).toBe(true);
  });

  it("a clean diff still reaches the suite and lands", async () => {
    const { deps, trace, input } = harness({ outcome: "done", feedbackOk: true });
  ...
  it("a diff lookup failure is not a verdict — validation still runs, then the landing refuses", async () => {
    const { deps, input, trace } = harness({ outcome: "done", feedbackOk: true });
    const inner = deps.mergeExec;
    deps.mergeExec = async (argv) => {
      if (argv.join(" ").includes("diff --name-only")) throw new Error("git exploded");
      return await inner(argv);
    };

    const result = await processIssue(deps, input);

    // The early scan does not park on a git error…
    expect(trace.pnpmArgs.length).toBeGreaterThan(0);
    expect(trace.iterLogs.some((l) => l.includes("could not read the diff"))).toBe(true);
    // …but the landing-time guard still refuses to land an unscanned diff.
