...
`.trim();
    ...
    expect(entries[0]?.pattern).toBe("apps/dev/tests/supervisor.test.ts");
    expect(entries[1]?.pattern).toBe("apps/brain/tests/dashboard.test.ts");
  });
...
describe("validateQuarantine", () => {
  it("accepts a valid list where all entries carry an issue ref", () => {
    const entries: QuarantineEntry[] = [
      { pattern: "apps/dev/tests/supervisor.test.ts", issue: SUPERVISOR_ISSUE, since: "2026-07-02" },
      { pattern: "apps/brain/tests/dashboard.test.ts", issue: BRAIN_ISSUE, since: "2026-07-02" },
    ];
  ...
  it("throws QuarantineConfigError when entry 0 is missing an issue ref", () => {
    const entries: QuarantineEntry[] = [
      { pattern: "apps/dev/tests/supervisor.test.ts", issue: "", since: "2026-07-02" },
    ];
  ...
  it("throws QuarantineConfigError when entry 1 is missing an issue ref", () => {
    const entries: QuarantineEntry[] = [
      { pattern: "apps/dev/tests/supervisor.test.ts", issue: SUPERVISOR_ISSUE, since: "2026-07-02" },
      { pattern: "apps/brain/tests/dashboard.test.ts", issue: "   ", since: "2026-07-02" },
    ];
    expect(() => validateQuarantine(entries)).toThrow(QuarantineConfigError);
  });
  ...
  it("error message names the violating pattern", () => {
    ...
    expect(msg).toContain("apps/dev/tests/supervisor.test.ts");
...
describe("quarantineExcludeArgs", () => {
  ...
  it("returns --exclude args relative to the scope for a matching scope", () => {
    expect(quarantineExcludeArgs(entries, "apps/dev")).toEqual([
      "--exclude",
  ...
  it("returns args for all entries under the root scope", () => {
    ...
    expect(args).toEqual([
      ...
      "--exclude",
      "apps/brain/tests/dashboard.test.ts",
    ]);
  });
  ...
  it("returns empty array for an empty quarantine list", () => {
    expect(quarantineExcludeArgs([], "apps/dev")).toEqual([]);
  });
});
...
describe("scopeQuarantineEntries", () => {
  ...
  it("returns entries for the matching scope", () => {
    expect(scopeQuarantineEntries(entries, "apps/dev")).toHaveLength(1);
    expect(scopeQuarantineEntries(entries, "apps/dev")[0]?.pattern).toBe(
      "apps/dev/tests/supervisor.test.ts",
    );
  });
...
describe("runFeedback quarantine integration", () => {
  ...
  it("passes --exclude args to the test exec for quarantined patterns", async () => {
    ...
    expect(calls[0]).toEqual(["pnpm", "-C", "/wt/apps/dev", "test", "--", "--exclude", "tests/supervisor.test.ts"]);
  });
  ...
  it("emits a visible skipped sidecar record for each quarantined test", async () => {
    ...
    const qRecord = result.checks.find((c) => c.name === "quarantined:apps/dev/tests/supervisor.test.ts");
  ...
  it("does not add --exclude to non-test scripts (typecheck, lint, build)", async () => {
    const multiLayout = fakeLayout({
      packages: ["apps/dev"],
      scripts: { "apps/dev": ["test", "typecheck", "lint", "build"] },
    });
    const { exec, calls } = fakeExec();
    await runFeedback(exec, {
      worktree: "/wt",
  ...
  it("does not modify exec call when no quarantine entries match the scope", async () => {
    ...
    await runFeedback(exec, {
      ...
      layout,
      now: fakeClock(),
      quarantine: [{ pattern: "apps/brain/tests/dashboard.test.ts", issue: BRAIN_ISSUE, since: "2026-07-02" }],
    });

    expect(calls[0]).toEqual(["pnpm", "-C", "/wt/apps/dev", "test"]);
  });
...
describe("runFeedback quarantine config-error", () => {
  ...
  it("fails the gate (ok:false) when an entry is missing an issue ref", async () => {
    ...
    const result = await runFeedback(exec, {
      ...
      layout,
      now: fakeClock(),
      quarantine: [{ pattern: "apps/dev/tests/supervisor.test.ts", issue: "", since: "2026-07-02" }],
    });
...
describe("checkQuarantineStaleness", () => {
  ...
  it("returns a stale entry when the referenced issue is closed", async () => {
    ...
    expect(result.staleEntries[0]?.entry.pattern).toBe("apps/dev/tests/supervisor.test.ts");
  ...
  it("warning message names the pattern and the closed issue URL", async () => {
    ...
    expect(warning).toContain("apps/dev/tests/supervisor.test.ts");
