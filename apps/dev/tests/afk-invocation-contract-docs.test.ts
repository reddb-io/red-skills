...
describe("AFK invocation contract docs (#1946)", () => {
  it("AFK documents runtime resolution, npm direct-run fallback, and valid subcommands", async () => {
    ...
    expect(skill).toContain("npx -y -p @reddb-io/red-skills-dev@<version> red-skills-dev");
  ...
  it("shared report-runtime wrapper owns the binary precedence and missing-shim action", async () => {
    ...
    expect(wrapper).toContain("npx -y -p @reddb-io/red-skills-dev@<version> red-skills-dev <subcommand> [args]");
  ...
  it("sibling skills route through the same shared runtime contract", async () => {
    ...
    expect(go).toContain("npx -y -p @reddb-io/red-skills-dev@<version> red-skills-dev go");
    expect(retake).toContain("npx -y -p @reddb-io/red-skills-dev@<version> red-skills-dev retake");
  });
  ...
  it("installed shim failure message names the npm fallback", async () => {
    ...
    expect(script).toContain("npx -y -p @reddb-io/red-skills-dev@<version> red-skills-dev");
