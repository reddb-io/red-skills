 ...
 * (or `-r X`) optionally pins the backend; `--scout` switches to read-only
 * investigation mode. A leading `--` is tolerated so `/go -- --literal`
 * passes a dashed demand through. */
export function parseGoArgs(args: readonly string[]): { demand: string; runner?: string; scout?: boolean } {
  ...
  return { demand: positional.join(" ").trim(), runner, scout };
}
