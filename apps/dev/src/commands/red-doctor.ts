...
function renderClassifierSections(classifiers: DoctorClassifierReports): string[] {
  ...
  return [
    ...
    `worktrees checked: ${worktreeLanes.checked}`,
    `worktree lane findings: ${worktreeLanes.findings.length}`,
    ...worktreeLanes.findings.map((finding) => `  ${finding.verdict} ${finding.kind} ${finding.path}: ${finding.reason}`),
    ...worktreeLanes.findings.map((finding) => `  fix: ${finding.canonicalFix}`),
    "",
...
function renderHuman(
  ...
  const lines = [
    ...
    "",
    "red-doctor red-castle state lane",
    `red-castle lane: ${castleReport.checked.castleLanePresent ? "present" : "absent"}`,
    `legacy afk lane: ${castleReport.checked.legacyLanePresent ? "present" : "absent"}`,
    `red-castle findings: ${castleReport.findings.length}`,
...
export async function redDoctorCommand(args: readonly string[], cwd = process.cwd()): Promise<number> {
  try {
    ...
    const precheckFacts = await collectPrecheckFacts(ctx, { includeNpmBundleCoherence: true, includeLaneCensus: true, includeProcessCensus: true });
