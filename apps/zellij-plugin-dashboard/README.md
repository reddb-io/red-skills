...
cp apps/zellij-plugin-dashboard/layouts/red-dashboard.kdl ~/.config/zellij/layouts/
export RED_PANE=$PWD/apps/zellij-plugin-dashboard/bin/red-pane.sh
...
`monitor`: a watch flag would add a sleep loop to `apps/dev/src`, which is the
