...
describe("upstream skill trio docs contract", () => {
  it("registers wait-what, wizard, and to-questionnaire on every public surface", async () => {
    ...
    expect(productivityReadme).toContain("**[wait-what](./wait-what/SKILL.md)**");
  ...
  it("keeps the two explicit escape hatches user-invoked and wizard model-invocable", async () => {
    const [waitWhat, wizard, questionnaire] = await Promise.all([
      readRepoFile("plugins/dev/skills/productivity/wait-what/SKILL.md"),
    ...
    expect(waitWhat).toContain("disable-model-invocation: true");
  ...
  it("ports each behavior into RedSkills vocabulary", async () => {
    ...
    expect(waitWhat).toContain("ASD-STE100 Simplified Technical English");
    expect(waitWhat).toContain(".red/CONTEXT-MAP.md");
    expect(waitWhat).toContain(".red/contexts/<name>/CONTEXT.md");
  ...
  it("ships generated Codex sidecars and a syntactically valid wizard template", async () => {
    const sidecars = await Promise.all([
      readRepoFile("plugins/dev/skills/productivity/wait-what/agents/openai.yaml"),
