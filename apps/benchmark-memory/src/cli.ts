...
async function runBenchEvalCmd(args: ParsedArgs): Promise<number> {
  ...
  const corpusDir = stringFlag(args, "corpus") ?? join(rootDir, "apps/memory/bench/eval/structured");
...
async function runBenchRecallCmd(args: ParsedArgs): Promise<number> {
  const rootDir = resolveRepoRoot(String(process.cwd()));
  const corpusDir = stringFlag(args, "corpus") ?? join(rootDir, "apps/memory/bench/recall");
...
async function runBenchLatencyCmd(args: ParsedArgs): Promise<number> {
  const rootDir = resolveRepoRoot(String(process.cwd()));
  const workloadDir = stringFlag(args, "workload") ?? join(rootDir, "apps/memory/bench/latency");
...
function resolveRepoRoot(cwd: string): string {
  ...
  while (true) {
    if (existsSync(join(cursor, "apps/memory/bench"))) return cursor;
