...
**OpenCode host**:
The developer-facing opencode TUI surface that runs interactively (`opencode .`) on the same repo where AFK's inner-agent OpenCode runner executes headless. Treated as a **third host** alongside Claude Code and Codex (ADR 0075), with `plugins/<name>/` definitions shared across all three — the `apps/opencode-host/` adapter emits opencode-native config (`opencode.json` `provider>`, plus later slices' skills → custom tools, hooks → plugin events, MCP passthrough, agents → subagents) from the same `.red/config.yaml` AFK already reads. Reuses the ADR 0059 env-precedence and `<provider>/<model>` slug verbatim (Amendment 3 of 0059).
_Avoid_: hand-authored `opencode.json` per project, second source of truth for the same model/auth config, duplicating `SKILL.md` into opencode-native skills
