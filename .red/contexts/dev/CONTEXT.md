...
The rule deciding which GitHub API answers a given call, owned by one module (`packages/github`) that both the **redskilled** daemon and the castle import. **The principle is cardinality, not frequency**: a single-object read (`issue view`, `pr view`) goes to REST, and a multi-node listing or a multi-repository aggregate goes to GraphQL. GitHub budgets the two surfaces separately — REST by request, GraphQL by node points, Search by minute — so a hot path polling single objects over GraphQL exhausts one pool while the other sits untouched, which is the failure this rule exists to end. Cardinality is decidable statically and never changes for an operation, which is why it beats a frequency or budget-symmetry policy that would need telemetry and a different answer each release. The **quota ledger** is NOT here: the daemon owns it, because a budget is per token and a per-process view of a shared budget is the same blindness that makes per-process RSS sampling miss a cgroup's real total.
_Avoid_: rate limiter (a ledger records, it does not throttle), "the GraphQL client", per-process quota check
