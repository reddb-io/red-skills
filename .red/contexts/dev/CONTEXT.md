...
**Host hook**:
How a program **outside** RedSkills learns that the **redskilled** daemon's state changed, without polling for it (#3503). The mechanism is the host event lane itself — `~/.red/redskilled/redskilled.log.toonl`, the append-only TOONL record the daemon already writes — read by a consumer watching the file rather than by a callback the daemon dials out to. That choice serves the consumer who registers NOTHING: an outside program needs no standing with this host to watch a file, and the daemon needs no way to reach it. **A project that DOES register may ask for a callback** — Amendment 1 of ADR 0140 admits a hook scoped to the registering project, shaped as a launch template and charged to that project's budget, because the invariant was never that the daemon does not spawn but that everything it creates is admitted, budgeted and on the lane. **A declared set of event kinds fires it** (`worker-birth`, `worker-death`, …), never "any change to `host-state`" — the host state carries RSS samples that move continuously, so a consumer woken by all of it would be woken by nothing in particular. A consumer that wants the full picture asks `host-state` after the change, which is cheap precisely because the changes are rare. **A hook is a notification, never a veto** (`demand-producer.ts`): a consumer that throws, hangs or dies changes nothing about the work, and the daemon neither waits for it nor hears about it. Registration survives a daemon restart by construction, because there is no registration — the lane is a file. **Only a declared subset of the lane is public** — `worker-birth`, `worker-death`, `worker-budget-kill` — and the rest stays internal and free to change, because today 77% of the lane is `demand-refusal` and `worker-metrics` telemetry that nobody promised anybody. **The lane rotates**, which the event shape was designed for from the start (every event carries its own identity so a lane whose head was rotated away still reads); a consumer that fell behind its position re-baselines by asking `host-state` rather than by replaying history it can no longer see. **It is watchable only from the side that writes it**: the daemon detects WSL and states that the lane is not observable across the boundary, because a native-Windows consumer watching a WSL-side lane gets no events at all and silence is the worst answer a contract can give. What the hook does NOT include is the drawing: turning an event into a desktop notification, a wallpaper or any other surface belongs to the consumer's own repository, which is why #3503's second half is red-dev's and not ours. First consumer: **Redwall** (`red-dev#52`), the wallpaper that draws the live **Worker** count.
