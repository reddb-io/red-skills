...
2. **Spec conformance, no house dialect.** Output is encoded with the public TOON spec v3.2 (<https://toonformat.dev/>) via the shared encoder/decoder — never a hand-rolled variant, because reversibility (ADR 0095) depends on decoding what we emit. Status tokens are plain field values (`ok`, `FAIL`), not bracketed syntax; scalar attributes are `key: value`. A human skims it; a parser is optional.
