...
**`req:` targets must be executable issues, never a `type:prd`.** `/to-issues` and `/triage` refuse to author a `req:N` edge whose target #N carries `type:prd`, and require the author to re-point it at that PRD's concrete executable slice(s) (its `prd:N` children) instead — or, when the PRD has no slices yet, to create the named slice the dependent actually waits on and point `req:N` at that. **Rationale (#907/#928 incident, 2026-07-02):** a PRD closes only after a manual bookkeeping step long after its substance is delivered (#907/#928 reached 46/46 children closed while both PRDs stayed open), so a `req:<PRD>` edge couples slice throughput to that bookkeeping — 14 slices carrying `req:907`/`req:928` could never promote, the structural half of the queue freeze. Pointing edges at executable slices keeps the auto-unblock cascade honest: the dependent promotes the moment the *work* it waits on lands, not when someone remembers to close the PRD.
