...
- **0102** Red-castle owns the quarantined GitHub-aware AFK engine — absorbs the whole engine plus GitHub tracker adapter into red-castle under `src/engine/` and `tracker/github/`, makes `gh` an official dependency, keeps labels as injected config, emits claim/envelope/HITL/blocker formats from the castle, moves monitor into the castle, and leaves dev as the thin skin for CLI forwarding, skills/prompts/docs, hook bodies, host guardrails, and statusline.
- **0103** AFK removes attempts and recovers by fresh re-queue — deletes `{ticket}-a<N>` directories, attempt ledgers, snapshot branches, salvage-uncommitted, ExitReceipt, and attempt-progress guard; keys worktrees by `workerId-ticketId`; preserves retry caps as per-Ticket re-queue policy, worker vitals telemetry, and `prev-failure-reason` prompt annotations while treating uncommitted work as disposable.
