...
- **0097** `.red/` lifecycle taxonomy — four tiers by lifecycle (tracked knowledge/config, plugin stores, durable machine state under `.red/state/`, disposable scratch under `.red/tmp/`), a named-lane registry for every writer, lane-specific TTL policy, tmp disposability (`rm -rf .red/tmp` safe by contract), and a self-gitignore contract for `tmp/`, `state/`, and `researches/`; amends ADR 0095 so the shared Repo store lives at `.red/state/red-skills.rdb`.
