...
- **0084** Plugin runtime distribution is the launcher + Release-bundle pattern for **all** plugins (dev, memory, brain) — marketplaces ship definitions only, `.red/config.yaml` gates activation (0067), a committed launcher resolves the Release bundle into `~/.cache/red-skills/` on first enabled session start with background in-range self-update (never sync in a render path, #379 lesson), and `/setup-red-skills` + `/doctor` are the control plane auditing enabled-vs-runtime-present, version, and freshness. Kills the memory/brain silent hook no-op; rejects committed `dist/` and build-on-install *(extends 0038/0039 to memory/brain; relies on 0067 gate, 0052 naming, 0040 versioning)*
