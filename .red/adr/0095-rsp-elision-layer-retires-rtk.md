...
- **Normalization** — ANSI codes, carriage-return progress bars, trailing whitespace, repeated blank lines — is a **closed allowlist** of provably information-free removals: silent, no handle. The allowlist grows only by explicit decision, mirroring the mechanical/intent split the AFK gate already uses.
