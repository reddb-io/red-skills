...
**Declare the HITL types to the tracker, or the dependency machinery will queue them for an agent.** On a decision-shaped map most dependents of a decision ticket are themselves grilling or prototype, so every resolution would push human decisions into the autonomous lane (#2966). The label and the declaration are one protection with two halves, so install them in one act — `npx -y -p @reddb-io/red-skills-dev@<version> red-skills-dev install-type-labels` creates the `wayfinder:<type>` labels and lists `wayfinder:grilling` + `wayfinder:prototype` in `.red/config.yaml` under `plugins.dev.afk.labels.hitl_types` (#3013). Never create a type label with bare `gh label create`: the label without the declaration reads as protection while every unblocked HITL child still enters the queue. Declared, an unblocked HITL child parks `ready-for-human` instead. See [triage-labels.md](../red-setup/triage-labels.md) *HUMAN-ONLY types*.
