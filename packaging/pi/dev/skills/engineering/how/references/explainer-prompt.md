...
Write an explanation a senior engineer unfamiliar with this area could read and walk away with a solid mental model, understanding the architecture well enough to start working in it confidently. When the project keeps a domain glossary (`.red/CONTEXT-MAP.md` pointing at `.red/contexts/<name>/CONTEXT.md`), use its ubiquitous language for domain terms instead of inventing synonyms.
