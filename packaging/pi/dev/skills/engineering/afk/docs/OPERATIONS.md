...
| `.red/tmp/workers/{id}/{N}-a{n}/implementer-runtime.toon` | Per-run skill-projection artifact: enabled plugins, selected `plugin:skill` names, exact serialized skill-manifest bytes, projection-setup timing, and actual runner startup from invocation to first stream event. A configured historical startup baseline provides the before/delta comparison. Finalized measurements are emitted as `worker.implementer-environment`, stamped into worker state, and aggregated by `/afk dashboard`. |
