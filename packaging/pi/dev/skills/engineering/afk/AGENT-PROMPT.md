...
0. **Already-done short-circuit (do this first, every invocation).** Before any exploration, check whether the branch you are on **already** satisfies the issue. Read the handoff, then `git log --oneline origin/main..HEAD` and inspect the tip commit against the acceptance criteria. If the work is already present and correct — a prior worker or invocation finished it — do **not** re-explore the codebase, re-plan, or re-run a full-suite sanity pass: run your touched package's gate **once** if you need confidence, append a one-line `<agent-notes>` entry recording that the work was already complete, emit the Acceptance Summary, and emit `<promise>DONE</promise>` as your final line. Steps 1–5 are skipped entirely on this path. This short-circuit is what stops a re-invoked agent from grinding iterations re-verifying a finished commit (see *"Already done" still requires the sentinel* above). Only fall through to step 1 when the branch does **not** yet carry the complete change.
