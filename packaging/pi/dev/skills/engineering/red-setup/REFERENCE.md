...
- **Required host binaries** — install `tq` at or above `TQ_VERSION=v0.21.0` through the toon repo installer and record `host_binaries.tq.version` as the floor, so `/red-doctor` can enforce the no-jq-fallback TOON/TOONL contract in a repo that has no pnpm catalog to derive it from
...
- `tq --version` and `.red/config.yaml` `host_binaries.tq.version` — is the required host binary present, and at or above the `0.21.0` floor? A newer `tq` is correct, not drift.
