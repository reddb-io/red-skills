...
- **Required host binaries** — install `tq` at or above `0.21.0` from the official `reddb-io-tq` crate and record `host_binaries.tq.version` as the floor, so `/red-doctor` can enforce the no-jq-fallback TOON/TOONL contract in a repo that has no pnpm catalog to derive it from
