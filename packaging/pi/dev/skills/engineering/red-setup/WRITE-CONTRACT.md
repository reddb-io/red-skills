...
- The Section E2 required-host-binary record: `host_binaries.tq.version: 0.26.2`
...
- The Section G Worktree setup facts and the exact confirmed `plugins.dev.afk.setup` command(s), including any `LEFTHOOK=0` / `HUSKY=0` opt-out
- The Section G harness inventory (package/path plus discovered `test`, `typecheck`, `lint`, and `build` scripts), CI/merge-queue evidence, and the exact confirmed `plugins.dev.afk.validation` block
- The Section G Release standard choices, detected and extra Version surfaces, and the exact confirmed `release.*` block
...
**No-clobber rule (governs every write below).** Never overwrite, rewrite, or reorder content in a file this skill did not just create: if a target already exists, skip it, log a one-line notice, and move on — no second ask, and never `git add` on the user's behalf. Surgical merges are allowed only where a step below names them: plugin activation/rsp defaults, the user-confirmed `plugins.dev.afk.setup` authority, the exact confirmed `plugins.dev.afk.validation` schedule, the exact confirmed `release.*` block, accepted command guards/hooks, the required host-binary pin, and missing `.red/.gitignore` state-lane entries. (Copied workflow YAML is the one "ask, don't silently skip" case — diff and ask first.)
...
1. If `.red/config.yaml` already exists at the repo root, apply the named surgical exceptions: add/update the `plugins.<name>.enabled` flags to match Section A0, add/update the top-level `rsp:` defaults, write the exact user-confirmed ordered command(s) under canonical `plugins.dev.afk.setup`, and — only when each is separately confirmed in Section G — merge the exact task routes under `plugins.dev.afk.routes`, Validation moments schedule under `plugins.dev.afk.validation`, and the exact `release.*` block. Preserve every unrelated key and comment. Log `.red/config.yaml present - merged plugin activation, rsp defaults, and confirmed AFK route/setup/Validation and Release authority; left the rest as-is`.
2. Otherwise, ensure `.red/` exists (this section is the authorized creator) and copy [config-template.yaml](./config-template.yaml) to `.red/config.yaml`, then surgically edit only the template-provided values that Section A0 selected and replace the commented `plugins.dev.afk.setup` example with the confirmed command(s). Do not compose the `plugins:` block from memory: start from the shipped template and preserve its key nesting. The `rsp` block ships enabled with default retention because the hook remains strictly per-repo opt-in. The rest of the template is a fully-commented snapshot of the documented config knobs, so it stays a no-op until lines are uncommented — including `command_guard`, the three Validation moments, and `release.*`; setup is the deliberate exception because AFK must not guess dependency installation.
...
4. **Self-ignore `.red/`'s local state.** Whenever `.red/` exists (fresh scaffold or pre-existing), make the directory protect itself so `.red/tmp`, `.red/state`, and `.red/researches` never get committed regardless of the repo-root `.gitignore`. Write `.red/.gitignore` if it is **missing** with exactly:
   ...
   If `.red/.gitignore` already **exists**, apply the **gitignore-append exception**: append whichever of `tmp/`, `state/`, or `researches/` is missing, and never rewrite or reorder existing lines. Keep tracked `.red` content (`config.yaml`, `contexts/`, `adr/`, `agents/`, `contracts/`, `hooks/`) committable — only local state and generated research reports are ignored. Do **not** `git add` `.red/.gitignore` (step 5 — the user controls when `.red/` lands in git).
5. **Task-class route proposal (fresh or existing `.red/config.yaml`).** Show the four runner/model/effort results from Section G and the complete `plugins.dev.afk.routes` YAML. On explicit approval, replace the fresh template's commented route example or surgically merge only the confirmed classes into an existing config. Preserve unmapped classes and unrelated config. Without approval, make no route edit; `afk.default_runner` remains the fallback.
6. **Validation moments proposal (fresh or existing `.red/config.yaml`).** Use the Section G package-manifest inventory and CI/merge-queue evidence to present the exact proposed `plugins.dev.afk.validation` YAML. Name every discovered script and package before proposing the package-manager invocation, explain each moment and every omitted moment, and ask for edits or approval. Never write discovered commands without confirmation. On explicit approval, replace the fresh template's commented example or surgically merge the confirmed `iteration`, `post_done`, and `landing` lists into an existing config; preserve unrelated config and remove no legacy alias unless the operator explicitly approved that migration. Without approval, make no Validation moments edit.
7. **Release standard proposal (fresh or existing `.red/config.yaml`).** Use the Section G release choices and workspace inspection to show the exact top-level `release.*` YAML: `scheme`, `trigger`, `prerelease`, `execution`, non-empty `version_surfaces`, and optional `sync_command`. Keep every detected npm/Cargo manifest, preserve existing declared exotic surfaces on rerun, and ask the operator to add or edit carriers before approval. Never write detected Version surfaces without confirmation. On explicit approval, replace the fresh template's commented example or surgically merge the exact confirmed block into an existing config while preserving unrelated keys and comments. Without approval, make no Release standard edit. This confirmation gate is identical for a fresh or existing `.red/config.yaml`. Print the prerequisite that every generated job that pushes a ref — the tag-cutting job, and the version-branch job under the Version-PR trigger — needs a repository secret named `RELEASE_PAT`, holding a contents-write credential whose pushes can start workflows; the operator provisions the secret before the first release, and setup never asks for or stores its value.
...
9. **Mandatory post-write config check.** Load the just-written `.red/config.yaml` through the real dev config loader before setup can finish. From a source checkout, run `pnpm --filter @reddb-io/dev dev red-doctor` from the target repo root; from an installed plugin, call the rs_dev `red_doctor` read tool. If the loader prints an off-contract-spelling warning such as root-level `dev.*` or bare `afk.*`, treat the check as red: fix `.red/config.yaml` back to the template spelling, re-run the same check, and only continue when the warning is gone.
10. Do **not** `git add` or commit `.red/config.yaml` or `.red/.gitignore` — the user controls when they land in git.
...
1. Install the pinned official crate exactly as documented:
   ...
   ```bash
   cargo install reddb-io-tq --version 0.26.2 --locked --force
   ```

2. Verify `tq --version` reports `0.26.2` or newer. If it is older, stop and report the mismatch; do not offer a jq fallback.
...
1. Write the canonical block from this skill rather than hand-editing prose into the rules file. `/red-setup` owns every write into a repository's agent rules (ADR 0147 rule 1 left it no shipped injector to shell out to): upsert the `## Development workflow` section — including the four-Working-mode table — into both `AGENTS.md` and `CLAUDE.md`, create `.red/config.yaml` if still missing, and set `plugins.dev.lock.primary-branch: true`. From a source checkout the same writer is reachable as `pnpm --filter @reddb-io/dev dev inject-development-workflow --root <repo-root>`.
2. Making the same changes by hand: add or update the canonical `plugins:` → `dev:` → `lock:` block in `.red/config.yaml` so `primary-branch` is `true`, then upsert the canonical `## Development workflow` block in both `AGENTS.md` and `CLAUDE.md`. Never append a duplicate block; update the existing section in place. Leave any legacy top-level `dev.lock.*` keys untouched; `/red-doctor --fix` owns that migration.
...
1. **Ask the session, not the disk.** For each plugin just enabled, look for the MCP tools it declares in `plugins/<name>/.mcp.json` (`dev` declares `rs_dev`). Hosts prefix them, so the identifiers to look for are `mcp__<slug>__<tool>` (for example `mcp__plugin_dev_rs_dev__project_status`). Resolve them with a tool search; do not infer presence from the manifest you just read.
