...
1. If `.red/config.yaml` already exists at the repo root, apply the named surgical exceptions: add/update the `plugins.<name>.enabled` flags to match Section A0, add/update the top-level `rsp:` defaults, write the exact user-confirmed ordered command(s) under canonical `plugins.dev.afk.setup`, and — only when separately confirmed in Section G — merge the exact Validation moments schedule under `plugins.dev.afk.validation`. Preserve every unrelated key and comment. Log `.red/config.yaml present - merged plugin activation, rsp defaults, and confirmed AFK setup/Validation authority; left the rest as-is`.
2. Otherwise, ensure `.red/` exists (this section is the authorized creator) and copy [config-template.yaml](./config-template.yaml) to `.red/config.yaml`, then surgically edit only the template-provided values that Section A0 selected and replace the commented `plugins.dev.afk.setup` example with the confirmed command(s). Do not compose the `plugins:` block from memory: start from the shipped template and preserve its key nesting. The `rsp` block ships enabled with default retention because the hook remains strictly per-repo opt-in. The rest of the template is a fully-commented snapshot of every v1 knob the AFK config loader (`apps/dev/src/core/config.ts`) reads, so it stays a no-op until lines are uncommented — including `command_guard` and the three Validation moments; setup is the deliberate exception because AFK must not guess dependency installation.
