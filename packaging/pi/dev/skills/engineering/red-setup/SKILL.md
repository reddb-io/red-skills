...
7. **Report the files this run wrote and never committed — before the recap.** Name every path (`.red/config.yaml`, `.red/.gitignore`, `.red/hooks/**`), say they are uncommitted, offer the exact `git commit` and leave the decision with the user. See [WRITE-CONTRACT.md](./WRITE-CONTRACT.md#report-the-files-this-run-wrote-and-never-committed).
8. **Finish with the setup recap.** Tell the user which plugins are enabled here, carry the session verdict from step 6 into it, say `written, restart needed` when this run wrote the `statusLine` (a skipped write announces nothing — see [WRITE-CONTRACT.md](./WRITE-CONTRACT.md#write)), remind them that other directories stay inert until setup runs there too, point memory/brain users at their next init step, and route one-off concrete work through `/go`, backlog work through `/afk`, and parked issues through `/retake`.
...
- **Do not add generated setup files for the user — but never leave silently.** Do not `git add` `.red/config.yaml`, `.red/.gitignore`, or hook files written by this setup; the decision is the operator's. **A run that writes those files and says nothing about them is an unclosed cycle (#3106)**: it leaves the tree dirty by contract and the operator with no idea it did. Name them, offer the commit, run it only if asked.
...
- **Validation commands are proposal-only.** Discover `test`, `typecheck`, `lint`, and `build` scripts from the repository's package manifests, show the exact proposed `plugins.dev.afk.validation` block, and leave fresh and existing configs untouched until the operator confirms it.
- **Release configuration is proposal-only.** After Validation moments, collect the scheme, trigger, pre-release, and execution choices; detect and propose the Version surfaces; then leave `release.*` untouched in fresh and existing configs until the operator confirms the complete block.
