#!/usr/bin/env node
/**
 * red-skills-memory — thin shim that execs the `memory` runtime bundle packaged
 * inside this npm tarball (`dist/memory.bundle.min.mjs`). ADR 0091: the JS bundle
 * ships in the tarball; no postinstall download. (The Memory runtime's native
 * `red` engine binary is resolved separately by the plugin at runtime — see the
 * memory bootstrap — because per-platform native binaries cannot live in a
